...
export default function Page() {
  ...
  const [logLine, setLogLine] = useState(
    "Upload your Amazon order CSV to begin.",
  );
  ...
  function loadText(text, name) {
    ...
    setBatchTotal(0);
    setLogLine(
      `Loaded ${list.length} products. Press “Process batch” when ready.`,
    );
  }
  ...
  async function runBatch(limit) {
    ...
    setBatchCursor(0);
    setLogLine(
      `Processing ${slice.length} products in the background of this tab…`,
    );
    ...
    for (let i = 0; i < slice.length; i++) {
      ...
      setBatchCursor(i + 1);
      setLogLine(
        `Working ${i + 1} of ${slice.length}: ${slice[i].title.slice(0, 60)}…`,
      );
    ...
    if (!stopRef.current) {
      setLogLine(
        `Batch finished. ${ok} of ${slice.length} packs ready. Scroll down to download.`,
      );
    }
  }
  ...
  const doneCount = Object.values(packs).filter(
    (p) => p.status === "done",
  ).length;
  const errorCount = Object.values(packs).filter(
    (p) => p.status === "error",
  ).length;
  ...
  return (
    <main style={styles.wrap}>
      <h1 style={styles.h1}>Family order packs</h1>
      <p style={styles.sub}>
        Private tool for you and your family. Upload orders once, process a
        batch, download lifestyle pictures. Leave this tab open while it runs.
      </p>
      ...
      <section style={styles.card}>
        ...
        <p style={{ color: "#a0a0b8", fontSize: 14, marginBottom: 12 }}>
          The app walks products for you (not one manual click each). Start with
          10 if you want a shorter run.
        </p>
            ...
            <div style={styles.barBg}>
              <div
                style={{
                  ...styles.barFill,
                  width: batchTotal
                    ? `${Math.round((batchCursor / batchTotal) * 100)}%`
                    : "0%",
                }}
              />
            </div>
                      ...
                      {(pack.images || []).map((im) => (
                        <div key={im.index}>
                          ...
                          <button
                            ...
                            onClick={() =>
                              downloadImg(im.url, p.asin, im.index)
                            }
                          >
                            Save {im.index}
                          </button>
      ...
      <p
        style={{ fontSize: 13, color: "#555", marginTop: 24, lineHeight: 1.5 }}
      >
        Family-only. Bookmark this site. Your daughter only needs the password
        and this page — no PowerShell, no GitHub.
      </p>
    </main>
  );
}
