...
| 54 | `arsc-labels.yml` | ARSC label management — auto-labels new issues (`triage`), draft PRs (`draft`), and supports manual add/remove/set/clear operations | `issues` opened, `pull_request`, Manual (`workflow_dispatch`) |
