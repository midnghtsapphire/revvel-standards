...
- **Full access is deliberate.** This build removes least-privilege, sandbox,
  human-escalation, and the "may not modify your own permissions" rule.
- **Snapshot-before-self-edit is kept** so the agent doesn't brick itself.
- **Honesty line is kept** so the agent doesn't invent results.
- **Pairs with:** SKILL.md in the source product folder.
