...
| `Fleet Controller (us) via .github/workflows/fleet-controller.yml` | Credit-free, fail-open grid scheduler over every orchestrator (`GITHUB_TOKEN` only). Cuts + reassigns stalled/runaway runs to fallback LLMs, then escalates to self-healing. Core: `scripts/controller/core.js`; driver: `scripts/controller/controller.js`. Feeds: `docs/controller/controller-status.json` (Lovable), `controller-ingestion.json` (self-healing). Design: `docs/controller/README.md`. |
