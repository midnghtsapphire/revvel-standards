...
5. **Add the service token to GitHub**: Repo Settings → Secrets and variables → Actions → `DOPPLER_AGENT_TOKEN` (recommended — has Admin access to all projects) or any of `DOPPLER_TOKEN`, `DOPPLER_LOCAL_TOKEN`, `DOPPLER_API_KEY`, `DOPPLER_AGENT_ODIC`, `DOPPLER_CIRCLECI_OIDC`
   - All workflows check all six names automatically in this priority order: `DOPPLER_AGENT_TOKEN` → `DOPPLER_TOKEN` → `DOPPLER_LOCAL_TOKEN` → `DOPPLER_API_KEY` → `DOPPLER_AGENT_ODIC` → `DOPPLER_CIRCLECI_OIDC`
