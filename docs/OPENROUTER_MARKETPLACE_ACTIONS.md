...
| 14 | XAI Review | [marketplace](https://github.com/marketplace/actions/xai-review) — [`Nikita-Filonov/ai-review`](https://github.com/Nikita-Filonov/ai-review) | [`xai-review.yml`](../templates/cicd/xai-review.yml) | ✅ | Opt-in inline/context/summary PR review via OpenRouter (`/xai-review`, `xai-review` label, or manual dispatch) |
...
| `xai-review.yml` | Opt-in only (`/xai-review`, `xai-review` label, dispatch) | Inline + context + summary review via `Nikita-Filonov/ai-review` |
...
`iara-code-reviewer`, `bc-ai-code-reviewer`, `gass-scoring`) that auto-trigger
on `pull_request` are complementary — each has a different lens (summary,
inline, SAST, domain-specific, on-chain scoring) — but if you want only one
for a given repo, delete the others. `xai-review.yml` is deliberately
**opt-in only** (label / slash / dispatch) so it does not reintroduce the
multi-bot auto-noise cut in `CODE_REVIEW_WORKFLOW_STATUS.md`.
...
`iara-code-reviewer`, and the `xai-review` label lane).
...
- **`xai-review.yml`** — default model `google/gemini-2.5-flash`, opt-in
  only (label / `/xai-review` / dispatch). Default command is
  `run-summary` (cheapest useful mode); use `run-inline` /
  `run-context` only when you need line-level or cross-file depth.
