# Grant Readiness System

> **For:** Audrey Evans - Fidelity Trust Services + All Businesses
> **Created:** 2026-04-25
> **Status:** URGENT - May 2026 CAGE Code Renewal

---

## ⛔ EXPIRED: May 2026 Deadlines (Past Due)

| Item                                                     | Due            | Status     |
| -------------------------------------------------------- | -------------- | ---------- |
| SAM.gov CAGE Code Renewal (90SN0 — Freedom Angel Corp)   | **2026-05-14** | ⛔ EXPIRED |
| SAM.gov CAGE Code Renewal (8ZRW3 — XI Website Solutions) | **2026-05-05** | ⛔ EXPIRED |
| DUNS Number                                              | ASAP           | ⚠️ Missing |
| UEI Registration                                         | ASAP           | ⚠️ Missing |

**DLA Contact for CAGE renewal:** 1-877-352-2255 · <dlacontactcenter@dla.mil> · <https://cage.dla.mil>

---

## Required Registrations

### 1. SAM.gov (System for Award Management)

**What it does:**

- Federal contractor registration
- Required for federal grants AND contracts
- CAGE code assignment

**CAGE Code:**

- 5-character identifier
- Required for government contracts
- **RENEW EVERY 5 YEARS**
- Freedom Angel Corp: **90SN0** (EXPIRED 2026-05-14)
- XI Website Solutions: **8ZRW3** (EXPIRED 2026-05-05)

**DLA CAGE Renewal Contact:**

- Phone: **1-877-352-2255** (1-877-DLA-CALL, 24/7)
- Email: **<dlacontactcenter@dla.mil>**
- Portal: <https://cage.dla.mil>
- Renew at: <https://sam.gov>

**Action:**

- [ ] Log into sam.gov
- [ ] Verify CAGE code status
- [ ] Submit renewal for both CAGE codes
- [ ] Update entity information

### 2. UEI (Unique Entity ID)

**What it is:**

- 12-character unique ID
- Replaced DUNS in 2022
- Required for grants.gov

**How to get:**

1. Go to sam.gov/entity
2. Create account
3. Register entity
4. Get UEI instantly (free)

**Action:**

- [ ] Register at sam.gov/entity
- [ ] Get UEI: _____________
- [ ] Save confirmation

### 3. Grants.gov

**What it does:**

- Central portal for federal grants
- 26 federal grant-making agencies
- 1,000+ grant programs

**Registration:**

- [ ] Create grants.gov account
- [ ] Add UEI
- [ ] Subscribe to alerts

---

## Grant Opportunities by Business

### Fidelity Trust Services

| Grant                             | Agency      | Purpose            | Requirements               |
| --------------------------------- | ----------- | ------------------ | -------------------------- |
| Legal Services Corp               | DOJ         | Legal aid          | Non-profit, legal services |
| State Court Improvement           | CO Judicial | Court programs     | CO entity                  |
| Community Development Block Grant | HUD         | Community services | CO locality                |

### Rental Company

| Grant                       | Agency | Purpose   | Requirements   |
| --------------------------- | ------ | --------- | -------------- |
| SBA Microloan               | SBA    | Equipment | Small business |
| Rural Business Development  | USDA   | Rural ops | Rural area     |
| State Small Business Credit | CO     | Growth    | CO business    |

### All Businesses

| Grant             | Agency | Purpose     | Requirements         |
| ----------------- | ------ | ----------- | -------------------- |
| R&D Tax Credit    | IRS    | Innovation  | R&D activities       |
| WOTC              | DOL    | Hiring      | Qualified employees  |
| Disabled Veterans | VA     | Contracting | Service-disabled vet |

---

## Grant Discovery Resources

| Resource            | URL                 | Notes               |
| ------------------- | ------------------- | ------------------- |
| Grants.gov          | grants.gov          | Federal grants      |
| SAM.gov             | sam.gov             | Contracts + grants  |
| USAspending.gov     | usaspending.gov     | Award data          |
| Colorado.gov/grants | colorado.gov/grants | State grants        |
| COBiz               | cobiz.co            | State opportunities |

---

## Monthly Grant Scan Routine

### First Monday of Month

1. Check grants.gov new opportunities
2. Filter by: Legal, Business, Colorado
3. Save matches to tracker
4. Note deadlines

### Weekly

1. Check agency-specific listings
2. Review state opportunities
3. Update tracking spreadsheet

---

## Grant Application Checklist

### Required for ALL federal grants

- [ ] UEI (from SAM.gov)
- [ ] TIN/EIN
- [ ] Bank account (for direct deposit)
- [ ] SAM.gov active registration
- [ ] Capabilities statement
- [ ] Budget (SF-424A)
- [ ] Timeline/Logic model

### Additional for Fidelity Trust

- [ ] Proof of CLE certification
- [ ] Organizational chart
- [ ] Board list (if non-profit)
- [ ] 501(c)(3) letter (if applicable)

---

## Tracking Spreadsheet Columns

| A          | B      | C        | D                 | E      | F     | G          |
| ---------- | ------ | -------- | ----------------- | ------ | ----- | ---------- |
| Grant Name | Agency | Due Date | Requirements Met? | Status | Notes | Submitted? |

---

## Next Steps This Week

1. **TODAY:** Verify CAGE code in SAM.gov
2. **THIS WEEK:** Get UEI from sam.gov/entity
3. **THIS WEEK:** Create grants.gov account
4. **THIS MONTH:** Set up monthly scan routine

---

_Grant Readiness System for revvel-standards_
