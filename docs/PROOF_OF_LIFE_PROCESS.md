...
**Issue reference:** _"proof of life new app-review revvel-standards-run process"_ — stand up a manually-triggered **proof-of-life** for the new app-review `revvel-standards-run` pipeline, with **OpenRouter as the orchestrator or fixer** and a **choice in `assigned to`** so the run can either work **independently of Copilot** or **use Copilot / Codex within OpenRouter**.
...
1. Open **Actions → _Proof of Life — app-review revvel-standards-run_ → Run workflow**.
...
4. _(Optional)_ Enter a `target_issue` (issue or PR number) to have the workflow assign, label and comment on it. Leave blank for a log-only heartbeat.
...
| `target_issue` | string (number)                                       | _(empty)_      | Optional issue or PR number to assign / label / comment on. Blank → log-only run.                                                                                                       |
| `dry_run`      | boolean                                               | `false`        | Log what _would_ happen without making any GitHub mutations.                                                                                                                            |
...
| ---------------- | ------------------ | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `openrouter`     | _(none)_           | `openrouter`, `proof-of-life`, `role-<role>`            | **Independent of Copilot.** OpenRouter owns the run end-to-end; routing is label-only.                     |
| `Copilot`        | `@Copilot`         | `openrouter`, `proof-of-life`, `role-<role>`, `copilot` | Orchestrated by OpenRouter, executed by GitHub Copilot.                                                    |
| `codex`          | _(none)_           | `openrouter`, `proof-of-life`, `role-<role>`, `codex`   | Orchestrated by OpenRouter, executed by the Codex model (`openai/codex-5.1`) — label-only, no GitHub user. |
| `Rex`            | _(none)_           | `openrouter`, `proof-of-life`, `role-<role>`, `rex`     | oAudrey Agent Factory lane for no-key Perplexity and research-first runs — label-only, no GitHub user.     |
...
- The workflow does **not fail** if the secret is missing; it logs a warning and annotates the proof-of-life comment with `⚠️ not configured` so you can see at a glance which repos still need provisioning. That is intentional — the whole point of _proof of life_ is to surface configuration state, not to fail on it.
...
1. _Actions_ → _Proof of Life — app-review revvel-standards-run_ → _Run workflow_.
2. Fill in the inputs, click _Run workflow_.
