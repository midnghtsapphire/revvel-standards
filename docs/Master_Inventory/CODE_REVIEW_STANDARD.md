...
_Important Note: While the Dev → Test → Live pipeline is the official standard, we are currently operating under a "Live-First" deployment exception to save time and avoid sandbox environment limitations._
...
Waydev reads only repository _metadata_ (commit timestamps, PR titles, author usernames). It does **not** access source code content and has no write permissions to any repository.
