...
- **Net new infra cost: $0 baseline.** GitHub Pages is unchanged. The only variable cost is OpenRouter token spend, which is gated by a free-tier rate limit (see §5) and a server-side proxy (Cloudflare Worker on the existing Cloudflare account, or — if Workers are not desired — a one-file Vercel Edge Function on the free hobby tier). No new vendor onboarding required.
