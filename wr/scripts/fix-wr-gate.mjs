...
const CLOSES_REF =
  /\b(?:clos(?:e|es|ed)|fix(?:e|es|ed)?|resolv(?:e|es|ed))\s+(?:[A-Za-z0-9_./-]+)?#\d+/i;
...
const FIX_SIGNAL =
  /\b(fix|resolve|repair|correct|patch|remove|add (missing|the)|implement)\b/i;
...
if (claimsFix && wrOnly && !isTrackingOnly && isResearchWR) {
  console.log(
    `note: weekly-research WR accepted (research deliverable that links its originating issue). The fix lands in a follow-up against that issue.`,
  );
}
...
if (claimsFix && wrOnly && isTrackingOnly) {
  console.log(
    `note: tracking-only fix WR accepted (labeled + Tracks: ref present). Ensure the referenced follow-up actually applies the fix.`,
  );
}
