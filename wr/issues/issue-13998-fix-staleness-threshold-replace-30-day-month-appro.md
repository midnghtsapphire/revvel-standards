# WR: [WR] Fix staleness threshold: replace 30-day month approximation with accurate days
...
| Description | ## Summary
The staleness threshold calculation in the third-party action audit script approximates every calendar month as exactly 30 days. This introduces a silent inaccuracy of up to 15 days depending on the configured threshold, which can cause actions to be incorrectly flagged as stale before their true calendar-based deadline has passed.

## Details
The expression stale_threshold_days=$(( STALENESS_MONTHS * 30 )) is used to convert a month-based configuration value into a day count. For a 12-month threshold, this yields 360 days rather than the correct ~365, meaning an action published 361 days ago would be treated as stale despite falling within the intended calendar year window. Since the staleness threshold is the single most critical parameter governing the audit's behavior — directly determining which actions trigger work requests — even minor imprecision here can produce incorrect and potentially disruptive outcomes. The error compounds at higher month values and is invisible to operators who configure the system in months expecting calendar-accurate results.

## Location
File: scripts/audit-third-party-actions.sh, line 133
...
## Suggested Action
1. Replace the 30-day constant with a more accurate approximation. For a standard year-based default, substitute the multiplier with 365, yielding stale_threshold_days=$(( STALENESS_MONTHS * 365 / 12 )) to preserve month-level granularity while improving accuracy.
2. Alternatively, if the common use case is a fixed 12-month threshold, consider hardcoding 365 days directly and documenting the assumption explicitly.
3. Add a comment near the calculation explaining the chosen approximation and its known margin of error so future maintainers understand the trade-off.
4. Review any existing flagged actions produced under the previous calculation to determine whether any were incorrectly marked as stale. |
| Private | {IS_PRIVATE} |
