# WR: [WR] Fix broken reference to missing docs/PROVENANCE_STANDARD.md in tracking doc
...
| Description | ## Summary
The auto-merge gate tracking document references a file, docs/PROVENANCE_STANDARD.md, that does not currently exist in the repository. Both Item 2 acceptance criteria and the Tracking section at the bottom of the document cite this file, meaning implementers following the document will encounter a dead reference and may be uncertain how to proceed.

## Details
The tracking document instructs implementers to add a row to docs/PROVENANCE_STANDARD.md as part of fulfilling Item 2 acceptance criteria. The same non-existent path is also cited in the Tracking section of the document. Because the file has not yet been created, these references are broken. While the omission is low-severity and the file can be created when the item is promoted, leaving the broken reference unaddressed risks confusion for contributors who attempt to follow the documented process before the file exists.

## Location
File: wr/issues/issue-octopus-audit-2026-05-28-tracking.md, line 60
...
## Suggested Action
1. Either create a stub docs/PROVENANCE_STANDARD.md file in this PR so the reference resolves immediately, or update the acceptance criteria and Tracking section to explicitly note that docs/PROVENANCE_STANDARD.md will be created as part of promoting Item 2.
2. If deferring file creation, add an inline comment or note in the tracking document clarifying that the file does not yet exist and will be introduced in the promotion PR.
3. Once docs/PROVENANCE_STANDARD.md is created, verify all references in the tracking document point to the correct path. |
| Private | {IS_PRIVATE} |
