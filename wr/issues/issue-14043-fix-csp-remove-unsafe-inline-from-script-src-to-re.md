# WR: [WR] Fix CSP: remove 'unsafe-inline' from script-src to restore XSS protection
...
| Description | ## Summary
The Content Security Policy defined in fieldwork/index.html includes 'unsafe-inline' in the script-src directive, which effectively nullifies the XSS protection CSP is designed to provide. Any attacker capable of injecting an inline script — through a reflected or stored XSS vector — can fully bypass the policy. The codebase already acknowledges this as a temporary measure, but the current state creates a false sense of security in a production environment.

## Details
CSP is a defence-in-depth mechanism that restricts which scripts the browser will execute. Permitting 'unsafe-inline' in script-src means the browser will execute any inline <script> block regardless of its origin, rendering the directive largely useless against the most common XSS payloads. The root cause appears to be a dependency on an inline tailwind.config block required by the Tailwind CDN script. Until a proper build-time bundle is in place, this unsafe permission has been left in the policy. However, the risk is real and the workaround is feasible without a full build pipeline change: either generate a per-request server-side nonce, embed it in the CSP header as 'nonce-<value>', and add the matching nonce attribute to each legitimate <script> tag; or extract the Tailwind configuration to a separate static .js file loaded via src attribute, eliminating the need for any inline script execution at all.

## Location
File: fieldwork/index.html, line 6
Pull Request: Octopus audit 2026-05-28 — low-hanging fixes (figma typo, CSP, issue-export pagination, bcrypt note) (#13977)
PR URL: https://github.com/midnghtsapphire/revvel-standards/pull/13977

## Suggested Action
1. Preferred — nonce-based approach: Generate a cryptographically random nonce server-side on every response. Set the CSP header as script-src 'nonce-<generated_value>' <other_sources> and add the nonce="<generated_value>" attribute to every legitimate <script> tag in the template. Remove 'unsafe-inline' entirely.
2. Alternative — eliminate inline scripts: Move the Tailwind configuration object into a standalone static file (e.g. tailwind.config.js) and reference it via a src attribute. Load the Tailwind CDN script normally. With no inline scripts remaining, 'unsafe-inline' can be dropped without any other changes.
3. As a follow-up, prioritise the planned build-time Tailwind bundle, which will remove the CDN dependency altogether and allow a strict script-src policy with only explicit trusted origins. |
| Private | {IS_PRIVATE} |
