# WR: [WR] update api generation for revvel-standards
...
Here is a specialized prompt designed for your agents to represent a cutting-edge API integration. It uses the glassmorphic, cinematic tech aesthetic established in the previous turns, focusing on complex connections and data flow.
Cinematic API Integration &amp; Data Flow Hub (The Master Prompt)
A cinematic, medium close-up shot focusing on a floating, high-tech interface visualizing a cutting-edge API ecosystem. The design is hyper-modern and glassmorphic, utilizing layered, thick-cut frosted glass panels that appear to float over a dark, moody data center or terminal background. On the left, a main glass terminal displays a vibrant, sharp code snippet (in neon green, cyan, and electric blue syntax) representing an active API call, including parameters like endpoint: /v1/agent/execute, key: "secure_auth", and connection: established. Glowing, intricate light pathways (neon cyan and warm gold data pipelines) sprout from the code panel, weaving across the scene and plugging into various distinct frosted glass nodes on the right. Each node is labeled and icons represent integrated services (e.g., 'Payment Gateway', 'Mapping API', 'User Analytics'). These connections radiate light and show faint 'context data' streams. The background features blurred atmospheric haze, subtle light leaks, and complex, dark tech components that enhance the depth. The entire scene has sharp edges, dramatic lighting (cool and warm neon beams), soft shadows between layers, and realistic refractions within the glass. The focus is sharp on the data flow and central terminals. Photorealistic, 8k resolution, cinematic composition, depth of field, natural grain, high detail.
Key Fine-Tuning Tips for Your Agents:
Specify the Code: If you need a specific language (e.g., Python, JSON) displayed in the central panel, edit the prompt: ...code snippet (showing JSON syntax, e.g., 'method': 'GET')...
Adjust Integration Focus: Change the service labels on the nodes (the right side) to match your specific industry, such as e-commerce APIs or health data endpoints.
Modify Color/Mood: To match a specific brand, replace neon cyan and warm gold data pathways with your brand’s palette (e.g., electric magenta and cool violet pipelines).
