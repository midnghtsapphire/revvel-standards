# WR: [WR] Add source citations for SEO keyword volume and CPC data in WR
...
| Description | ## Summary
The work record for issue-13971 includes specific SEO keyword volume and CPC figures without citing the tools or data sources used to obtain them. This violates the WR_TEMPLATE_FULL standard, which requires this section to be verifiable, and contradicts the fleet instruction mandating cross-validation against live data.

## Details
The following figures appear in the document without attribution: keyword monthly search volumes of 12,000, 45,000, 5,000, and 8,000, and CPC estimates of $1.50 and $2.00. Because these are precise numerical claims, reviewers and stakeholders have no way to verify their accuracy, assess their freshness, or reproduce the research. The WR_TEMPLATE_FULL explicitly marks this section as REQUIRED and implies data must be researchable. Unsourced specifics undermine the defensibility of the WR and create ambiguity about whether the figures are estimates, historical snapshots, or current live data.

## Location
File: wr/issues/issue-13971-add-this-repository-as-a-high-value-seo-sem-deep-w.md, line 186
...
## Suggested Action
1. Identify the tool(s) used to obtain each figure (e.g., Google Keyword Planner, Ahrefs, Semrush).
2. Add an inline citation or a footnote next to each keyword volume and CPC value specifying the source tool, the date the data was retrieved, and any relevant regional or match-type scope.
3. If the figures were estimated rather than pulled from a live tool, label them explicitly as estimates and note the methodology used.
4. Verify all numbers against current live data before merging, per the fleet cross-validation requirement. |
| Private | {IS_PRIVATE} |
| Archived | {IS_ARCHIVED} |
