# WR: [WR] Sync template roster with live workflow after Octopus Review addition
...
| Description | ## Summary
A recent pull request updated the live workflow file to add Octopus Review to the reviewer roster and apply provenance annotations to every reviewer, but the corresponding canonical template file was left unchanged. Because operators bootstrap new repositories directly from the template, any repo created after this PR merges will inherit an outdated roster that is missing both the Octopus Review entry and the provenance annotations.

## Details
The files templates/cicd/ready-for-review.yml and .github/workflows/ready-for-review.yml are intended to stay in sync: the template is the source operators copy when standing up a new repository, and the live workflow is the deployed instance. PR #13976 applied changes only to the live workflow (roster lines ~222-243), leaving the template stale. This creates a silent drift problem: newly bootstrapped repositories will silently lack Octopus Review coverage and provenance metadata, and the discrepancy may not be caught until a review gap or compliance audit surfaces it.

## Location
File: templates/cicd/ready-for-review.yml, lines 222-243
Related live file: .github/workflows/ready-for-review.yml, same line range
Pull Request: Add Octopus Review to roster + apply provenance to every reviewer (#13976)
PR URL: https://github.com/midnghtsapphire/revvel-standards/pull/13976

## Suggested Action
1. Open templates/cicd/ready-for-review.yml and update the roster block (lines 222-243) to mirror the changes made to .github/workflows/ready-for-review.yml in PR #13976, including the Octopus Review entry and all provenance annotations.
2. Review the remainder of both files for any other sections that may have diverged in this PR or previously.
3. Consider adding a CI lint step or a diff-check script that fails the build when the roster blocks in these two files are out of sync, preventing similar drift in the future. |
| Private | {IS_PRIVATE} |
