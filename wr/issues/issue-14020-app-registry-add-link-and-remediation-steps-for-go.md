# WR: [WR] APP_REGISTRY: Add link and remediation steps for GodsofInsurance delete risk
...
| Description | ## Summary
The APP_REGISTRY.md file contains a warning referencing a `delete_repo "GodsofInsurance"` line found in docs/Walter-Evans-GitHub-Repo-Inventory.md, but the warning neither links to that file nor provides actionable remediation guidance. This reduces the effectiveness of the warning and increases the risk that an agent or developer will overlook or deprioritize the hazard.

## Details
At line 87 of docs/APP_REGISTRY.md, a risk note flags the presence of a destructive `delete_repo` directive in the inventory file. However, the note lacks two critical elements: a direct markdown hyperlink to the referenced file so readers can navigate to it immediately, and a concrete one-line remediation instruction explaining how to neutralize the risk (e.g., remove the offending line, comment it out, or wrap it in a conditional guard). Without these, an automated agent parsing the registry may treat the warning as informational noise and skip it, leaving an active deletion risk unaddressed in the codebase.

## Location
File: docs/APP_REGISTRY.md, line 87
Referenced file: docs/Walter-Evans-GitHub-Repo-Inventory.md
Pull Request: Add App & Template Registry — reuse-first, fast-path for saturated types (#13916)
PR URL: https://github.com/midnghtsapphire/revvel-standards/pull/13916

## Suggested Action
1. In docs/APP_REGISTRY.md at line 87, convert the plain-text reference to the inventory file into a markdown hyperlink pointing to docs/Walter-Evans-GitHub-Repo-Inventory.md.
2. Append a one-line remediation instruction directly beneath the warning, for example: "Remediation: Remove or comment out the `delete_repo` line in the inventory file before merging, or wrap it in an explicit environment guard to prevent accidental execution."
3. Optionally add a checklist item or action label (e.g., [ACTION REQUIRED]) to the warning so automated agents and reviewers treat it as a blocking item rather than a passive note. |
| Private | {IS_PRIVATE} |
