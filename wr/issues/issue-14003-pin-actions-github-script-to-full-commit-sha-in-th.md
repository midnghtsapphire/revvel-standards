...
## Location
File: .github/workflows/third-party-action-audit.yml, line 64
...
## Suggested Action
1. Identify the full commit SHA for the latest `actions/github-script` v9 release (e.g., by checking the official action repository's release tags).
2. Replace `actions/github-script@v7` on line 64 with the full SHA pin followed by an inline version comment, following the established pattern used elsewhere in the repo, for example: `actions/github-script@3a2844b7e9c422d3c10d287c895573f7108da1b3 # v9.0.0`.
3. Verify the updated workflow passes the existing `workflow-action-ref-audit.yml` check before merging.
4. Consider adding a note in the PR description confirming the SHA was verified against the official release to satisfy code review requirements. |
| Private | {IS_PRIVATE} |
