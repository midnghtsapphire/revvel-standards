# WR: [WR] Fix broken docs/APP_REGISTRY.md reference in Agent Monitoring Standard §2
...
| Description | ## Summary
The Agent Monitoring Standard references docs/APP_REGISTRY.md as the authoritative reuse-first registry that agents and reviewers must consult during the §2 step 3 gate check. This file does not exist in the repository, making the gate effectively unenforceable. Any agent or reviewer following the documented process will encounter a dead reference and be unable to complete the required verification step.

## Details
In section 2, step 3 of docs/AGENT_MONITORING_STANDARD.md, the 'Reuse-first' gate instructs readers to check docs/APP_REGISTRY.md before building new capabilities. Because this file is absent from the repository file tree, the gate has no backing artifact to validate against. Two files that do exist and may serve a similar purpose are docs/PROJECT_CATALOG.md and docs/PROJECTS_TO_SHIP.md. Leaving a cited but missing registry in place risks silent non-compliance: agents may skip the gate entirely or behave unpredictably when the referenced resource cannot be resolved.

## Location
File: docs/AGENT_MONITORING_STANDARD.md, line 47
...
## Suggested Action
1. Determine whether a dedicated APP_REGISTRY.md should exist as its own document or whether an existing file (docs/PROJECT_CATALOG.md or docs/PROJECTS_TO_SHIP.md) already fulfills the same purpose.
2a. If creating a new file: add docs/APP_REGISTRY.md to the repository, at minimum as a stub that describes its intended structure and current contents, so the gate is immediately actionable.
2b. If reusing an existing file: update the reference on line 47 of docs/AGENT_MONITORING_STANDARD.md to point to the correct path (e.g., docs/PROJECT_CATALOG.md).
3. Verify no other sections of AGENT_MONITORING_STANDARD.md or related standards documents contain additional references to the missing file.
4. Update or add a repository-level document inventory check to catch missing referenced files in future PRs. |
| Private | {IS_PRIVATE} |
