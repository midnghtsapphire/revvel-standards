# WR: [WR] Document license and BOM entry for referenced GitHub repository
...
| Description | ## Summary
The work request document references an external GitHub repository (https://github.com/filiksyos/mcptoskill) as the source of functionality to be implemented into revvel-standards, but no license verification, maintenance status check, or Bill of Materials (BOM) entry has been created for this dependency. This omission puts the implementation at risk of non-compliance with the revvel-standards compliance checklist, which mandates that licensing and Terms of Service notes be recorded for every external dependency before work begins.

## Details
The referenced repository's license type (MIT, Apache, GPL, or proprietary) is currently unknown within the context of this work request. Implementing functionality derived from a repository with an incompatible or restrictive license could introduce legal and compliance risks into the revvel-standards codebase. Additionally, the maintenance status and long-term viability of the upstream repository have not been assessed. The revvel-standards compliance checklist explicitly requires a BOM entry covering ToS acceptance and licensing details for all external dependencies, and this requirement has not been met in the current draft of the work request document.

## Location
File: wr/issues/issue-13969-implement-this-repository-functionality-into-revve.md, line 155
...
## Suggested Action
1. Visit https://github.com/filiksyos/mcptoskill and identify the repository's license from its LICENSE file or repository metadata.
2. Verify the license is compatible with the revvel-standards project's licensing policy.
3. Assess the repository's current maintenance status (last commit date, open issues, active maintainers).
4. Add a formal BOM entry to the work request document that includes the repository URL, identified license, compatibility assessment, maintenance status, and any relevant ToS notes.
5. If no license is found, treat the repository as proprietary and seek explicit written permission from the author before proceeding with implementation. |
| Private | {IS_PRIVATE} |
