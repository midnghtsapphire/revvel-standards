# WR: [WR] Header block in issue-14081 missing trailing spaces on lines 8-10, breaking layout
...
**Language:** JavaScript  
**Research Date:** 2026-05-31 <!-- Use YYYY-MM-DD format -->  
...
| Description | ## Summary
The header metadata block in wr/issues/issue-14081-create-a-new-asset-artifact-process-for-merchandis.md uses inconsistent Markdown line-break syntax. Lines 3-7 terminate with two trailing spaces to force a line break, while lines 8-10 do not, causing the last three header fields to collapse onto a single line when rendered.

## Details
In Markdown, a hard line break within a paragraph requires two trailing spaces (or a backslash) at the end of a line. The header block currently mixes styles: **Issue:**, **Repository:**, **Created:**, **Last Updated:**, and **Language:** all end with two trailing spaces and render on separate lines, but **Research Date:**, **Researcher:**, and **WR Status:** lack the trailing spaces. As a result, those three fields are joined into one rendered line, breaking the visual structure of the header. Other WR files in the repository, such as issue-13569 and issue-13803, consistently apply two-space line breaks to every header line, so this file deviates from established convention.

## Location
File: wr/issues/issue-14081-create-a-new-asset-artifact-process-for-merchandis.md
Lines: 8-10
PR: #14189 - docs(wr): rewrite issue-14081 to adhere to WR_TEMPLATE_FULL
PR URL: https://github.com/midnghtsapphire/revvel-standards/pull/14189

## Suggested Action
Append two trailing spaces to the end of lines 8, 9, and 10 (the **Research Date:**, **Researcher:**, and **WR Status:** entries) so that every line in the header block uses the same hard line-break syntax. Verify the rendered output displays each header field on its own line, matching the format used in issue-13569 and issue-13803. |
| Private | {IS_PRIVATE} |
