# WR: [WR] Fix broken path reference to SYSTEM_MAP.md in PROVENANCE_STANDARD.md
...
| Description | ## Summary
The provenance standards document references an incorrect file path for SYSTEM_MAP.md in its surface table. The path listed is docs/SYSTEM_MAP.md, but the file actually resides at docs/process/SYSTEM_MAP.md. Because the linked path does not exist, contributors working in the correct file have no way to discover that this standard applies to it, undermining the document's purpose as an authoritative guide.

## Details
Line 84 of docs/PROVENANCE_STANDARD.md includes a surface table entry that points to docs/SYSTEM_MAP.md. A review of the repository file tree confirms this path is invalid; the closest and likely intended match is docs/process/SYSTEM_MAP.md. In a standards document, stale or incorrect cross-references erode trust and discoverability. Contributors maintaining docs/process/SYSTEM_MAP.md will not encounter this standard during normal file navigation, making silent non-compliance likely.

## Location
File: docs/PROVENANCE_STANDARD.md, line 84 (reported at line 82)
...
## Suggested Action
1. Open docs/PROVENANCE_STANDARD.md and navigate to line 84.
2. Replace the path docs/SYSTEM_MAP.md with docs/process/SYSTEM_MAP.md in the surface table entry.
3. Verify no other entries in the surface table contain similarly stale paths by cross-checking each referenced path against the current file tree.
4. Consider adding a CI lint step or link-checker script to catch broken internal references in standards documents automatically. |
| Private | {IS_PRIVATE} |
