# WR: [WR] Fix silent pagination abort causing incomplete exports to report success
...
| Description | ## Summary
In the issue export script, a `|| break` guard on the `gh api` pagination call silently terminates the loop whenever an API error occurs (e.g. rate limiting, 5xx responses, network timeouts). After the loop exits, the script unconditionally prints a success message and exits zero, giving the caller no indication that the exported file is incomplete. A user or downstream process relying on this output will silently operate on partial data.

## Details
The `|| break` idiom causes the pagination loop to exit cleanly on any non-zero return from `gh api`, treating an error condition the same as a legitimate end-of-pages condition. Because no error state is recorded before breaking, the success banner printed afterward is factually incorrect whenever fewer pages than expected were retrieved. In a scenario where 30 pages exist but only 3 are written before a transient failure, the resulting file appears valid and complete. This is a correctness and reliability issue: silent data loss with a misleading success signal makes debugging and auditing significantly harder.

## Location
File: scripts/issues/quick-actions.sh, line 172
...
## Suggested Action
1. Replace the bare `|| break` with an error handler that sets a flag or immediately emits a diagnostic message to stderr and exits with a non-zero status code.
2. Ensure the success message is only printed when the loop completes without error, for example by checking the error flag before printing.
3. Consider adding a retry with exponential back-off for transient errors (rate limits, 5xx) before giving up, to improve resilience without masking genuine failures.
4. Add a test or CI check that simulates a mid-pagination API failure and asserts the script exits non-zero with no success output. |
| Private | {IS_PRIVATE} |
