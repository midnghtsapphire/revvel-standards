# WR: [WR] Fix unverifiable package ref  in provenance standard table
...
| Description | ## Summary
The quick-reference table in PROVENANCE_STANDARD.md cites `BeksOmega/jules-comms@v1.0.0` as the canonical package for the Jules feedback lane, but no evidence of this package exists in the codebase. The actual workflow file `jules-pr-comment.yml` references `BeksOmega/jules-publish@v1.0.0` instead, suggesting `jules-comms` may be a fabricated or mistaken identifier. This directly undermines the standard's goal of providing unambiguous, verifiable package references.

## Details
The provenance standard is intended to eliminate ambiguity by requiring an exact, named publisher and package version for every tool. Listing a package reference that cannot be verified against the live workflow files introduces the very ambiguity the standard was designed to prevent. Two workflow files are present — `jules-feedback.yml` and `jules-pr-comment.yml` — and neither appears to use `jules-comms`. The discrepancy between the documented reference (`jules-comms`) and the observed reference (`jules-publish`) means consumers of this standard cannot reliably reproduce or audit the Jules feedback lane provenance.

## Location
File: docs/PROVENANCE_STANDARD.md, line 109
...
## Suggested Action
1. Audit `jules-feedback.yml` and `jules-pr-comment.yml` to confirm which GitHub Action package is actually used for the Jules feedback lane.
2. Replace the `BeksOmega/jules-comms@v1.0.0` entry in the quick-reference table with the verified package name and version (likely `BeksOmega/jules-publish@v1.0.0` based on current workflow content).
3. If `jules-comms` is a planned or future package, add a clear note marking it as not yet available and do not include it in the normative reference table until it exists.
4. Add a CI or linting check that cross-validates package references in the provenance standard against the actual workflow files to prevent future drift. |
| Private | {IS_PRIVATE} |
