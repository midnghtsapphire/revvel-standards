# WR: [WR] Add no-destroy-guard.js cross-reference to standard header block
...
| Description | ## Summary
The Agent Monitoring Standard references the No-Destroy Guard in mid-document sections (§2 step 3 and §4) but omits the backing script from the header cross-references block. A reader following the document linearly will encounter the guard concept and its associated review step before learning where the implementation lives, creating an inconsistency with how other guards are referenced throughout the standard.

## Details
The script scripts/no-destroy-guard.js exists in the repository file tree, confirming the guard is implemented. However, the standard's header block — which lists cross-references for other guards — does not include an entry for this script. This breaks the established pattern of citing guard scripts upfront and forces readers to scan into §4 before discovering the file path. Consistency in cross-referencing is important for maintainability and for reviewers who rely on the header block as a quick-reference index when applying the standard.

## Location
File: docs/AGENT_MONITORING_STANDARD.md, line 42
...
## Suggested Action
1. Locate the cross-references section in the header block of docs/AGENT_MONITORING_STANDARD.md (around line 42).
2. Add an entry for scripts/no-destroy-guard.js alongside the existing guard script cross-references, following the same format used for the other entries.
3. Verify that the label used in the header matches the terminology in §2 step 3 and §4 so all three locations are consistent.
4. Review the remainder of the document to confirm no other guard scripts are similarly missing from the header block. |
| Private | {IS_PRIVATE} |
