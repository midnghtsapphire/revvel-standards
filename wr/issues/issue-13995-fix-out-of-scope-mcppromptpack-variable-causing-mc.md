...
| Description | ## Summary
In `generateUIRecommendations`, the code attempts to read `mcpPromptPack` to set the `hasMcpPromptPack` flag and determine the token budget for prompt truncation. However, `mcpPromptPack` is a local variable defined inside `buildUIRecommendationsUserPrompt` and is never returned or otherwise exposed to the outer function. As a result, `mcpPromptPack` is always `undefined` at the point of access, `hasMcpPromptPack` is always `false`, and the fallback 24,000-character budget is always applied. This completely negates the purpose of PR #13854, which was specifically intended to inject the MCP master prompt pack into UI recommendation generation.

## Details
The variable `mcpPromptPack` is computed as a local binding inside `buildUIRecommendationsUserPrompt`. Because JavaScript closures do not grant the caller access to a callee's local variables, any reference to `mcpPromptPack` in `generateUIRecommendations` resolves to `undefined` at runtime rather than the computed value. The conditional branch that enables MCP-aware behavior is therefore unreachable under all circumstances, making the feature silently broken with no error thrown. This is a logic defect with critical impact: the new functionality ships as dead code, and the larger 24,000-character budget is applied unconditionally instead of only as a fallback.

## Location
File: scripts/ui-creation-engine.js, line 521
Pull Request: fix: inject MCP master prompt pack in UI recommendation generation (#13854)
URL: https://github.com/midnghtsapphire/revvel-standards/pull/13854

## Suggested Action
1. Refactor `buildUIRecommendationsUserPrompt` to return the computed `mcpPromptPack` value alongside its existing return value, for example by returning an object such as { prompt, mcpPromptPack } instead of a plain string.
2. Update all call sites of `buildUIRecommendationsUserPrompt` to destructure the new return shape.
3. In `generateUIRecommendations`, derive `hasMcpPromptPack` and the token budget from the value returned in step 1 rather than referencing the out-of-scope variable.
4. Add a unit test that asserts `hasMcpPromptPack` is `true` and the correct budget is selected when a valid MCP prompt pack is present, to prevent regression. |
| Private | {IS_PRIVATE} |
