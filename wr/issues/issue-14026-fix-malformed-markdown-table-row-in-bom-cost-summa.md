# WR: [WR] Fix malformed Markdown table row in BOM cost summary section
...
| Description | ## Summary
The BOM cost summary section in the WR document contains a standalone pipe-delimited row that is not part of a valid Markdown table. Because it lacks a preceding header row and delimiter line, most Markdown renderers — including GitHub — will display it as a raw pipe-separated string rather than a formatted table cell, breaking the visual presentation of cost data.

## Details
The offending line `| **Total Infrastructure** | | **$0/mo** |` appears at line 222 as an isolated row with no table header or separator above it. Valid GitHub-Flavored Markdown tables require at minimum a header row followed by a delimiter row (e.g., `| --- | --- | --- |`) before any data rows. Without this structure, the renderer treats the content as plain text. The original WR_TEMPLATE_FULL contained a properly structured 3-column cost summary table, which was lost during the restructuring performed in PR #13991.

## Location
File: wr/issues/issue-13971-add-this-repository-as-a-high-value-seo-sem-deep-w.md, line 222
...
## Suggested Action
1. Locate line 222 in the affected file.
2. Replace the standalone row with a complete, valid Markdown table that includes a header row and a delimiter row above the data row.
3. Use the WR_TEMPLATE_FULL cost summary table as the reference structure, ensuring all three columns (item, description/notes, cost) are properly defined.
4. Verify the rendered output on GitHub after the fix to confirm correct table formatting. |
| Private | {IS_PRIVATE} |
