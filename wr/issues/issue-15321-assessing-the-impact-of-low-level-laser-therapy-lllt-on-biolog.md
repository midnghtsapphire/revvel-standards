# WR: Assessing the impact of low level laser therapy (LLLT) on biological systems — Sellable PDF + Tools/Apps
...
Research and productize the biological mechanisms and clinical applications of Low Level Laser Therapy (LLLT) / Photobiomodulation (PBM). Source: [University of Plymouth research paper](https://pearl.plymouth.ac.uk/cgi/viewcontent.cgi?article=1203&context=secam-research). Deliverables: a sellable PDF guide + companion tools/apps for practitioners and consumers.

### Objective

Leverage academic LLLT/PBM research to create a high-value sellable PDF and a suite of supporting tools/apps targeting the growing wellness, sports medicine, and clinical photobiomodulation market.
...
### 1. What is LLLT / Photobiomodulation
...
- Neuroprotection and cognitive performance modulation
- Hair follicle stimulation (FDA-cleared for androgenetic alopecia)
- Pain reduction via opioid and serotonin pathways

**Source PDF (Plymouth University):** [pearl.plymouth.ac.uk/cgi/viewcontent.cgi?article=1203&context=secam-research](https://pearl.plymouth.ac.uk/cgi/viewcontent.cgi?article=1203&context=secam-research) — academic review of LLLT biological impact, part of the SECAM research series.
...
- `red light therapy benefits` — 10K–100K monthly searches (consumer-friendly synonym)
- `cold laser therapy for pain` — 1K–10K monthly searches
...
- `photobiomodulation.guide` — niche TLD, memorable
- `llltcalculator.com` — tool-specific, high intent traffic

**TLD recommendation:** `.com` for primary commercial; `.guide` or `.app` for companion tool.
...
4. **MCP skill** — `lllt-protocol` skill exposing protocol lookup and dosage calculation via AI agents
...
- PDF batch of 20: 20 condition-specific variants (pain, wound healing, hair growth, cognition, sports recovery, inflammation, neuroprotection, skin rejuvenation, joint health, TMJ, peripheral neuropathy, tendinitis, plantar fasciitis, carpal tunnel, fibromyalgia, arthritis, post-surgical, dental, veterinary, and a practitioner master guide)
...
- **Mito Red MitoPRO 300:** $349 ([mitored.com](https://www.mitored.com))
- **iGrow Hair Growth Helmet:** $695 ([igrowlaser.com](https://www.igrowlaser.com))
...
- `lllt dosage` — 1K–10K monthly searches
- Long-tail: `lllt protocol for wound healing`, `red light therapy wavelength guide`, `photobiomodulation dosage calculator` — combined ~5K–20K monthly searches (internal estimate)
...
- [ ] Master PDF (≥40 pages) with dosage tables, condition protocols, contraindications, and ≥20 cited sources
- [ ] 20 condition-specific PDF variants generated and stored under `assets/pdf/wr-15321/`
- [ ] Dosage calculator app deployed to Vercel (or staging) and accepting wavelength/power/time inputs
- [ ] CLI tool `lllt-calc` passes `npm test` and produces correct J/cm² output for known test cases
- [ ] Landing page live with email capture and PDF purchase link
- [ ] All artifacts committed under `products/lllt-protocol/` or `docs/pdf/wr-15321/`
- [ ] wr-lint passes on this WR document

### Do Not Under-Scope

- Do NOT ship just a stub PDF — the master guide must be ≥40 pages with real dosage tables
- Do NOT omit the 20-variant batch — each variant targets a specific condition audience (SEO + sales)
- Do NOT ship the calculator without the condition-specific protocol presets
- Do NOT skip the landing page — it is the revenue-generating surface

### Explicit Exclusions

- Mobile native app (iOS/Android) — deferred to Phase 2 after web app validation
- LLLT device hardware — out of scope (information/software product only)
- Clinical trials or FDA submission — not in scope
...
4. PR 4: CLI tool `lllt-calc`
5. PR 5: Landing page + Polar.sh integration
...
1. **Scaffold the research brief** — commit a Markdown file under `docs/pdf/wr-15321/brief.md` with all dosage tables, biological mechanisms, and condition protocols sourced from the Plymouth University paper and WALT guidelines
2. **Run Gumloop PDF pipeline** — configure `scripts/gumloop_pdf_pipeline.py` for 20 LLLT condition variants using the master content
3. **Register domain** — `llltguide.com` or `redlightprotocol.com` (check availability)

### P1 — Short-Term

1. **Scaffold `products/lllt-protocol/`** — Next.js app at port 3010 with dosage calculator UI
2. **Build `lllt-calc` CLI** — Node.js script: inputs (wavelength nm, power mW/cm², time s) → outputs dose (J/cm²), compares to WALT recommended range for selected condition
3. **SEO landing page** — target `red light therapy dosage guide` and `lllt protocol pdf`

### P2 — Medium-Term

1. **MCP skill** — `lllt-protocol` skill: `calculate_dose(wavelength, power, time, condition)` → returns protocol recommendation + contraindication warnings
2. **White-label API** — REST endpoint for device manufacturers to integrate protocol lookup
3. **Affiliate partnerships** — Joovv, Mito Red, Kaiyan affiliate programs for device recommendations in PDFs
