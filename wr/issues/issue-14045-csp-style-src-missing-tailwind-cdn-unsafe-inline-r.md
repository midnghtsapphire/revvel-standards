# WR: [WR] CSP style-src missing Tailwind CDN; unsafe-inline removal plan needed
...
| Description | ## Summary
The Content Security Policy in osint-hub/index.html does not explicitly list the Tailwind CDN origin (https://cdn.tailwindcss.com) in the style-src directive. While the current presence of 'unsafe-inline' in style-src technically permits Tailwind's runtime-injected styles to function, this arrangement creates a future trap: any effort to harden the CSP by removing 'unsafe-inline' will silently break Tailwind styling unless the CDN origin and a nonce or hash strategy are added at the same time.

## Details
The Tailwind CDN script injects <style> elements into the document at runtime. The style-src directive currently relies on 'unsafe-inline' to allow these injections rather than explicitly allowlisting the CDN endpoint. 'unsafe-inline' is a broad permission that weakens the protective value of CSP against cross-site scripting attacks involving style injection. The correct long-term posture is to remove 'unsafe-inline' from style-src, add https://cdn.tailwindcss.com as an allowed source, and apply nonces or hashes to all inline styles Tailwind injects. Without documenting this dependency now, a future engineer removing 'unsafe-inline' may introduce a regression without understanding why.

## Location
File: osint-hub/index.html, line 6
Pull Request: Octopus audit 2026-05-28 — low-hanging fixes (figma typo, CSP, issue-export pagination, bcrypt note) (#13977)
URL: https://github.com/midnghtsapphire/revvel-standards/pull/13977

## Suggested Action
1. Add https://cdn.tailwindcss.com to the style-src directive so the CDN origin is explicitly permitted, independent of 'unsafe-inline'.
2. Add a code comment near the CSP meta tag documenting that 'unsafe-inline' is currently required by Tailwind CDN's runtime style injection and must not be removed without simultaneously implementing a nonce or hash strategy for those injected styles.
3. Track removal of 'unsafe-inline' from style-src as a follow-up hardening task, ideally linked to a migration away from the Tailwind runtime CDN toward a build-time compiled stylesheet, which would eliminate the inline style dependency entirely. |
| Private | {IS_PRIVATE} |
