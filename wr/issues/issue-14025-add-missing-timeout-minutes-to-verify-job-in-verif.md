# WR: [WR] Add missing timeout-minutes to verify job in verify-security-fix.yml
...
| Description | ## Summary
The `verify` job in the `verify-security-fix.yml` GitHub Actions workflow is missing a required `timeout-minutes` directive. This gap was identified by the Automation Doctor report and documented in PR #13991, but no remediation was applied. Without a timeout, a hung runner could consume GitHub Actions minutes indefinitely, violating the established codebase standard.

## Details
The codebase enforces a standard requiring all GitHub Actions jobs to declare an explicit `timeout-minutes` value. This policy exists to prevent runaway or hung jobs from blocking runner capacity and incurring unnecessary CI cost. The Automation Doctor report flagged the `verify` job in `verify-security-fix.yml` as non-compliant. PR #13991 acknowledges the finding in documentation but does not apply the corrective change to the workflow file itself, leaving the risk active in the repository.

## Location
File: AUTOMATION-DOCTOR-REPORT.md, line 13
Workflow file requiring fix: .github/workflows/verify-security-fix.yml, `verify` job definition
PR reference: docs(wr): restructure issue-13971 WR to match WR_TEMPLATE_FULL (#13991)
PR URL: https://github.com/midnghtsapphire/revvel-standards/pull/13991

## Suggested Action
1. Open .github/workflows/verify-security-fix.yml and locate the `verify` job definition.
2. Add a `timeout-minutes` directive to the job, setting an appropriate value based on the expected maximum runtime of the job (e.g., `timeout-minutes: 15`).
3. Commit the change as part of PR #13991 or raise an immediate follow-up PR targeting the same branch.
4. Confirm the Automation Doctor report no longer flags this job after the fix is merged. |
| Private | {IS_PRIVATE} |
