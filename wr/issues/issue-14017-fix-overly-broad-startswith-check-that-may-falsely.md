# WR: [WR] Fix overly broad startsWith check that may falsely match unrelated GitHub Apps
...
| Description | ## Summary
The check-run detection logic in the Augment auto-detection workflow was recently changed from a strict exact-match approach to a prefix-based approach using startsWith('augment'). This broadening introduces a risk of false positives where any GitHub App whose slug or name begins with the string 'augment' could incorrectly trigger the workflow, regardless of whether it is actually the Augment Code application.

## Details
The previous implementation used explicit equality checks against a known set of identifiers: === 'augment', === 'augment-code', and === 'augment code'. These were intentionally conservative to avoid matching unrelated applications. The new implementation replaces these with appSlug.startsWith('augment') and appName.startsWith('augment'), which is significantly more permissive. Any third-party GitHub App with a slug or display name beginning with 'augment' — for example 'augment-labs', 'augmentor', or 'augmented-ci' — would now satisfy the condition and trigger a false positive. This could cause the workflow to run unexpectedly, produce incorrect status checks, or interfere with CI gating on unrelated pull requests.

## Location
File: .github/workflows/augment-check.yml, line 152
...
## Suggested Action
1. Replace the bare startsWith calls with a stricter regular expression that covers only known legitimate Augment App identifiers.
2. Use a pattern such as /^augment(?:-code)?$/i tested against both appSlug and appName, which anchors the match to prevent partial-prefix collisions.
3. If future slug variants are anticipated, maintain an explicit allowlist of known identifiers and update it deliberately rather than relying on open-ended prefix matching.
4. Add a comment in the workflow file explaining the rationale for the chosen pattern to prevent well-intentioned broadening in future changes. |
| Private | {IS_PRIVATE} |
