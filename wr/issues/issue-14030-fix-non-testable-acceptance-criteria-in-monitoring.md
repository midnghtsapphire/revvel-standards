# WR: [WR] Fix non-testable acceptance criteria in monitoring queue example row
...
| Description | ## Summary
The first entry in the rolling monitoring queue in docs/AGENT_MONITORING_STANDARD.md uses a location description as its acceptance criterion rather than a genuinely testable statement. Because this row serves as the canonical example for all future contributors appending queue entries, the flawed pattern will propagate unless corrected.

## Details
The 'Acceptance' column for the first queue row currently reads 'banner above the example block in docs/AGENT_AUTONOMY_PROTOCOLS.md'. This is a description of a file location, not a verifiable pass/fail criterion. The project's Definition of Done (docs/DEFINITION_OF_DONE.md) and the document's own §1 step 2 explicitly require acceptance criteria to be testable. A testable criterion specifies an observable, binary outcome such as a CI check result, a grep-verifiable string presence, or a constraint on which files were modified. Without this quality in the example row, future contributors have no clear model to follow and are likely to continue writing untestable acceptance entries.

## Location
File: docs/AGENT_MONITORING_STANDARD.md, line 100
...
## Suggested Action
1. Open docs/AGENT_MONITORING_STANDARD.md at line 100.
2. Replace the current acceptance value with a fully testable criterion, for example: 'CI passes; docs/AGENT_AUTONOMY_PROTOCOLS.md contains a caveat banner immediately above the OpenRouter example block; no other files modified.'
3. Review any additional queue rows already present and apply the same standard if they also contain descriptive rather than testable acceptance values.
4. Optionally add a short inline comment or column note clarifying that acceptance entries must be binary pass/fail statements aligned with docs/DEFINITION_OF_DONE.md. |
| Private | {IS_PRIVATE} |
