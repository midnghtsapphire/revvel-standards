# WR: [WR] Fix broken references to non-existent docs in PROVENANCE_STANDARD.md
...
| Description | ## Summary
The Provenance Standard document cites two files — docs/TESTING_STACK.md and docs/TOOL_COST_INDEX.md — as authoritative references and sources of truth, but neither file exists in the repository. Any contributor or automated agent following these references will find nothing, undermining the credibility of the standard and potentially causing confusion or incorrect assumptions about tooling and testing conventions.

## Details
At lines 86 and 122–124 of docs/PROVENANCE_STANDARD.md, the document declares docs/TESTING_STACK.md and docs/TOOL_COST_INDEX.md as canonical reference patterns. A review of the repository file tree confirms both files are absent. Referencing non-existent files as sources of truth is particularly problematic in a standards document, as it is likely to be treated as reliable by contributors and automated agents alike. This creates a gap between documented intent and actual repository state, and may cause downstream decisions to be based on guidance that cannot be verified or consulted.

## Location
File: docs/PROVENANCE_STANDARD.md, lines 86 and 122–124
...
## Suggested Action
1. Create docs/TESTING_STACK.md and docs/TOOL_COST_INDEX.md with appropriate content before merging this PR, so that references in the standard resolve to real documents.
2. Alternatively, if these files are not yet ready, remove or replace the references in PROVENANCE_STANDARD.md with placeholders or TODO comments that clearly indicate the documents are forthcoming.
3. After the missing files are created, verify all links within PROVENANCE_STANDARD.md resolve correctly before marking the PR ready for review.
4. Consider adding a CI check or linting rule to detect broken internal documentation references to prevent similar issues in the future. |
| Private | {IS_PRIVATE} |
