# WR: [WR] PROVENANCE_STANDARD.md lacks enforcement mechanism for compliance
...
| Description | ## Summary
The newly introduced Provenance Standard document defines expectations for naming tool publishers and exact package references, but provides no described mechanism for enforcing those expectations. Without an enforcement path, the standard risks gradual erosion as contributors unfamiliar with it introduce non-compliant tool references over time.

## Details
The document is currently aspirational only — it states what should be done but not how compliance will be verified or maintained. The repository already demonstrates precedent for automated enforcement through existing files such as anti-scaffolding-enforcer.yml and .pre-commit-config.yaml, which shows the project is capable of and accustomed to tooling-based guardrails. The absence of a comparable mechanism for the Provenance Standard creates an inconsistency in the enforcement model. At minimum, the standard should document how violations are caught, whether that is a grep or semgrep rule, a pre-commit hook, a PR checklist item, or explicit manual PR review. Without this, new contributors have no signal that compliance is checked and no tooling to help them self-correct before review.

## Location
File: docs/PROVENANCE_STANDARD.md, line 1 (document-wide concern)
Pull Request: Add Provenance Standard — name publisher + exact package for every tool (#13975)
PR URL: https://github.com/midnghtsapphire/revvel-standards/pull/13975

## Suggested Action
1. Add a dedicated 'How this is enforced' section to docs/PROVENANCE_STANDARD.md that clearly states the current enforcement strategy.
2. If enforcement is manual for now, state that explicitly so contributors understand review expectations.
3. As a follow-up, evaluate adding a grep or semgrep rule targeting tool references that lack a publisher or pinned version, and wire it into .pre-commit-config.yaml or an existing CI workflow consistent with anti-scaffolding-enforcer.yml.
4. Optionally add a PR checklist item in the pull request template reminding authors to verify provenance entries before requesting review. |
| Private | {IS_PRIVATE} |
