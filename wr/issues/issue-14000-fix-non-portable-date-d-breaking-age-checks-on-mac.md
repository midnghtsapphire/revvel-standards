...
| Description | ## Summary
The audit script uses `date -d`, a GNU coreutils-only flag that is not available on macOS or other BSD-based systems. Because the failure is silently swallowed by a `|| echo ""` fallback, every age calculation returns an empty string on non-GNU runners. This causes all single-author action age checks to report as unknown and be downgraded to warn severity instead of flag, making the tool completely ineffective when run locally on macOS.

## Details
The `date -d "$iso"` invocation on line 102 is a GNU extension. The BSD `date` command available on macOS requires a different syntax: `date -j -f`. Because the current code uses `|| echo ""` as a fallback, the failure is invisible — no error is surfaced, the age variable is set to an empty string, and the downstream condition silently demotes every result. This creates a misleading audit experience: a developer running the script locally on macOS will never see any flagged actions regardless of how stale they are, defeating the purpose of the audit tool entirely. The script already applies a similar platform guard around the `gh` CLI invocation, confirming the pattern is known but was not applied consistently here.

## Location
File: scripts/audit-third-party-actions.sh, line 102
...
## Suggested Action
1. Replace the bare `date -d "$iso"` call with a cross-platform wrapper that detects the available date implementation at runtime, for example by checking whether `date --version` succeeds (GNU) or falls back to the BSD `date -j -f` form.
2. If the platform cannot be detected, emit an explicit error or warning rather than silently returning an empty string, so the caller is aware the check could not be performed.
3. Add a CI job or test that runs the script on a macOS runner to catch regressions of this kind automatically.
4. Review other date-related calls in the script for the same portability issue. |
| Private | {IS_PRIVATE} |
