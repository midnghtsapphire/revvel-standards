# WR: [WR] Fix non-canonical model ID in PROVENANCE_STANDARD.md 'Good' example
...
| Description | ## Summary
The 'Good' example in docs/PROVENANCE_STANDARD.md references `anthropic/claude-opus-4-7` as an illustrative model ID, which does not match the canonical model identifiers standardised across the repository. This creates a risk that contributors and automated agents will copy this unverified identifier into production configurations, undermining consistency and potentially causing runtime failures.

## Details
Per WR_PROMPTFOO_HARDENING_COMPLETE.md, the repository completed a standardisation pass across 16 files, establishing `anthropic/claude-sonnet-4` as the primary model ID and `anthropic/claude-sonnet-4.5` as the fallback. The model ID `anthropic/claude-opus-4-7` used in the example does not appear in either of those approved identifiers and has not been verified as a valid Anthropic model string. Because PROVENANCE_STANDARD.md is a normative standards document intended to guide contributors, any model ID appearing in its examples carries implicit authority. Using a non-canonical identifier here directly contradicts the repo-wide standardisation effort and introduces a silent divergence point.

## Location
File: docs/PROVENANCE_STANDARD.md, line 57
...
## Suggested Action
1. Replace `anthropic/claude-opus-4-7` on line 57 with the canonical primary model ID `anthropic/claude-sonnet-4`, or use the fallback `anthropic/claude-sonnet-4.5` if the example specifically intends to illustrate a fallback scenario.
2. Optionally add a brief inline comment in the document noting that example model IDs must match the values defined in WR_PROMPTFOO_HARDENING_COMPLETE.md to prevent future drift.
3. Search the rest of PROVENANCE_STANDARD.md for any other occurrences of non-canonical model identifiers and apply the same correction. |
| Private | {IS_PRIVATE} |
