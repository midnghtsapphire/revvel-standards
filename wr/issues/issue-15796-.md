...
steps: - name: Checkout
uses: actions/checkout@v6.0.3 - name: Self-hosted Renovate
...
name: Renovate
on:
schedule: # The "_" (#42, asterisk) character has special semantics in YAML, so this # string has to be quoted. - cron: '0/15_ ** *'
jobs:
renovate:
runs-on: ubuntu-latest
steps: - name: Checkout
...
schedule: # Run every 30 minutes: - cron: '0,30 ****'
