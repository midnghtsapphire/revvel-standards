# WR: [WR] Normalize string quoting in jules-pr-comment.yml expression to use single quotes
...
| Description | ## Summary
The jules-pr-comment.yml workflow uses a double-quoted empty string in a GitHub Actions expression comparison (`secrets.ADMIN_GITHUB_TOKEN != ""`), diverging from the single-quoted convention (`secrets.ADMIN_GITHUB_TOKEN != ''`) used consistently across every other workflow in this repository. Although both forms are technically valid at runtime, the inconsistency introduces a maintenance hazard and risks false positives from YAML linters or expression parsers that enforce a single quoting style.

## Details
GitHub Actions expression syntax accepts both single and double quotes for string literals inside `${{ }}` blocks. However, the established convention throughout this repository — visible in pr-lifecycle.yml, pr-state-orchestrator.yml, and priority-router.yml — is to use single-quoted strings for ternary-style comparisons. Introducing double quotes in the new env block of jules-pr-comment.yml breaks that consistency. Over time, mixed quoting styles complicate automated refactoring, template generation, and static analysis. Enforcing a uniform style now prevents the pattern from propagating to future workflow files.

## Location
File: .github/workflows/jules-pr-comment.yml, line 38
Pull Request: #13951 — test: add detailed tests for red ocean scoring (https://github.com/midnghtsapphire/revvel-standards/pull/13951)

## Suggested Action
1. Open .github/workflows/jules-pr-comment.yml and navigate to line 38.
2. Replace the double-quoted empty string in the expression, changing `secrets.ADMIN_GITHUB_TOKEN != ""` to `secrets.ADMIN_GITHUB_TOKEN != ''`.
3. Review the rest of the env block introduced in this PR for any other double-quoted string literals and apply the same correction.
4. Consider adding a linting rule (e.g., via actionlint or a custom pre-commit hook) to enforce single-quoted strings in GitHub Actions expressions repository-wide, preventing future drift. |
| Private | {IS_PRIVATE} |
