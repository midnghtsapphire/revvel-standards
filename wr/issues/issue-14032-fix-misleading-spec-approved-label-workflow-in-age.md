# WR: [WR] Fix misleading spec-approved label workflow in Agent Monitoring Standard
...
| Description | ## Summary
The AGENT_MONITORING_STANDARD.md documents a `spec-approved` label-triggered workflow transition that does not currently function in the pipeline. Section 1, step 1 describes research-first Work Requests advancing to the coder stage when an owner applies the `spec-approved` label, implying automated promotion to `openrouter-coder`. In practice, this path is broken due to known trigger mismatches, and no such label exists in the repository's label sync configuration. Documenting a non-functional workflow as operational will mislead both automated agents and human contributors.

## Details
The REVVEL_STANDARDS_AUDIT.md documents a confirmed trigger mismatch: `research-engine.js` emits the label `research:complete` rather than `wr:research-complete`, and no automated mechanism exists to promote a researched issue to `wr:code` status. The only current promotion path is manual via `workflow_dispatch`. Additionally, the `spec-approved` label referenced in the standard is absent from all visible label sync definition files in the codebase, meaning even if a workflow listened for it, the label cannot be applied through the standard syncing process. Publishing this section in its current form creates a false expectation of automation, risks causing agents to stall waiting for a transition that will never fire, and undermines the integrity of the standards documentation.

## Location
File: docs/AGENT_MONITORING_STANDARD.md, line 22
...
## Suggested Action
1. Do not merge this section until the underlying pipeline trigger mismatch is resolved, OR explicitly annotate the `spec-approved` promotion step as a manual process pending automation.
2. Add an inline note in the standard clarifying that auto-promotion from the research stage to `openrouter-coder` is not yet implemented and currently requires a manual `workflow_dispatch` trigger.
3. Add `spec-approved` to the repository's label sync definition files if it is intended for use, so it can be reliably applied by contributors and agents.
4. Once the pipeline fix that aligns `research-engine.js` to emit `wr:research-complete` is merged, revisit and update this section to accurately reflect the automated path. |
| Private | {IS_PRIVATE} |
