# WR: [WR]  revamp website and mobile app ui glassmorphic look and feels
...
Here arecl the prompts to use for designing the websites please add them to revvel-standards:Here are three master image generation prompts tailored specifically for glassmorphic, hyper-realistic website designs. They are structured with the same cinematic depth, atmospheric lighting, and high-fidelity detail as your reference prompt, but adapted for a premium, 3D glossy tech aesthetic.
...
​For Shifting the Color Palette: Replace terms like cool blue and warm amber or neon blue and vibrant magenta with your specific brand colors (e.g., emerald green and deep gold).
​To Control the Transparency: If the background is bleeding through too much, change semi-transparent to heavy frosted glass or thick, low-transparency translucent panels.
​To Enhance the Realism: Words like refracting light edges, sharp caustics, and micro-textures are key to forcing the AI generator to treat the glass like a physical, 3D object rather than a flat vector shape.

There is now templates available too so include that folder-loveable look and feel dedigned front to back there should be logo, every logo size, domain check, buy domain, that vibes with the website.

Please provide me the system here in an artifact in the readme file for website and mobile app ui. The fonts cannot be overlapping or running together. Everyone i have got straight off the presses is bad. Except when copilot makes it out of the standards I need Leonardo, Picasso, 11 lab, Nana banana cutting edge graphics out the gate, including a product or app video describing it for every project we need deep research -competitors top trending words in that category, competitors that trend and what they do so copy the structure the hot trends top product searches and sub searches for that product, etc we need an engine that generates the whole product. I need an imementaion agent for the website, mobile apps apple ios and android using capacitor so every product will get the website when approved then generates the mobile equivalent that gets auto posted to the stores. I need an agent that can make sure no overlapping, the images are not dead, the shopping cart functions-testing the look and feel. I want this in revvel-standards for all products. If I ask to create an a p I or m c p or skill, then I need a website where I can see how they work, or if they're working. Please add this to revvel-standards. And from now on, every product will need a website. Mobile apps at CLI and M CP and API and a skill or skills and the skill should be very, very layered and intricate and there needs to be a test harness just for skills. Maybe the agent factory I don't know.
