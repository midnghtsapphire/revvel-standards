# WR: [WR] Fix unfilled template tokens in Repository Metadata table
...
| Description | ## Summary
Three placeholder tokens in the Repository Metadata table were never replaced with real values before the document was submitted. The fields Stars, Open Issues, and Archived still contain raw template strings ({STARS}, {OPEN_ISSUES}, {IS_ARCHIVED}), which will render literally in GitHub and create an inconsistent, unprofessional document appearance alongside the other correctly populated metadata fields.

## Details
The Repository Metadata table appears to have been generated from a template where most fields (e.g., Description, Private) were populated correctly, but Stars, Open Issues, and Archived were overlooked. Rendering these literal token strings in a published markdown document signals an incomplete authoring step, reduces document credibility, and may confuse readers who expect actual repository data. The inconsistency also suggests a gap in the template-filling process or review checklist.

## Location
File: wr/issues/issue-13755-evaluate-and-research-and-implement-boberdoo-for-l.md, line 85
...
## Suggested Action
1. Locate the Repository Metadata table at line 85 of the referenced file.
2. Replace {STARS}, {OPEN_ISSUES}, and {IS_ARCHIVED} with the actual values retrieved from the repository (e.g., via the GitHub API or manual lookup).
3. If the actual values are not available or not applicable, replace each token with the string N/A to maintain table consistency.
4. Consider adding a template validation step or pre-commit check to catch unfilled placeholders before future PRs are opened. |
| Private | {IS_PRIVATE} |
