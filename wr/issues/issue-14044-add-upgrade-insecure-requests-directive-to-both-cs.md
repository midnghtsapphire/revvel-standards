# WR: [WR] Add upgrade-insecure-requests directive to both CSP headers
...
| Description | ## Summary
Neither of the two newly introduced Content Security Policies in fieldwork/index.html includes the upgrade-insecure-requests directive. While both policies address XSS attack surface adequately, the omission leaves open the possibility that accidental http:// subresource references could be loaded over an unencrypted connection rather than being silently promoted to https://.

## Details
The upgrade-insecure-requests CSP directive instructs the browser to automatically rewrite any http:// URLs in subresource requests (scripts, stylesheets, images, fonts, etc.) to https:// before the request is made. This is a low-cost, defensive measure that prevents mixed-content issues and transport-layer downgrade scenarios on static pages. Without it, a mistyped or legacy http:// reference in markup or injected content could bypass HTTPS enforcement entirely, even when the page itself is served over TLS.
...
## Suggested Action
1. Locate both Content-Security-Policy meta tags or HTTP response headers defined in or around fieldwork/index.html line 6.
2. Append the upgrade-insecure-requests directive to each CSP string, for example: Content-Security-Policy: <existing directives>; upgrade-insecure-requests
3. Verify no legitimate http:// subresources exist that would break if upgraded, and update any such references to https:// explicitly.
4. Test the pages in a browser with mixed-content warnings enabled to confirm no console errors appear after the change. |
| Private | {IS_PRIVATE} |
