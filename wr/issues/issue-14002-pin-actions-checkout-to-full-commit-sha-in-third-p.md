# WR: [WR] Pin actions/checkout to full commit SHA in third-party audit workflow
...
| Description | ## Summary
The third-party action freshness audit workflow uses a mutable version tag reference (actions/checkout@v4) rather than a full commit SHA pin. This is inconsistent with the repository's established supply-chain security standard enforced by workflow-action-ref-audit.yml, and creates an ironic situation where the audit workflow designed to catch unpinned actions is itself vulnerable to the same class of supply-chain risk it is meant to detect.

## Details
Mutable tag references such as @v4 can be silently redirected to a different — potentially malicious — commit without any change to the workflow file. The repository already mandates SHA-pinned action references via workflow-action-ref-audit.yml to mitigate this risk. The audit workflow at third-party-action-audit.yml violates that very standard on line 38 by referencing actions/checkout@v4. If the auditor were ever extended to scan its own file, it would flag itself. Beyond the logical inconsistency, any compromise of the actions/checkout v4 tag would affect this workflow without detection.

## Location
File: .github/workflows/third-party-action-audit.yml, line 38
...
## Suggested Action
1. Look up the full commit SHA for the intended actions/checkout v4 release (e.g., via the actions/checkout releases page or by running: git ls-remote https://github.com/actions/checkout refs/tags/v4).
2. Replace the reference on line 38 from actions/checkout@v4 with actions/checkout@<full-40-character-SHA>.
3. Add an inline comment on the same line noting the version tag that the SHA corresponds to, for human readability (e.g., # v4.1.7).
4. Consider adding third-party-action-audit.yml itself to the scope of workflow-action-ref-audit.yml scans so self-referential violations are caught automatically in future. |
| Private | {IS_PRIVATE} |
