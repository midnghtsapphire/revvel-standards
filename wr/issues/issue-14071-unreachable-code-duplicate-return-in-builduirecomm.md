...
## Location
File: scripts/ui-creation-engine.js, line 504
PR: #13854 — fix: inject MCP master prompt pack in UI recommendation generation
...
3. Add a unit or snapshot test that asserts the returned prompt contains the key section headers (design system, component library, page layout, differentiation) to prevent regressions.
4. Verify the file parses without SyntaxError and that generated prompts match expectations end-to-end. |
| Private | {IS_PRIVATE} |
