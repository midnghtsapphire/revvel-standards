...
Fallback needs to be perplexity no api then open router the ollama. Create a look up table or spreadsheet and assign everything to variables this is getting to hard. No hardcoded anything in any code. We change one bit field on or off for model. Load all top models f4om openrouter using top performing models, uncensored, etc what ones are good for seo, put what they are good for especially legal, dominance, patents, copyright, cancer,  chemical spills, etc. Another thing we need an enterprise table Enterprise and Participants ill revisit my old architecture that way I can load us as enterprise, but I can load other enterprises, participant can relate to oAudrey as a drop shipper, an api. Cli, mcp, notion, I can put notion as a participant linked to enterprise oAudrey then i can give notion participants like slack, github, supabase, openrouter or Slack with all its connectors that way we can know what's available in each product and use it. Or get a product like linear to use its pr functionality-we dont know if its not there. I dont want to depend on resesrch engine because I will grow old and die before it runs with consistency. There will be look up tables for everything and then we can use numbers for search values instead of text strings. So if my enterprise oAudrey is the first 1 then my participant will be 1, etc its more complicated I need to find my model.we are really frowning on searching string values 'common frequently used strings will be given numeric values
