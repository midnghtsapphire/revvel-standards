# WR: [WR] Fix broken doc reference to non-existent CREDENTIAL_AUTOMATION_ROADMAP.md
...
| Description | ## Summary
Item 4 of the Octopus audit tracking issue contains an acceptance criterion that references docs/CREDENTIAL_AUTOMATION_ROADMAP.md as the update target for the 90-day blob rotation watchdog. This file does not exist anywhere in the repository, making the acceptance criterion unverifiable and potentially misleading for anyone attempting to complete or review that work item.

## Details
The rotation policy section relevant to this work item is being written into docs/SECRETS_MANAGEMENT.md, which does exist. The reference to docs/CREDENTIAL_AUTOMATION_ROADMAP.md appears to be either a leftover from an earlier document restructuring, a copy-paste error, or a placeholder that was never resolved. Because acceptance criteria are used to gate completion of deferred items, a ghost file reference means the criterion can never be formally satisfied and may cause confusion during handoff or audit review.

## Location
File: wr/issues/issue-octopus-audit-2026-05-28-tracking.md, line 87
...
## Suggested Action
1. Open wr/issues/issue-octopus-audit-2026-05-28-tracking.md and locate line 87.
2. Replace the reference to docs/CREDENTIAL_AUTOMATION_ROADMAP.md with the correct target file, docs/SECRETS_MANAGEMENT.md, ensuring the surrounding acceptance criterion text remains accurate.
3. Confirm that docs/SECRETS_MANAGEMENT.md already contains or is slated to contain the 90-day blob rotation watchdog policy section; if not, open a follow-up task to add it.
4. Search the rest of the tracking file and any related documents for additional occurrences of docs/CREDENTIAL_AUTOMATION_ROADMAP.md and update or remove them as appropriate. |
| Private | {IS_PRIVATE} |
