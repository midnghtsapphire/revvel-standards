# WR: [WR] Separate generated dashboard.html artifact from feature PR commits
...
| Description | ## Summary
The file dashboard.html contains a hardcoded timestamp string that is produced by a generation script rather than written by hand. This generated artifact has been committed alongside feature changes in the same pull request, which introduces unnecessary diff noise and conflates generated output with intentional source modifications.

## Details
At line 155 of dashboard.html, a timestamp value appears as a literal string. Because this value originates from a dashboard generation script, the file is a build artifact rather than a maintained source file. Committing generated artifacts in feature PRs makes code review harder, pollutes the git history with unrelated changes, and risks merge conflicts when multiple branches regenerate the same file independently. The root concern is not the hardcoded value itself but the workflow that bundles generated output with source changes.

## Location
File: dashboard.html, line 155
...
## Suggested Action
1. Determine whether dashboard.html is fully generated or partially hand-edited. If fully generated, add it to .gitignore so it is never committed as part of feature PRs.
2. If the file must be tracked in version control (e.g., for deployment or auditing purposes), configure CI to regenerate and commit it in a dedicated automated commit or release step, separate from feature branches.
3. If option 2 is chosen, document the generation process in a README or CONTRIBUTING guide so contributors know not to manually edit or commit this file.
4. Review any existing commits in the repository where dashboard.html was bundled with unrelated changes and consider whether a cleanup or rebase is warranted. |
| Private | {IS_PRIVATE} |
