...
This is labeled `work-request` + `openrouter` + `wr:code` so the OpenRouter coder picks it up directly (no research phase needed — the change is fully specified above).
