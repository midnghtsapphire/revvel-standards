...
| Description | ## Summary
When AUDIT_FORMAT=json is set, the script pipes JSON output to stderr and suppresses all jq errors with '|| true', meaning the JSON result is silently lost. The workflow never sets this environment variable, making the entire JSON branch dead code that gives future maintainers a false expectation of reliable JSON output.

## Details
The JSON formatting branch in audit-third-party-actions.sh writes its output to stderr instead of stdout or a dedicated file, and appends '|| true' to swallow any jq processing failures without surfacing them. This combination means that even if a caller did set AUDIT_FORMAT=json, the resulting JSON would never be captured, validated, or made available to downstream workflow steps. Because the workflow itself never sets AUDIT_FORMAT=json, this code path is entirely untested and unreachable in practice. The silent failure mode is particularly dangerous because it could mask malformed JSON or broken jq filters with no indication that anything went wrong.

## Location
File: scripts/audit-third-party-actions.sh, line 174
...
## Suggested Action
1. Decide whether JSON output is a required feature. If yes, redirect the jq output to a dedicated file (e.g., audit-results.json), expose the path via GITHUB_OUTPUT, and set AUDIT_FORMAT=json in the workflow that should consume it. Remove the '|| true' suppression and add explicit error handling so jq failures fail the step. If no, remove the JSON branch entirely to eliminate misleading dead code and reduce maintenance burden. In either case, add a workflow-level test or a dry-run invocation that exercises the chosen path to prevent silent regressions. |
| Private | {IS_PRIVATE} |
