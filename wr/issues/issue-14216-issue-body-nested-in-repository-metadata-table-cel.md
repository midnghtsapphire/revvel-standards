# WR: [WR] Issue body nested in Repository Metadata table cell breaks Markdown rendering
...
## Details
Starting at line 94 of the issue file, approximately 22 lines of content — including headings and pipe characters — are placed inside a single `| Description |` table cell. The table then attempts to resume with `| Private | {IS_PRIVATE} |`. Because Markdown tables do not support multi-line content, headings, or unescaped pipe characters within a cell, renderers will either treat the embedded pipes as new column delimiters or terminate the table prematurely. The result is a broken metadata table and poorly formatted issue content, which undermines readability and the document's usefulness as a standards artifact.

## Location
File: wr/issues/issue-14067-wr-doc-retains-unfilled-template-placeholders-use-.md, line 94
Pull Request: [WR] WR doc retains unfilled template placeholders — use basic template or mark N/A (#14184)
URL: https://github.com/midnghtsapphire/revvel-standards/pull/14184

## Suggested Action
1. Extract the issue body content out of the Description cell in the Repository Metadata table.
2. Place this content in a dedicated section (e.g., 'Issue Context' or 'Source Issue') above the metadata table, using proper Markdown headings and paragraphs.
3. Keep the Description cell either empty, set to a short single-line summary, or remove it from the table if it is no longer needed.
4. Verify that the Repository Metadata table renders correctly end-to-end (all rows visible, no broken columns) in a standard Markdown viewer such as GitHub. |
| Private | {IS_PRIVATE} |
