# WR: [WR] Move multi-paragraph issue description out of Markdown table cell
...
| Description | ## Summary
The Description column of the Repository Metadata table in issue-13987-docs-add-for-illustration-only-caveat-above-openro.md contains multi-paragraph Markdown content — including headers, blockquotes, and code spans — embedded directly inside a pipe-delimited table cell. This breaks the table layout in most Markdown renderers, including GitHub, because any newline inside a table cell is interpreted as the end of the row.

## Details
Pipe-delimited Markdown tables do not support multi-line cell content. When headers, blockquotes, or code spans spanning multiple lines are placed inside a table cell, renderers either collapse the content incorrectly or abandon the table structure entirely, producing malformed output. This makes the rendered document harder to read and undermines the intent of the structured metadata table. The issue is low severity but affects documentation readability and professionalism across any GitHub interface or Markdown preview tool.
...
## Suggested Action
1. Remove the multi-paragraph Markdown content from the Description column of the Repository Metadata table.
2. Replace the table cell content with a short, single-line plain-text summary that does not contain newlines, headers, blockquotes, or code spans.
3. Add a new dedicated section — ## Issue Description — below the metadata table and move the full descriptive content there.
4. Verify the rendered output on GitHub after the change to confirm the table displays correctly and the description section is readable. |
| Private | {IS_PRIVATE} |
