# WR: [WR] Regex pattern change in augmentCheckRunPattern widens match scope unintentionally
...
| Description | ## Summary
A regex pattern change in the augment-check workflow replaced the end-of-string anchor `$` with a word boundary `\b`, broadening the set of check-run names that will match. While this may be intentional to support multi-word check-run names, the behavioral change is significant and has not been explicitly confirmed, especially given that other nearby changes already independently widen slug and name matching via `startsWith` logic.

## Details
The original pattern `/^augment(?:[-\s]?code)?(?::|$)/i` strictly required the check-run name to either end immediately after the app name or be followed by a colon separator. The updated pattern `/^augment(?:[-\s]?code)?(?::|\b)/i` replaces `$` with `\b` (word boundary). After a word character, `\b` matches any position followed by a non-word character, including a space and arbitrary trailing text. This means names such as `augment code review`, `augment something`, or any string prefixed with the app name and followed by a space will now match. Combined with the `startsWith`-based changes elsewhere in the same PR, the cumulative effect is a considerably wider detection net than the original implementation intended. If the new behavior is unintentional, it could cause the workflow to trigger on unrelated check runs, producing false positives in CI.

## Location
File: .github/workflows/augment-check.yml, line 131
Pull Request: Restore augment-check auto-detection (re-enable pull_request_target trigger) (#13974)
URL: https://github.com/midnghtsapphire/revvel-standards/pull/13974

## Suggested Action
1. Confirm whether matching multi-word check-run names (e.g., `augment code review`) is an explicit requirement. If yes, document the intent with an inline comment explaining why `\b` is preferred over `$`.
2. If the change was unintentional, revert the pattern to `/^augment(?:[-\s]?code)?(?::|$)/i` to restore the original strict boundary.
3. Add or update unit tests covering edge cases: names that end exactly at the app name, names with a colon separator, names with trailing words, and unrelated names that share a prefix.
4. Review the interaction between this pattern change and the `startsWith` broadening changes in the same PR to ensure the combined matching logic does not produce unintended false positives. |
| Private | {IS_PRIVATE} |
