...
## Suggested Action
1. Replace `anthropic/claude-opus-4.7` in the provenance example with either the actual primary model slug `claude-3.7-sonnet` as documented in `standards/AGENT_STACK_SETUP.md`, or a clearly marked placeholder such as `{model}` to signal that the value must be populated dynamically at runtime.
2. If a placeholder approach is chosen, add an inline comment or accompanying note in the document explaining that `{model}` must be substituted with the active model slug before inclusion in a PR body.
3. Consider cross-referencing `standards/AGENT_STACK_SETUP.md` in this section so readers know where the authoritative model list is maintained. |
| Private | {IS_PRIVATE} |
