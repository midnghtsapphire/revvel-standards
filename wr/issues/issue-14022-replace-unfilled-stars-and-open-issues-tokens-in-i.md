# WR: [WR] Replace unfilled {STARS} and {OPEN_ISSUES} tokens in issue-13971 WR
...
| Description | ## Summary
The work report for issue-13971 is marked as complete but still contains two unreplaced template placeholder tokens, {STARS} and {OPEN_ISSUES}, in the Repository Metadata table. A work report flagged as complete must have all template tokens substituted with real data or an appropriate approximation before being merged.

## Details
In the Repository Metadata table at line 92 of the WR file, the Stars and Open Issues rows retain the raw template tokens {STARS} and {OPEN_ISSUES} rather than actual or estimated values. This is inconsistent with the WR being marked with a completion status indicator. Other completed WRs in the same directory, such as issue-13535, demonstrate the expected pattern of replacing these tokens with real figures or clearly annotated approximations (e.g., ~10, N/A). Leaving placeholder tokens in a completed document reduces its credibility, breaks downstream automation or tooling that may parse these fields, and signals an incomplete review process.

## Location
File: wr/issues/issue-13971-add-this-repository-as-a-high-value-seo-sem-deep-w.md, line 92
...
## Suggested Action
1. Look up the current star count and open issue count for the target repository on GitHub or the relevant platform.
2. Replace {STARS} with the actual star count or an annotated approximation such as ~500 if exact data is unavailable.
3. Replace {OPEN_ISSUES} with the actual open issue count or an annotated approximation such as ~20 if exact data is unavailable.
4. If the data cannot be obtained, substitute N/A with a brief explanatory note.
5. Verify no other unreplaced template tokens remain in the document before re-requesting review. |
| Private | {IS_PRIVATE} |
