# WR: [WR] Standardize Octopus Review trigger description in bot roster table
...
| Description | ## Summary
The Octopus Review entry in the bot roster table uses a vague trigger description ('auto on PR') that is inconsistent with the level of detail provided for other bots such as CodeRabbit and Jules, which specify their exact invocation mechanisms. This inconsistency reduces the clarity and maintainability of the documentation.

## Details
In the workflow file, the Trigger cell for Octopus Review reads 'auto on PR', while other bot entries explicitly state the GitHub Actions event and conditions that activate them, for example 'pull_request: [opened, synchronize, ready_for_review]' or a reference to the relevant workflow filename. The vague description makes it harder for contributors to understand when Octopus Review will run, how to reproduce or debug trigger-related issues, and how to maintain consistency across the roster as new bots are added. Following the pattern already established by existing rows would make the table a reliable reference.

## Location
File: .github/workflows/ready-for-review.yml, line 256
...
## Suggested Action
1. Identify the exact GitHub Actions event and activity types that trigger Octopus Review (e.g., pull_request: [opened, synchronize, ready_for_review]) or note the specific workflow filename responsible.
2. Replace the current 'auto on PR' text in the Trigger cell with the precise trigger expression or filename.
3. Review all other rows in the roster table to confirm they follow the same level of detail, and update any similarly vague entries for consistency. |
| Private | {IS_PRIVATE} |
