# WR: [WR] Fix broken doc reference in Issue #7 acceptance criteria (TESTING_STACK.md)
...
| Description | ## Summary
Item 7 of the Octopus audit tracking issue references a non-existent file, docs/TESTING_STACK.md, as the authoritative Keploy guidance source within its acceptance criteria. Because this file does not exist in the repository, any contributor picking up this work item will immediately hit a dead end trying to locate the referenced documentation.

## Details
The acceptance criteria for Item 7 (sparse test coverage) cite docs/TESTING_STACK.md as the source of Keploy guidance. A review of the repository file tree confirms this path does not exist. The two closest candidates are standards/TESTING.md and docs/Master_Inventory/TESTING_STANDARD.md. Linking to a missing document in acceptance criteria is a workflow blocker: it erodes confidence in the issue's accuracy, wastes triage time, and may cause the wrong standards to be applied if a contributor guesses the intended target incorrectly.

## Location
File: wr/issues/issue-octopus-audit-2026-05-28-tracking.md, line 120
...
## Suggested Action
1. Confirm which file is the intended Keploy guidance source by consulting the PR author or the standards team — likely standards/TESTING.md or docs/Master_Inventory/TESTING_STANDARD.md.
2. Update line 120 of wr/issues/issue-octopus-audit-2026-05-28-tracking.md to reference the correct, verified file path.
3. If neither existing file fully covers Keploy guidance, create the missing documentation at the agreed canonical path before or alongside this fix, and update the reference accordingly.
4. Review the remaining 7 deferred items in the same tracking file for any additional stale or incorrect document references. |
| Private | {IS_PRIVATE} |
