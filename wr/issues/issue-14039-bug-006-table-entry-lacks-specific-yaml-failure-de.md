# WR: [WR] BUG-006 table entry lacks specific YAML failure details for faster resolution
...
| Description | ## Summary
The tracking table row for BUG-006 describes the affected files in generic terms and instructs the implementer to run linting tools locally without identifying the specific failure modes already documented in the audit. This forces the implementer to rediscover known issues rather than acting on existing findings.

## Details
The table entry at line 133 states 'YAML failures in `api-rate-limit-handler.yml` + `jules-coding-agent.yml`' and offers only 'run yamllint/actionlint locally' as guidance. However, the original audit in `docs/REVVEL_STANDARDS_AUDIT.md` already identifies `jules-coding-agent.yml` as an admitted stub with concrete known issues such as empty branches and invalid expressions. Omitting this context means the assignee must independently re-run tooling and interpret output before they can begin the actual fix, wasting time and risking inconsistent diagnosis. Referencing the specific offending block or failure type (e.g., empty branch in the on-push job, invalid expression syntax) would make the work item immediately actionable and aligned with the audit record.

## Location
File: wr/issues/issue-octopus-audit-2026-05-28-tracking.md, line 133
PR: Octopus audit 2026-05-28 tracking WR (8 deferred items) (#13978)
PR URL: https://github.com/midnghtsapphire/revvel-standards/pull/13978

## Suggested Action
1. Open `docs/REVVEL_STANDARDS_AUDIT.md` and locate the BUG-006 entry to retrieve the specific failure descriptions already captured there.
2. Update the BUG-006 table row to name the exact offending block or construct for each affected file, for example: 'empty branch in on-push job (`jules-coding-agent.yml`)' and the corresponding issue in `api-rate-limit-handler.yml`.
3. Replace or supplement the generic 'run yamllint/actionlint locally' instruction with a reference to the audit finding so the implementer has a clear starting point without needing to re-run discovery tooling.
4. Verify the updated row is consistent with any related entries in the audit document to avoid discrepancies between the two sources. |
| Private | {IS_PRIVATE} |
