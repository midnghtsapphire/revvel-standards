# WR: [WR] upgrade cli engine in revvel-standards
...
Here are two master prompts specifically designed to generate a high-end, glassmorphic UI landing page for a **Command Line Interface (CLI) Agent**.
These prompts blend a gritty, hyper-technical terminal look with a premium, sleek 3D glossy aesthetic, perfectly capturing the moment an engine executes code, handles a transaction, or generates a downloadable artifact.
### Prompt 1: The Active CLI Terminal & Code Execution (Tech-Heavy & Premium)
> **Prompt:** A cinematic hero shot of an ultra-modern landing page featuring a premium glassmorphic CLI (Command Line Interface) terminal window. The terminal is a floating, thick-cut piece of heavy frosted glass with a dark, translucent finish, hovering over a moody, minimalist background. Inside the terminal, rows of razor-sharp, glowing green and neon-white monospaced code strings and script executions cascade down. A semi-transparent glass panel hovers nearby, displaying a dynamic data stream or purchase confirmation log. Soft atmospheric haze and low drift wrap around the UI layers, catching sharp cool-blue rim lighting on the refractive, glossy glass edges. Photorealistic, 8k resolution, elegant 3D realism, studio lighting, hyper-detailed terminal UX.
> 
### Prompt 2: The CLI Developer Canvas with PDF Export (Sleek & Action-Oriented)
> **Prompt:** A medium close-up cinematic shot of a developer agent landing page showcasing an interactive CLI engine. The central UI features a layered glassmorphic terminal window running automated scripts, showing a crisp code block with an amber and white syntax theme. Overlapping the terminal is a floating, translucent 3D glass card symbolizing a generated PDF report with a sleek, minimalist layout. A prominent, glossy glassmorphic "Run & Export" button reflects a warm accent light, casting soft, realistic shadows onto the dark background below. Hyper-realistic, 8k, ray-traced reflections, premium developer tool visualization, 3D glossy realism, tactile glass textures.
> 
### 💡 Tips for Fine-Tuning the CLI Vibe:
 * **To change the prompt style:** If you want a classic hacker/security vibe, specify matrix-green syntax highlighting. For a modern cloud/devops feel, change it to cool white and electric blue monospaced text.
 * **To make it feel more active:** Add terms like with a flashing block cursor or showing real-time compilation logs to give the static image a sense of movement.
