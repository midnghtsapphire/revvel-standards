# WR: [WR] Fix orphaned table row breaking BOM Cost Summary Markdown table
...
| Description | ## Summary
A stray pipe-delimited row in the BOM Cost Summary table is separated from the table body by a blank line, causing it to render as an orphaned string rather than a proper table row in GitHub Markdown. This makes the document appear visually broken in the rendered view.

## Details
The row `| **Total Infrastructure** | | **$0/mo (Varies heavily by volume)** |` appears after a blank line following the last valid table row. In GitHub Flavored Markdown, a blank line terminates a table block, so any pipe-separated content placed after it is treated as plain paragraph text rather than a table row. As a result, readers see a dangling pipe-separated string beneath the table instead of a properly formatted summary row, degrading document readability and professionalism.

## Location
File: wr/issues/issue-13755-evaluate-and-research-and-implement-boberdoo-for-l.md, line 140
...
## Suggested Action
1. Remove the blank line between the last valid table row and the Total Infrastructure row so that it immediately follows the preceding row with no empty lines in between.
2. Verify that the column count of the Total Infrastructure row matches the header row exactly, filling any missing cells with empty pipe-delimited cells if needed.
3. Re-render the Markdown locally or via the GitHub preview to confirm the row displays correctly inside the table. |
| Private | {IS_PRIVATE} |
