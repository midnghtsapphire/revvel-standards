# WR: [WR] Fix shell injection risk from unquoted workflow output in audit step
...
| Description | ## Summary
A workflow output variable is expanded directly into a shell command and a Node.js file-read call without adequate quoting or sanitization. While the current value of report_file originates from a controlled run step, the unquoted expansion creates a latent shell injection vector that would become exploitable if the output were ever sourced from user-controlled input in a future refactor.

## Details
In the third-party-action-audit workflow, the expression ${{ steps.audit.outputs.report_file }} is interpolated unquoted into a cat shell command and also passed directly into a fs.readFileSync() call inside a github-script step. GitHub Actions expressions are resolved before the shell interprets the command, meaning any special characters or whitespace in the value are passed raw to the shell. Even though RUNNER_TEMP is a trusted path today, the output binding is not enforced to stay that way. If a future change sets report_file from an external trigger, pull request title, or any other user-supplied data, this becomes a full shell injection and arbitrary file read vulnerability. The risk is rated high because the pattern is structurally unsafe regardless of current usage, and automated security scanners will flag it as a critical finding in CI.

## Location
File: .github/workflows/third-party-action-audit.yml, line 57
Pull Request: Third-party action freshness audit — quarterly scan + auto-WR on stale single-author actions (#13993)
URL: https://github.com/midnghtsapphire/revvel-standards/pull/13993

## Suggested Action
1. In the shell step, assign the expression to an environment variable first (e.g., REPORT_FILE: ${{ steps.audit.outputs.report_file }}) and then reference it as the quoted shell variable "$REPORT_FILE" in the cat command. Environment variable interpolation happens at the shell level and is not subject to expression injection.
2. In the github-script step, retrieve the value through context.env.REPORT_FILE (set via the env block) rather than embedding the Actions expression directly inside the script string.
3. Add an explicit validation step that asserts the resolved path is within RUNNER_TEMP before use, to enforce the trust assumption programmatically.
4. Consider defining the report file name as a workflow-level constant rather than a mutable step output to prevent accidental override in future iterations. |
| Private | {IS_PRIVATE} |
