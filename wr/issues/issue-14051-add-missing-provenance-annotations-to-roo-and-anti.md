# WR: [WR] Add missing provenance annotations to Roo and Antigravity reviewer rows
...
| Description | ## Summary
The Roo and Antigravity entries in the ready-for-review workflow roster were not updated during the provenance annotation pass applied to all other reviewer rows in PR #13976. This leaves those two rows in their original sparse format, lacking vendor identity and action/app references that every other row now carries, creating an inconsistency in the reviewer configuration.

## Details
As part of PR #13976, provenance metadata (vendor identity and action/app references) was systematically applied to reviewer roster entries to improve auditability and traceability. All rows except Roo and Antigravity received this treatment. The omission means these two entries cannot be audited or traced to the same standard as the rest of the roster. This inconsistency could cause confusion during maintenance, make automated tooling that relies on provenance fields behave unpredictably for those entries, and undermine the intent of the annotation effort. The severity is low since functionality is likely unaffected, but the gap should be resolved or explicitly documented as intentional.

## Location
File: .github/workflows/ready-for-review.yml, line 261
Pull Request: Add Octopus Review to roster + apply provenance to every reviewer (#13976)
URL: https://github.com/midnghtsapphire/revvel-standards/pull/13976

## Suggested Action
1. Open .github/workflows/ready-for-review.yml and locate the Roo and Antigravity rows near line 261.
2. Apply the same provenance fields (vendor identity, action reference, and any other metadata) used on the surrounding reviewer rows.
3. If there is a deliberate reason these two rows should remain unannotated, add an inline comment explaining the exception so future maintainers understand the intent.
4. Verify the updated entries are consistent with all other annotated rows before merging. |
| Private | {IS_PRIVATE} |
