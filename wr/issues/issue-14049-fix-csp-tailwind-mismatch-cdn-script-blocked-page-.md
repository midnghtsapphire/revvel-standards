# WR: [WR] Fix CSP/Tailwind mismatch: CDN script blocked, page renders unstyled
...
| Description | ## Summary
The osint-hub page declares a Content Security Policy with `script-src 'self'` but simultaneously loads Tailwind CSS from an external CDN and defines an inline configuration block. Because the CDN origin and inline scripts are not permitted by the policy, any browser that enforces CSPs will block both resources, causing the entire page to render completely unstyled.

## Details
The CSP directive `script-src 'self'` whitelists only same-origin scripts. It does not include `https://cdn.tailwindcss.com`, so the `<script src="https://cdn.tailwindcss.com">` tag will be blocked. Additionally, the inline `tailwind.config = {...}` assignment is blocked because neither `'unsafe-inline'` nor an appropriate hash/nonce is present in the directive. The two resources are effectively dead weight under the current policy, yet they are relied upon for all visual styling. This is a functional regression: in security-conscious environments (and all modern browsers in strict mode) the UI breaks entirely. The mismatch also undermines the security intent of the CSP, since a developer may later "fix" it by weakening the policy rather than removing the CDN dependency.
...
## Suggested Action
1. Preferred approach — replace the CDN dependency with a build-time Tailwind bundle (e.g., via the Tailwind CLI or PostCSS). Ship the compiled CSS as a static asset loaded from the same origin. This eliminates the need to touch the CSP at all and removes the runtime CDN call.
2. If a CDN approach must be kept as a short-term measure, add `https://cdn.tailwindcss.com` to `script-src` and supply a `'nonce-<value>'` or SHA-256 hash for the inline `tailwind.config` block so both resources are explicitly permitted. Update the CSP header or meta tag accordingly.
3. After applying either fix, verify the CSP in browser DevTools (Network and Console panels) to confirm no blocked-resource errors remain before merging. |
| Private | {IS_PRIVATE} |
