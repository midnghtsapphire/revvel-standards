# WR: [WR] Fix broken source paths in APP_REGISTRY.md pointing to non-existent build/src dirs
...
| Description | ## Summary
The APP_REGISTRY.md file contains a 'Source of truth' column that references module source paths under build/src subdirectories (e.g., products/life-insurance-lead-engine/build/src/components and products/life-insurance-lead-saas/build/src/components). These paths do not exist in the repository. The actual product directories contain only top-level files such as BLUEPRINT.md, BOM.md, state.json, and empty lifecycle directories, with no build/ subdirectory present. This breaks the reuse-first goal the registry is designed to enforce.

## Details
When agents or developers consult APP_REGISTRY.md to locate existing components for reuse, the invalid paths will cause lookups to fail silently. Unable to find referenced components, agents may incorrectly conclude that no reusable implementation exists and proceed to rebuild components from scratch — directly undermining the efficiency and consistency goals of the registry. The build/src path convention appears to have been assumed or copied from a different project structure without being verified against the actual repository layout. Both affected product entries (life-insurance-lead-engine and life-insurance-lead-saas) are impacted.

## Location
File: docs/APP_REGISTRY.md, line 22
...
## Suggested Action
1. Inspect the actual directory structure under products/life-insurance-lead-engine/ and products/life-insurance-lead-saas/ to identify where source components are genuinely located.
2. Update the 'Source of truth' column in APP_REGISTRY.md to reflect the correct, verified paths for each product entry.
3. If no source component directory yet exists for a product, either omit the path or mark the field explicitly as 'not yet available' to avoid misleading consumers.
4. Add a note to the registry contribution guidelines requiring that all source paths be verified against the actual repository tree before being committed. |
| Private | {IS_PRIVATE} |
