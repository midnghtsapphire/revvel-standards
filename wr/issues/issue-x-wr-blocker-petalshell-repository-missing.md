# WR: WR-BLOCKER petalshell repository missing

## Issue Context

The fleet-maintenance sweep triggered a Work Request (WR) for the repository `midnghtsapphire/petalshell`. However, attempting to access this repository returns a 404 response.

Running `curl -s https://api.github.com/repos/midnghtsapphire/petalshell` indicates the repository is missing.

Because of this, I cannot clone the repository or perform the required maintenance and updates. No Work Request or fix could be made.
...
Permanent for every WR type — implementers must not stop at the issue: N/A
