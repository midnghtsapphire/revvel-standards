...
4. K6 (Grafana k6 load-testing tool) repository evaluation sprint focused on load/performance automation patterns that can harden the prompt-to-asset pipeline, because this internal tool still needs predictable batch throughput when running large prompt chains and agent fan-out.
