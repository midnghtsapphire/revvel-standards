# WR: [WR] Clarify no-hyphen augmentcode patterns to prevent future redundancy confusion
...
| Description | ## Summary
The author detection patterns for augmentcode[bot] and augmentcode in the augment-check workflow are functionally correct but lack explanatory comments. Without documentation, future maintainers may incorrectly conclude these entries are redundant duplicates of the existing augment(?:-code)? patterns and remove them, which would break detection of the no-hyphen variant.

## Details
The existing patterns /^augment(?:-code)?\[bot\]$/i and /^augment(?:-code)?$/i use an optional group that matches only augment or augment-code (with a hyphen). They do not match augmentcode (without a hyphen), making the newly added /^augmentcode\[bot\]$/i and /^augmentcode$/i entries genuinely necessary. However, because the optional group (?:-code)? superficially resembles a catch-all for all code-suffixed variants, the no-hyphen entries look like duplicates at first glance. This false appearance of redundancy is a maintenance hazard: a well-intentioned cleanup pass could silently remove required patterns and cause the auto-detection to miss a valid bot identity.

## Location
File: .github/workflows/augment-check.yml, line 120
...
## Suggested Action
1. Add an inline comment directly above or beside the /^augmentcode\[bot\]$/i and /^augmentcode$/i entries explaining that these cover the no-hyphen form of the username, which is intentionally distinct from the hyphenated augment-code variant handled by the (?:-code)? group.
2. Optionally, add a brief block comment near the full pattern list summarising the three distinct identity forms being matched: augment, augment-code, and augmentcode.
3. Review whether consolidating all variants into a single regex such as /^augment(-code|code)?\[bot\]$/i is feasible and would reduce the number of entries while preserving clarity. |
| Private | {IS_PRIVATE} |
