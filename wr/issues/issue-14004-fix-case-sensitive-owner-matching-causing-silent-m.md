# WR: [WR] Fix case-sensitive owner matching causing silent mis-classification in audit script
...
| Description | ## Summary
The owner classification logic in the third-party action audit script performs case-sensitive string matching when checking whether an owner belongs to the MULTI_AUTHOR_OWNERS list. Because GitHub treats owner names as case-insensitive, valid variations such as "beksomega" or "BEKSOMEGA" are functionally equivalent to "BeksOmega", but the current implementation will fail to match them and silently fall through to the single-author tier, causing incorrect flagging and potentially triggering unwarranted stale-action warnings.

## Details
The contains() helper function uses a Bash exact-match comparison ([[ "$item" == "$needle" ]]) with no case normalization applied beforehand. The MULTI_AUTHOR_OWNERS list already includes mixed-case entries such as "BeksOmega" and "Mergifyio". When a workflow file references those owners using any casing variant that GitHub itself accepts as valid, the lookup silently fails. The owner is then misclassified as single-author, and the auto-WR (watch-and-report) mechanism may raise a stale-action flag against an action that should have been exempt. This is a silent failure — no error or warning is emitted, making it difficult to detect during manual review.

## Location
File: scripts/audit-third-party-actions.sh, line 80
...
## Suggested Action
1. Normalize both the needle and each list item to lowercase before comparison inside the contains() function. This can be done using Bash parameter expansion: compare "${item,,}" against "${needle,,}".
2. Optionally, store all entries in MULTI_AUTHOR_OWNERS in lowercase to make the canonical form explicit and reduce reliance on runtime normalization.
3. Add a test case that passes a mixed-case owner name known to be in the list and asserts it is classified as multi-author, to prevent regression.
4. Review any other string-comparison sites in the script that handle owner or action names to ensure consistent case handling throughout. |
| Private | {IS_PRIVATE} |
