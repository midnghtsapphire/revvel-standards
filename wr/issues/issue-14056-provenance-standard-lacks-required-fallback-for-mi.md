# WR: [WR] Provenance standard lacks required fallback for missing version fields
...
| Description | ## Summary
The PROVENANCE_STANDARD.md defines a format template requiring `package@version`, but several entries in the quick-reference table at the bottom of the document omit the version component entirely. This inconsistency undermines the precision goal of the standard and leaves contributors without clear guidance on how to handle tools where an exact version is unavailable.

## Details
The format spec at line 16 presents `package@version` as the canonical template, implying version is always required. However, entries such as `github.com/apps/keploy + Chrome extension` and `openrouter-coder.yml` appear in the reference table without any version annotation. Because the document does not define a sanctioned fallback token for unknown or unversioned tools, contributors must either guess or leave the field blank, both of which introduce ambiguity. The standard should explicitly enumerate acceptable substitutes, such as `@latest` for tools that only publish a rolling release, or a sentinel like `(version unknown)` for cases where the version genuinely cannot be determined. Without this, the quick-reference table will continue to grow with inconsistent entries, making automated parsing and auditing of provenance records unreliable.

## Location
File: docs/PROVENANCE_STANDARD.md, line 16
Pull Request: Add Provenance Standard — name publisher + exact package for every tool (#13975)
URL: https://github.com/midnghtsapphire/revvel-standards/pull/13975

## Suggested Action
1. Update the format spec section (line 16) to explicitly document the required fallback tokens, for example: use `@latest` when the tool publishes only a rolling/latest release, and use `@unknown` or `(version unknown)` when the version cannot be determined at the time of entry.
2. Add a short prose paragraph explaining when each fallback is appropriate and discouraging blank version fields.
3. Retroactively update all existing quick-reference table entries that currently omit versions (including `github.com/apps/keploy + Chrome extension` and `openrouter-coder.yml`) to use the newly defined fallback tokens.
4. Consider adding a linting rule or CI check to reject provenance entries that do not include a version field or an approved fallback token. |
| Private | {IS_PRIVATE} |
