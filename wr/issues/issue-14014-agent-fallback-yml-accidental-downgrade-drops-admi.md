# WR: [WR] agent-fallback.yml: accidental downgrade drops ADMIN_GITHUB_TOKEN from checkout/PR steps
...
| Description | ## Summary
The agent-fallback workflow has had its ADMIN_GITHUB_TOKEN-first fallback expressions silently replaced with plain secrets.GITHUB_TOKEN on two critical steps — checkout (L158) and create-pull-request (L313). This contradicts the intent of the broader PR, which adds ADMIN_GITHUB_TOKEN fallbacks across several other workflows, and introduces a regression that can break downstream workflow triggers and write permissions for AI-agent-created branches and pull requests.

## Details
GitHub's loop-prevention rule prevents pull requests opened with secrets.GITHUB_TOKEN from triggering other workflows. The agent-fallback workflow exists specifically to create branches and open PRs on behalf of AI agents, making downstream workflow triggering essential to its purpose. Additionally, in repositories that enforce stricter branch protection rules, GITHUB_TOKEN may lack the write permissions that ADMIN_GITHUB_TOKEN would provide. The original expressions used a fallback pattern (ADMIN_GITHUB_TOKEN first, GITHUB_TOKEN as a secondary fallback), which was intentional and safe. The current change removes that safety net without explanation or accompanying documentation. The rest of the PR moves consistently in the opposite direction, suggesting this downgrade is likely an unintentional mistake rather than a deliberate design decision.

## Location
File: .github/workflows/agent-fallback.yml, lines 158 and 313
Pull Request: add detailed tests for red ocean scoring (#13951)
PR URL: https://github.com/midnghtsapphire/revvel-standards/pull/13951

## Suggested Action
1. Restore the ADMIN_GITHUB_TOKEN-first fallback expression on both the checkout step (L158) and the create-pull-request step (L313), matching the pattern used in the other workflows updated in this PR.
2. If the ADMIN_GITHUB_TOKEN secret has been intentionally removed from this repository, document that decision explicitly and audit all workflows that depend on it, confirming whether downstream workflow triggers will still function correctly with GITHUB_TOKEN alone.
3. Add a comment in the workflow file explaining the token fallback strategy so future contributors do not inadvertently simplify it again.
4. Verify end-to-end that PRs opened by the agent-fallback workflow successfully trigger any dependent workflows after the fix is applied. |
| Private | {IS_PRIVATE} |
