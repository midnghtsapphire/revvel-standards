...
Dedupe must run at every layer where duplication can occur. A guard at the source (GitHub) does not protect the mirror (files). Guard the _invariant_, not the _entry point_.
