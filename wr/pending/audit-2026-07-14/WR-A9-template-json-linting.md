...
**Fix.** Rename misp-profile.json → misp-profile.json.hbs (still pending on this branch — the rename was applied in the local audit copy; an agent should complete it here). Sweep for other `{{`-containing .json files; codify in standards/: _templates carry a template extension; rendered output carries the data extension._
