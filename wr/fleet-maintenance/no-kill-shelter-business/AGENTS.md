...
**No-Kill Shelter Business** is the private hub for no-kill animal shelter
business model documentation under Freedom Angel Corp / MIDNGHTSAPPHIRE.
It holds operating playbooks, funding notes, compliance checklists, and
links to adjacent rescue / adoption tooling. Documentation-and-standards
first; application code is optional under `src/`.

### Architecture

```text
README.md / CONTRIBUTING.md / AGENTS.md   # entry + agent contract
docs/OVERVIEW.md                          # monetization + competitor context
.github/workflows/                        # OpenRouter, Jules, Semgrep, CodeQL
package.json + tests/                     # baseline structure tests
docs/                                     # business plans, research, PDFs
...
- Docs hub and review jury workflows are the maintenance baseline.
- No application runtime is required for `npm test`.
- Do **not** paste unrelated marketplace or Expo app scaffolding into this repo.
- Preserve the no-kill mission framing: intake → foster/adopt → outcome tracking.
- When adding a tool, prefer FOSS stacks (adoption boards, volunteer scheduling)
  and record live URLs in README when deployed.

### Secrets

See CONTRIBUTING.md. Never commit `.env`. OpenRouter and Jules keys are
optional for local work; CI skips advisory lanes when unset.
