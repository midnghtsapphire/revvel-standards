...
/**
 * Baseline structure tests for no-kill-shelter-business.
...
if (!workflowsOnly) {
  ...
  test("README documents the review jury and no-kill mission", () => {
    ...
    assert.match(readme, /no-kill/i);
  });

  test("AGENTS.md project context is shelter business, not marketplace scaffold", () => {
    const agents = read("AGENTS.md");
    const project = agents.split("## Project-Specific Context")[1] || "";
    assert.match(agents, /No-Kill Shelter Business/i);
    assert.match(project, /foster|adoption|shelter/i);
  ...
  test("package.json exposes npm test", () => {
    ...
    assert.equal(pkg.name, "no-kill-shelter-business");
