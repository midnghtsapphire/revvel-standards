...
async function createGitHubRepo(token) {
  try {
    ...
    try {
      ...
      // Use gh CLI
      execSync(
        `gh repo create ${CONFIG.repoName} --public --clone --description "${CONFIG.description}"`,
        {
...
function initGit(token) {
  try {
    ...
    // Configure git
    execSync('git config user.email "research@rnd-fleet.com"', {
      stdio: "pipe",
    });
    ...
    try {
      ...
      execSync('git commit -m "Initial commit - R&D Research Fleet v1.0.0"', {
        stdio: "pipe",
      });
    }
