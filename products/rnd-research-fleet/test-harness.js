...
async function extractPackage(pkg) {
  ...
  try {
    if (ext === ".gz" || ext === ".tgz" || pkg.endsWith(".tar.gz")) {
      ...
      await pipeline(createReadStream(pkg), unzip.Extract({ path: WORK_DIR }));
...
function analyzeContents() {
  ...
  const size = execSync(`du -sh "${WORK_DIR}"`, { encoding: "utf8" }).split(
    "\t",
  )[0];

  log.info(
    `Files: ${metrics.filesCount} | Directories: ${metrics.dirsCount} | Size: ${size}`,
  );
...
function runTests() {
  ...
  log.info(
    `Tests: ${metrics.testsPassed} passed, ${metrics.testsFailed} failed (${metrics.testTime}s)`,
  );
}
...
function runNodeTests() {
  ...
  // ESLint
  if (
    existsSync(join(WORK_DIR, ".eslintrc.js")) ||
    existsSync(join(WORK_DIR, ".eslintrc.json"))
  ) {
          ...
          if (stat.isDirectory()) {
            ...
            for (const pattern of patterns) {
              if (pattern.regex.test(content)) {
                log.warn(
                  `Possible ${pattern.name} in ${fullPath.replace(WORK_DIR, ".")}`,
                );
...
function attemptAutoFix() {
  ...
  try {
    // Fix permissions
    execSync('find . -type f -name "*.sh" -exec chmod +x {} \\;', {
      cwd: WORK_DIR,
    });
