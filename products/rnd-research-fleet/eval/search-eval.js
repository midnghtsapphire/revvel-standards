...
// ---------------------------------------------------------------------------
const THRESHOLDS = {
  rubricRegression: -0.1, // candidate rubric drop that forces a rollback
  errorRateRegression: 0.1, // candidate error-rate rise that forces a rollback
  citationRegression: -0.15, // candidate citation-coverage drop that forces rollback
  freshnessRegression: -0.2, // candidate freshness drop that forces rollback
...
function twinMetricsFor(run) {
  ...
  const twinResults = results.filter(
    (r) => r && typeof r.twin === "object" && r.twin !== null,
  );
  ...
  for (const r of twinResults) {
    ...
    if (typeof t.agreement_score === "number")
      agreement.push(t.agreement_score);
    if (typeof t.adjudication_quality === "number")
      adjudication.push(t.adjudication_quality);
  ...
  return {
    model_pair:
      pairList.length === 1 ? pairList[0] : pairList.length ? pairList : null,
...
function npletMetricsFor(run) {
  const results = Array.isArray(run.results) ? run.results : [];
  const npResults = results.filter(
    (r) => r && typeof r.nplet === "object" && r.nplet !== null,
  );
  ...
  for (const r of npResults) {
    ...
    if (typeof t.adjudication_quality === "number")
      adjudication.push(t.adjudication_quality);
    if (typeof t.consensus_score === "number")
      consensus.push(t.consensus_score);
    disagreementCount += countOf(t.disagreements);
    unsupportedFlags += countOf(t.unsupported_claims);
    ...
    if (
      typeof t.consensus_score !== "number" &&
      ...
      if (domains.length)
        consensus.push(
          domains.filter((d) => counts.get(d) >= k).length / domains.length,
        );
    }
  }
...
function scoreRun(run, fixtures) {
  ...
  for (const r of results) {
    ...
    const isError =
      r.error === true ||
      (!r.answer && (!r.citations || r.citations.length === 0));
    ...
    const minCites =
      typeof fx.min_citations === "number" ? fx.min_citations : 0;
    if (minCites > 0) {
    ...
    if (r.usefulness_note)
      usefulnessNotes.push({ query_id: r.query_id, note: r.usefulness_note });
  ...
  const rubricScores = perQuery
    .map((p) => p.rubric_score)
    .filter((x) => x !== null);
  ...
  const scored = {
    ...
    metrics: {
      ...
      citation_coverage: citationApplicable
        ? round(citationMet / citationApplicable)
        : null,
      domain_diversity: totalCitations
        ? round(uniqueDomains.size / totalCitations)
        : null,
      unique_domains: uniqueDomains.size,
      duplicate_rate: totalCitations
        ? round(duplicateCitations / totalCitations)
        : 0,
      freshness_satisfaction: freshnessRequired
        ? round(freshnessSatisfied / freshnessRequired)
        : null,
      mean_latency_ms: latencies.length ? round(mean(latencies), 1) : null,
      total_cost_usd: costs.length
        ? round(
...
function isFreshEnough(result, fixture) {
  ...
  const wanted =
    typeof fixture.fresh_after_year === "number" ? fixture.fresh_after_year : 0;
...
function decide(baseline, candidate, deltas) {
  ...
  if (deltas.rubric_mean !== null && deltas.rubric_mean <= T.rubricRegression) {
    regression = true;
    reasons.push(
      `Rubric regressed by ${deltas.rubric_mean} (<= ${T.rubricRegression}).`,
    );
  }
  if (
    deltas.error_rate !== null &&
    deltas.error_rate >= T.errorRateRegression
  ) {
    regression = true;
    reasons.push(
      `Error rate rose by ${deltas.error_rate} (>= ${T.errorRateRegression}).`,
    );
  }
  if (
    deltas.citation_coverage !== null &&
    deltas.citation_coverage <= T.citationRegression
  ) {
    regression = true;
    reasons.push(
      `Citation coverage dropped by ${deltas.citation_coverage} (<= ${T.citationRegression}).`,
    );
  }
  if (
    deltas.freshness_satisfaction !== null &&
    deltas.freshness_satisfaction <= T.freshnessRegression
  ) {
    regression = true;
    reasons.push(
      `Freshness dropped by ${deltas.freshness_satisfaction} (<= ${T.freshnessRegression}).`,
    );
  }
  ...
  const citationOk =
    deltas.citation_coverage === null ||
    deltas.citation_coverage >= T.citationKeepFloor;
  const dupOk =
    deltas.duplicate_rate === null ||
    deltas.duplicate_rate <= T.duplicateKeepCeiling;

  if (rubricOk && errorOk && citationOk && dupOk) {
    reasons.push(
      "Candidate matches or beats baseline on rubric, errors, citations, and duplicates.",
    );
  ...
  reasons.push(
    "Mixed results: no hard regression, but candidate is not a clean win. Tune the prompt/routing and re-run.",
  );
...
function compareTwinToFable(twinScored, fableScored, referenceLabel = "fable") {
  ...
  const deltas = {
    ...
    freshness_satisfaction: diff(
      t.freshness_satisfaction,
      f.freshness_satisfaction,
    ),
  ...
  const { decision, reasons } = decideTwin(
    twinScored,
    fableScored,
    deltas,
    referenceLabel,
...
function decideTwin(twin, fable, deltas, referenceLabel = "fable") {
  ...
  if (
    deltas.error_rate !== null &&
    deltas.error_rate >= THRESHOLDS.errorRateRegression
  ) {
    reasons.push(
      `Twin error rate rose by ${deltas.error_rate} vs ${referenceLabel} (>= ${THRESHOLDS.errorRateRegression}).`,
    );
    return { decision: "rollback_twin", reasons };
  }
  if (
    deltas.citation_coverage !== null &&
    deltas.citation_coverage <= THRESHOLDS.citationRegression
  ) {
    reasons.push(
      `Twin citation coverage dropped by ${deltas.citation_coverage} vs ${referenceLabel} (<= ${THRESHOLDS.citationRegression}).`,
    );
    return { decision: "rollback_twin", reasons };
  }

  const qualityGain =
    deltas.rubric_mean !== null && deltas.rubric_mean >= T.minQualityGain;
  const costOk =
    deltas.cost_ratio === null || deltas.cost_ratio <= T.maxCostRatio;
  const latencyOk =
    deltas.latency_ratio === null || deltas.latency_ratio <= T.maxLatencyRatio;
  const adjudicationOk =
    typeof tm.adjudication_quality !== "number" ||
    tm.adjudication_quality >= T.adjudicationFloor;
  const sourcesOk =
    typeof tm.source_overlap !== "number" ||
    tm.source_overlap <= T.maxSourceOverlap;
  const hallucinationsOk =
    countOf(tm.hallucination_or_unsupported_claim_flags) <=
    T.maxUnsupportedClaims;
  ...
  if (qualityGain && (!costOk || !latencyOk || !sourcesOk)) {
    if (!costOk)
      reasons.push(
        `Cost ${deltas.cost_ratio}x ${referenceLabel} exceeds budget ${T.maxCostRatio}x.`,
      );
    if (!latencyOk)
      reasons.push(
        `Latency ${deltas.latency_ratio}x ${referenceLabel} exceeds budget ${T.maxLatencyRatio}x.`,
      );
    if (!sourcesOk)
      reasons.push(
        `Source overlap ${tm.source_overlap} > ${T.maxSourceOverlap}: the two runs are largely redundant.`,
      );
  ...
  if (!qualityGain && deltas.rubric_mean !== null) {
    reasons.push(
      `Rubric gain ${deltas.rubric_mean} is positive but below keep threshold ${T.minQualityGain}.`,
    );
  }
  if (!adjudicationOk)
    reasons.push(
      `Adjudication quality ${tm.adjudication_quality} below floor ${T.adjudicationFloor}.`,
    );
  if (!hallucinationsOk) {
    reasons.push(
      `Unsupported-claim flags (${countOf(tm.hallucination_or_unsupported_claim_flags)}) exceed allowance ${T.maxUnsupportedClaims}.`,
    );
  }
  reasons.push(
    "No hard regression vs reference, but twin is not a clean win. Tune and re-run.",
  );
  return { decision: "tune_twin", reasons };
}
...
// ---------------------------------------------------------------------------
function compareNpletToReference(
  npletScored,
  refScored,
  referenceLabel = "twin",
) {
  ...
  const { decision, reasons } = decideNplet(
    npletScored,
    refScored,
    deltas,
    referenceLabel,
...
function decideNplet(nplet, ref, deltas, referenceLabel = "twin") {
  ...
  if (deltas.rubric_mean !== null && deltas.rubric_mean <= 0) {
    reasons.push(
      `Triplet rubric does not beat ${referenceLabel} (delta ${deltas.rubric_mean} <= 0); not worth ~Nx cost/latency.`,
    );
  ...
  if (
    ...
    reasons.push(
      `Triplet error rate rose by ${deltas.error_rate} vs ${referenceLabel} (>= ${THRESHOLDS.errorRateRegression}).`,
    );
    return { decision: "rollback_triplet", reasons };
  }
  if (
    ...
    reasons.push(
      `Triplet citation coverage dropped by ${deltas.citation_coverage} vs ${referenceLabel} (<= ${THRESHOLDS.citationRegression}).`,
    );
    return { decision: "rollback_triplet", reasons };
  }
  ...
  const latencyOk =
    deltas.latency_ratio === null || deltas.latency_ratio <= T.maxLatencyRatio;
  const adjudicationOk =
    typeof nm.adjudication_quality !== "number" ||
    nm.adjudication_quality >= T.adjudicationFloor;
  const consensusOk =
    typeof nm.consensus_score !== "number" ||
    nm.consensus_score >= T.minConsensus;
  const hallucinationsOk =
    countOf(nm.hallucination_or_unsupported_claim_flags) <=
    T.maxUnsupportedClaims;
  ...
  if (qualityGain && (!costOk || !latencyOk || !consensusOk)) {
    ...
    if (!consensusOk)
      reasons.push(
        `Consensus ${nm.consensus_score} < ${T.minConsensus}: the arms rarely corroborate the same sources.`,
      );
  ...
  if (!adjudicationOk)
    reasons.push(
      `Adjudication quality ${nm.adjudication_quality} below floor ${T.adjudicationFloor}.`,
    );
  if (!hallucinationsOk)
    reasons.push(
      `Unsupported-claim flags (${countOf(nm.hallucination_or_unsupported_claim_flags)}) exceed allowance ${T.maxUnsupportedClaims}.`,
    );
  reasons.push(
    "No hard regression vs reference, but triplet is not a clean win. Tune and re-run.",
  );
  return { decision: "tune_triplet", reasons };
}
...
function evaluateStrategies(fixtures, runs) {
  ...
  const twinKey = scored.twin_llm_adjudicated
    ? "twin_llm_adjudicated"
    : scored.twin_llm
      ? "twin_llm"
      : null;
  if (twinKey && scored.fable) {
    report.comparisons.twin_vs_fable = compareTwinToFable(
      scored[twinKey],
      scored.fable,
      "fable",
    );
  ...
  if (twinKey && scored.baseline) {
    report.comparisons.twin_vs_baseline = compareTwinToFable(
      scored[twinKey],
      scored.baseline,
      "baseline",
    );
  ...
  if (scored.candidate && scored.baseline) {
    report.comparisons.candidate_vs_baseline = compareRuns(
      scored.baseline,
      scored.candidate,
    );
  }
  ...
  if (!primary) {
    report.reasons = [
      "Not enough strategies provided to compare (need twin+fable or candidate+baseline).",
    ];
  } else {
...
function renderMarkdown(report) {
  ...
  const rows = [
    [
      "Rubric mean",
      b.metrics.rubric_mean,
      c.metrics.rubric_mean,
      d.rubric_mean,
    ],
    ["Error rate", b.metrics.error_rate, c.metrics.error_rate, d.error_rate],
    [
      "Citation coverage",
      b.metrics.citation_coverage,
      c.metrics.citation_coverage,
      d.citation_coverage,
    ],
    [
      "Domain diversity",
      b.metrics.domain_diversity,
      c.metrics.domain_diversity,
      d.domain_diversity,
    ],
    [
      "Unique domains",
      b.metrics.unique_domains,
      c.metrics.unique_domains,
      null,
    ],
    [
      "Duplicate rate",
      b.metrics.duplicate_rate,
      c.metrics.duplicate_rate,
      d.duplicate_rate,
    ],
    [
      "Freshness satisfaction",
      b.metrics.freshness_satisfaction,
      c.metrics.freshness_satisfaction,
      d.freshness_satisfaction,
    ],
    [
      "Mean latency (ms)",
      b.metrics.mean_latency_ms,
      c.metrics.mean_latency_ms,
      d.mean_latency_ms,
    ],
    [
      "Total cost (USD)",
      b.metrics.total_cost_usd,
  ...
  lines.push("");
  lines.push(
    `- Baseline: \`${fmt(b.search_prompt_version)}\` (profile \`${fmt(b.router_profile)}\`, ${b.result_count} queries)`,
  );
  lines.push(
    `- Candidate: \`${fmt(c.search_prompt_version)}\` (profile \`${fmt(c.router_profile)}\`, ${c.result_count} queries)`,
  );
  lines.push("");
  ...
  if (c.usefulness_notes && c.usefulness_notes.length) {
    ...
    for (const u of c.usefulness_notes)
      lines.push(`- **${u.query_id}**: ${u.note}`);
    lines.push("");
  }
  lines.push(
    "> Green CI does not prove search quality. This report measures it. See standards/SEARCH_EVALUATION.md.",
  );
  lines.push("");
  return lines.join("\n");
}
...
function renderTwinMarkdown(cmp) {
  ...
  const rows = [
    [
      "Rubric mean",
      t.metrics.rubric_mean,
      f.metrics.rubric_mean,
      d.rubric_mean,
    ],
    ["Error rate", t.metrics.error_rate, f.metrics.error_rate, d.error_rate],
    [
      "Citation coverage",
      t.metrics.citation_coverage,
      f.metrics.citation_coverage,
      d.citation_coverage,
    ],
    [
      "Duplicate rate",
      t.metrics.duplicate_rate,
      f.metrics.duplicate_rate,
      d.duplicate_rate,
    ],
    [
      "Freshness satisfaction",
      t.metrics.freshness_satisfaction,
      f.metrics.freshness_satisfaction,
      d.freshness_satisfaction,
    ],
    [
      "Mean latency (ms)",
      t.metrics.mean_latency_ms,
      f.metrics.mean_latency_ms,
      d.latency_delta_vs_fable,
    ],
    [
      "Total cost (USD)",
      t.metrics.total_cost_usd,
      f.metrics.total_cost_usd,
      d.cost_delta_vs_fable,
    ],
  ];
  for (const [name, tv, fv, dv] of rows)
    lines.push(`| ${name} | ${fmt(tv)} | ${fmt(fv)} | ${fmt(dv)} |`);
  lines.push("");
  ...
  lines.push("| --- | --- |");
  lines.push(
    `| Model pair | ${fmt(Array.isArray(tm.model_pair) ? tm.model_pair.join(", ") : tm.model_pair)} |`,
  );
  ...
  lines.push(
    `| Hallucination / unsupported-claim flags | ${fmt(tm.hallucination_or_unsupported_claim_flags)} |`,
  );
  lines.push(
    `| Cost delta vs ${cmp.reference} (USD) | ${fmt(tm.cost_delta_vs_fable)} |`,
  );
  lines.push(
    `| Latency delta vs ${cmp.reference} (ms) | ${fmt(tm.latency_delta_vs_fable)} |`,
  );
  lines.push(`| Cost ratio vs ${cmp.reference} | ${fmt(d.cost_ratio)}x |`);
  lines.push(
    `| Latency ratio vs ${cmp.reference} | ${fmt(d.latency_ratio)}x |`,
  );
  lines.push("");
  return lines.join("\n");
}
...
function renderStrategyMarkdown(report) {
  ...
  lines.push("");
  lines.push(
    `## Overall decision: \`${report.decision}\` (from \`${fmt(report.primary_comparison)}\`)`,
  );
  lines.push("");
  for (const r of report.reasons || []) lines.push(`- ${r}`);
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "| Strategy | Version | Rubric | Error | Citations | Dup | Latency (ms) | Cost (USD) |",
  );
  lines.push("| --- | --- | --- | --- | --- | --- | --- | --- |");
  ...
  lines.push("");
  if (report.comparisons.twin_vs_fable)
    lines.push(renderTwinMarkdown(report.comparisons.twin_vs_fable));
  if (report.comparisons.twin_vs_baseline)
    lines.push(renderTwinMarkdown(report.comparisons.twin_vs_baseline));
  if (report.comparisons.candidate_vs_baseline) {
    lines.push(renderMarkdown(report.comparisons.candidate_vs_baseline));
  }
  lines.push("");
  lines.push(
    "> Green CI is not proof of search quality. CI proves the measurement layer runs.",
  );
  lines.push(
    "> A twin-LLM strategy must beat Fable/baseline on this evaluation report before it",
  );
...
function main() {
  ...
  const strategyMode = !!(
    args.fable ||
    args.twin ||
    args["twin-adjudicated"] ||
    tripletArg
  );
  if (strategyMode) {
    const haveStrategy = args.twin || args["twin-adjudicated"] || tripletArg;
    const haveReference =
      args.fable || args.baseline || args.twin || args["twin-adjudicated"];
    ...
    if (args["twin-adjudicated"])
      runs.twin_llm_adjudicated = loadJson(args["twin-adjudicated"]);
