...
function buildCheckpoint(state) {
  return {
    ...
    arms: state.arms.map((a) => ({
      ...
      last_progress_at:
        typeof a.lastProgressAt === "number" ? a.lastProgressAt : null,
      has_result: !!a.result,
      error: a.error || null,
    })),
...
function buildHandoffDoc(state) {
  ...
  for (const a of state.arms) {
    const srcCount =
      a.result && Array.isArray(a.result.citations)
        ? a.result.citations.length
        : 0;
    lines.push(
      `| ${a.id} | ${a.model} | ${a.status} | ${(a.triedModels || []).join(" → ")} | ${srcCount} |`,
    );
  }
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "- Re-dispatch any arm still `running`/`stalled` (reassign to its next untried model).",
  );
  ...
  function launch(arm) {
    ...
    Promise.resolve()
      .then(() =>
        dispatch(armView, {
          onProgress: () => {
            if (arm.generation === gen) arm.lastProgressAt = now();
          },
...
if (require.main === module) {
  ...
  const live =
    process.env.ORCH_LIVE === "1" || process.env.ORCH_LIVE === "true";
  ...
  if (live) {
    ...
    const candidates = [
      {
        ...
        kind: "single",
        model: "deep_search",
        profileModels: ["deep_search", "technical", "market_research"],
      },
      {
        id: "arm-twin",
        kind: "twin",
        model: DEFAULT_LIVE_MODEL,
        profileModels: [DEFAULT_LIVE_MODEL],
      },
      {
        id: "arm-triplet",
        kind: "triplet",
    ...
    armSpecs = candidates.filter((s) => {
      ...
      if (!present)
        console.error(
          `[orchestrator] live: skipping ${s.id} (${s.kind}) — runner ${script} not present on this branch yet`,
        );
      return present;
    });
    if (!armSpecs.length) {
      console.error(
        "[orchestrator] live: no runner scripts present; nothing to dispatch.",
      );
      process.exit(1);
    }
  } else {
    armSpecs = [
      {
        id: "arm-1",
        model: "model-a",
        profileModels: ["model-a", "model-a-fallback"],
      },
      {
        id: "arm-2",
        model: "model-b",
        profileModels: ["model-b", "model-b-fallback"],
      },
      {
        id: "arm-3",
        model: "model-c",
        profileModels: ["model-c", "model-c-fallback"],
      },
    ];
  }
    ...
    : (arm) =>
        new Promise((resolve) =>
          setTimeout(
            () =>
              resolve({
                answer: `stub answer from ${arm.model}`,
                citations: [`https://example.com/${arm.id}`],
                cost_usd: 0,
              }),
            50,
          ),
