...
for (const id of expectedPhaseIds) {
  ...
  assert.ok(
    phase.cfr.startsWith("820.30"),
    `Phase '${id}' cfr must reference 820.30`,
  );
  assert.ok(
    phase.items.length > 0,
    `Phase '${id}' must have at least one checklist item`,
  );
}
...
assert.equal(phaseCompletionPct(testPhase, new Set()), 0, "empty set → 0%");
...
if (testPhase.items.length >= 2) {
  const half = new Set(
    testPhase.items
      .slice(0, Math.floor(testPhase.items.length / 2))
      .map((i) => i.id),
  );
...
assert.ok(
  mdEmpty.includes("**Version:** 2.1"),
  "markdown must include version",
);
assert.ok(
  mdEmpty.includes("**Device Class:** Class II"),
  "markdown must include device class",
);
assert.ok(mdEmpty.includes("21 CFR 820.30"), "markdown must reference CFR");
assert.ok(
  mdEmpty.includes("820.30(a)"),
  "markdown must include planning phase cfr",
);
...
assert.ok(
  mdFull.includes("[x]"),
  "full checklist markdown must contain [x] items",
);
assert.ok(
...
assert.ok(csvLines[0].includes('"Phase"'), "CSV must start with Phase header");
assert.ok(csvLines[0].includes('"CFR"'), "CSV must include CFR column");
assert.ok(
  csvLines[0].includes('"Required"'),
  "CSV must include Required column",
);
assert.ok(
  csvLines[0].includes('"Completed"'),
  "CSV must include Completed column",
);
...
for (const row of dataRows) {
  assert.ok(
    row.includes('"No"'),
    `Every data row in empty map must show Completed="No"`,
  );
}
...
const fullDataRows = fullChecklistSection
  .split("\n")
  .slice(1)
  .filter((l) => l.trim() !== "");
for (const row of fullDataRows) {
  assert.ok(
    row.includes('"Yes"'),
    `Every data row in full map must show Completed="Yes"`,
  );
}

// Project metadata footer must be present in CSV
assert.ok(
  csvEmpty.includes('"CardioMonitor 3000"'),
  "CSV must include device name in footer",
);
assert.ok(
  csvEmpty.includes('"Class II"'),
  "CSV must include device class in footer",
);

// ─────────────────────────────────────────────────────────────────────────────
console.log(
  `✅ All FDA Design Controls tests passed (${total} items across ${DESIGN_PHASES.length} phases)`,
);
