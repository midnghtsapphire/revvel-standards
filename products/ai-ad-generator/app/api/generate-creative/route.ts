...
import type { GenerateCreativeResponse, AdTemplateId } from "../../../types";
...
export async function POST(
  req: NextRequest,
): Promise<NextResponse<GenerateCreativeResponse>> {
  let body: {
    templateId?: unknown;
  ...
  if (
    ...
    return NextResponse.json(
      {
        success: false,
        error: "productTitle is required and must be a non-empty string",
      },
      { status: 400 },
    );
  }
  ...
  if (!cta || typeof cta !== "string" || cta.trim() === "") {
    return NextResponse.json(
      {
        success: false,
        error: "cta is required and must be a non-empty string",
      },
      { status: 400 },
    );
  }
  ...
  if (
    ...
    !AD_TEMPLATES[templateId as AdTemplateId]
  ) {
    return NextResponse.json(
  ...
  if (imageUrl !== undefined) {
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: "imageUrl must be a valid URL" },
        { status: 400 },
      );
    }
  }
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: "Creative render failed. Please try again." },
      { status: 500 },
    );
  }
}
