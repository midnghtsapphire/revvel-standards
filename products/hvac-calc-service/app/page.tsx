...
const REFRIGERANT_STATUS_ORDER = [
  "low-gwp",
  "transitional",
  "phased-out",
] as const;
const REFRIGERANTS_BY_STATUS = [...REFRIGERANTS].sort(
  (a, b) =>
    REFRIGERANT_STATUS_ORDER.indexOf(a.status) -
    REFRIGERANT_STATUS_ORDER.indexOf(b.status),
);
...
export default function HomePage() {
  ...
  const equipment = useMemo(
    () => recommendEquipment(load.sensibleCoolingLoad),
    [load],
  );
  const duct = useMemo(
    () => calculateDuctSize({ coolingLoadBtu: load.sensibleCoolingLoad }),
    [load],
  );
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <header className="mb-8">
          ...
          <p className="text-gray-400 mt-2">
            Manual J (simplified) cooling &amp; heating loads · Equipment sizing
            (Manual S) · Duct sizing (Manual D) · Annual energy cost estimates
          </p>
        </header>
          ...
          <div className="lg:col-span-1 space-y-5 bg-gray-900 rounded-xl p-5">
            ...
            <InputRow label="Window Fraction (% of floor)">
              <input
                ...
                onChange={(e) =>
                  setWindowFraction(Number(e.target.value) / 100)
                }
                className="w-full bg-gray-800 rounded px-3 py-2 text-white"
              />
            </InputRow>
            ...
            <InputRow label={`Cooling Setpoint (°F)`}>
              <input
                ...
                onChange={(e) =>
                  setIndoorCoolingSetpoint(Number(e.target.value))
                }
                className="w-full bg-gray-800 rounded px-3 py-2 text-white"
              />
            </InputRow>
            ...
            <InputRow label={`Heating Setpoint (°F)`}>
              <input
                ...
                onChange={(e) =>
                  setIndoorHeatingSetpoint(Number(e.target.value))
                }
                className="w-full bg-gray-800 rounded px-3 py-2 text-white"
              />
            </InputRow>
            ...
            <InputRow
              label={isFurnace ? "Furnace AFUE (e.g. 0.95)" : "HSPF2 Rating"}
            >
              <input
                type="number"
                min={0.5}
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-gray-900 rounded-xl overflow-hidden">
              <div className="flex border-b border-gray-700">
                {(["load", "energy", "duct", "refrigerant"] as const).map(
                  (tab) => (
                    <button
                      key={tab}
                    ...
                    >
                      {tab === "duct"
                        ...
                        : tab === "refrigerant"
                          ? "Refrigerants"
                          : `${tab.charAt(0).toUpperCase() + tab.slice(1)} Detail`}
                    </button>
                  ),
                )}
              </div>
              ...
              <div className="p-5">
                {activeTab === "load" && (
                  <div className="space-y-4">
                    ...
                    <BreakdownTable
                      entries={Object.entries(load.coolingBreakdown)}
                      total={load.sensibleCoolingLoad}
                    />
                    <h3 className="font-semibold text-white mt-4">
                      Heating Load Breakdown
                    </h3>
                    <BreakdownTable
                      entries={Object.entries(load.heatingBreakdown)}
                      total={load.sensibleHeatingLoad}
                    />
                    <div
                      className={`mt-4 p-3 rounded-lg text-sm ${
                        equipment.verdict === "optimal"
                          ? "bg-emerald-950 text-emerald-300"
                          : equipment.verdict === "slightly-oversized"
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        <DataRow
                          ...
                          value={`${energy.annualCoolingKwh.toLocaleString()} kWh`}
                        />
                        <DataRow
                          label="Cooling Cost"
                          value={`$${energy.annualCoolingCost.toFixed(0)}`}
                        />
                        <DataRow
                          label="Annual Heating"
                          value={`${energy.annualHeatingKwh.toLocaleString()} kWh eq`}
                        />
                        <DataRow
                          label="Heating Cost"
                          value={`$${energy.annualHeatingCost.toFixed(0)}`}
                        />
                        <DataRow
                          label="Total Annual Cost"
                          value={
                            <strong>
                              ${energy.totalAnnualCost.toFixed(0)}
                            </strong>
                          }
                        />
                        <DataRow
                          label="CO₂ Equivalent"
                          value={`${energy.co2EquivalentKg.toFixed(0)} kg`}
                        />
                      </tbody>
                    </table>
                    <p className="text-xs text-gray-500 mt-2">
                      Based on ASHRAE climate zone design data. Actual costs
                      vary with usage patterns, utility rates, and equipment
                      age.
                    </p>
                  </div>
                )}
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        <DataRow
                          ...
                          value={`${duct.requiredCfm.toLocaleString()} CFM`}
                        />
                        <DataRow
                          label="Duct Area"
                          value={`${duct.ductAreaSqIn.toFixed(1)} sq in`}
                        />
                        <DataRow
                          label="Round Duct Diameter"
                          value={`${duct.roundDuctDiameter.toFixed(1)}"`}
                        />
                        <DataRow
                          label="Rectangular Duct"
                          value={`${duct.rectangularDuct.width}" × ${duct.rectangularDuct.height}"`}
                        />
                      </tbody>
                    </table>
                    <p className="text-xs text-gray-500 mt-2">
                      Sized for main trunk at 800 FPM supply velocity, 20°F
                      supply-return ΔT. Branch ducts require separate Manual D
                      calculations.
                    </p>
                  </div>
                )}
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            GWP: {r.gwp.toLocaleString()} · {r.notes}
                          </p>
                        </div>
                      </div>
                    ))}
            ...
            <p className="text-xs text-gray-600">
              Simplified Manual J estimates for preliminary sizing only. Have a
              licensed engineer perform a full Manual J/D/S before purchasing
              equipment.
            </p>
          </div>
        </div>
