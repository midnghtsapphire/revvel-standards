...
export default function Page() {
  ...
  useEffect(() => {
    const stored =
      typeof window !== "undefined"
        ? window.localStorage.getItem("pf-mode")
        : null;
  ...
  async function copyMarkdown() {
    try {
      ...
      setCopied(false);
      setCopyError(
        "Clipboard access is unavailable. Copy the markdown manually below.",
      );
    }
  }
  ...
  return (
    <main className={`mode-${mode} mx-auto min-h-screen max-w-5xl p-6`}>
      ...
      <section
        className="focus-keep mb-4 flex flex-wrap gap-2"
      ...
      {packet ? (
        <section className="focus-keep grid gap-4">
          ...
          {copyError ? (
            <p className="text-sm text-amber-200">{copyError}</p>
          ) : null}
