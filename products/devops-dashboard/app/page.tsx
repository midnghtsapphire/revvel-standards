...
export default function DevOpsDashboard() {
  ...
  const REPO =
    process.env.NEXT_PUBLIC_GITHUB_REPO ?? "midnghtsapphire/revvel-standards";
    ...
    <div style={S.page}>
      ...
      <div style={S.section}>
        ...
        {state.selfHealPRs.length === 0 ? (
          <p style={S.empty}>
            {state.loading
              ? "Loading…"
              : "No open self-healing PRs — system is clean ✅"}
          </p>
        ) : (
            ...
            <div key={pr.number} style={S.card}>
              <div style={S.row}>
                <div>
                  ...
                  {pr.draft && (
                    <span style={{ ...S.badge("#21262d"), marginLeft: 8 }}>
                      Draft
                    </span>
                  )}
                </div>
      ...
      <div style={S.section}>
        ...
        {state.recentRuns.length === 0 ? (
          <p style={S.empty}>
            {state.loading ? "Loading…" : "No recent runs found."}
          </p>
        ) : (
          state.recentRuns.map((run) => (
            <div key={run.id} style={S.card}>
              ...
              <div style={{ ...S.meta, marginTop: "0.25rem" }}>
                Branch:{" "}
                <code style={{ color: "#79c0ff" }}>{run.head_branch}</code>
      ...
      <div style={S.section}>
        ...
        <div style={S.card}>
          <p style={{ marginBottom: "0.75rem" }}>
            ...
            <code style={{ color: "#79c0ff" }}>.env.local</code> file to connect
            to your GitHub repo:
          </p>
          <pre
            style={{
...
NEXT_PUBLIC_GITHUB_TOKEN=ghp_...   # optional — increases rate limit`}
          ...
          <p style={{ ...S.meta, marginTop: "0.75rem" }}>
            Deploy to Vercel and add these as environment variables in the
            project settings. The dashboard auto-refreshes every 60 seconds.
          </p>
        </div>
      </div>
