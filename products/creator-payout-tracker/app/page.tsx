...
export default function HomePage() {
  ...
  const estimates = useMemo(
    () => estimatePlatforms(PLATFORMS, metrics),
    [metrics],
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (activeTab === "rpm")
      list = list.filter((p) => p.rpmMin !== null || p.rpmMax !== null);
    if (activeTab === "subscription")
      list = list.filter((p) => p.subPayoutPer5 !== null);
    ...
    <main className="min-h-screen bg-gray-950">
      ...
      <section className="max-w-6xl mx-auto px-4 pt-12 pb-8 text-center">
        ...
        <p className="text-gray-400 text-lg max-w-2xl mx-auto text-balance">
          Real 2025 payout rates for YouTube, TikTok, Patreon, Substack, Kick,
          Twitch, OnlyFans, Spotify & more. Calculate your estimated monthly
          earnings across every platform.
        </p>
        <div className="flex flex-wrap justify-center gap-2 mt-6 text-xs text-gray-500">
          ...
          <span className="bg-gray-800 px-3 py-1 rounded-full">
            ⚠️ Data are estimates — see disclaimer below
          </span>
        </div>
      ...
      <section id="rankings" className="max-w-6xl mx-auto px-4 pb-12">
        ...
        <div className="overflow-x-auto rounded-xl border border-gray-800">
          <table className="w-full text-sm">
            <thead className="bg-gray-900 border-b border-gray-800">
              <tr>
                <th className="text-left px-4 py-3 w-8 text-gray-500 text-xs">
                  Rank
                </th>
                <th className="text-left px-4 py-3">
                  <SortBtn k="name" label="Platform" />
                </th>
                <th className="text-left px-4 py-3 hidden sm:table-cell text-gray-400 text-xs font-semibold uppercase tracking-wider">
                  Category
                </th>
                {activeTab === "rpm" ? (
                  <>
                ...
                <th className="text-left px-4 py-3 hidden md:table-cell text-gray-400 text-xs font-semibold uppercase tracking-wider">
                  Paid
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-800/60">
              {filtered.map((platform, idx) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500 hidden sm:block">
                          {platform.bestFor.slice(0, 2).join(", ")}
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  {activeTab === "rpm" ? (
                    <>
                      <td className="px-4 py-3">
                        {platform.rpmMin !== null &&
                        platform.rpmMax !== null ? (
                    ...
                    <>
                      <td className="px-4 py-3">
                        {platform.subPayoutPer5 !== null ? (
                          <span className="font-semibold text-emerald-400">
                            ${platform.subPayoutPer5.toFixed(2)}
                          </span>
                        ) : (
                          <span className="text-gray-600">—</span>
                        )}
              ...
              {filtered.length === 0 && (
                <tr>
                  ...
                  >
                    No platforms match this filter + tab combination. Try
                    &quot;All Platforms&quot; or switch tabs.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <p className="text-xs text-gray-600 mt-2">
          * Estimated monthly earnings based on your inputs in the calculator
          below. RPM and payout figures are community-sourced estimates (Q2
          2025). Actual earnings vary by niche, audience geography, and content
          type.
        </p>
      </section>
      ...
      <section id="calculator" className="max-w-6xl mx-auto px-4 pb-16">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          ...
          <p className="text-gray-400 text-sm mb-6">
            Enter your metrics below. The table above will update with estimated
            monthly earnings for each platform.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-600 mt-1">
                Used for subscription tab estimates
              </p>
            </div>
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-600 mt-1">
                Your subscription price per subscriber
              </p>
            </div>
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                {
                  label: "Micro-creator (10K views, 20 subs)",
                  views: 10000,
                  subs: 20,
                  price: 5,
                },
                {
                  label: "Growing (100K views, 100 subs)",
                  views: 100000,
                  subs: 100,
                  price: 5,
                },
                {
                  label: "Mid-tier (500K views, 500 subs)",
                  views: 500000,
                  subs: 500,
                  price: 9,
                },
                {
                  label: "Pro creator (2M views, 2K subs)",
                  views: 2000000,
                  subs: 2000,
                  price: 10,
                },
                ...
                <button
                  key={s.label}
                  onClick={() => {
                    setMonthlyViews(s.views);
                    setMonthlySubscribers(s.subs);
                    setSubPrice(s.price);
                  }}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-xs text-gray-500 capitalize">
                    {p.category} · {p.paymentCadence} payouts
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                ...
                {p.platformCutPct !== null && (
                  <span
                    className={`px-2 py-0.5 rounded border ${p.platformCutPct <= 10 ? "bg-emerald-950 text-emerald-400 border-emerald-900" : p.platformCutPct <= 20 ? "bg-yellow-950 text-yellow-400 border-yellow-900" : "bg-red-950 text-red-400 border-red-900"}`}
                  >
      ...
      <section id="about" className="max-w-6xl mx-auto px-4 pb-16">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          ...
          <div className="text-sm text-gray-400 space-y-3">
            <p>
              <strong className="text-gray-200">Creator Payout Tracker</strong>{" "}
              is a free, creator-first reference tool that aggregates publicly
              available and community-reported payout rates across major creator
              platforms. It is not affiliated with any platform listed.
            </p>
            <p>
              <strong className="text-gray-200">Data sources:</strong> Payout
              figures are derived from platform documentation, creator community
              reports (Reddit r/NewTubers, r/Twitch, r/CreatorEconomy), industry
              publications (Influencer Marketing Hub, MilX, LiveWire Weekly,
              Fundmates), and aggregated creator surveys. Data is estimated and
              reflects Q2 2025 conditions.
            </p>
            <p>
              <strong className="text-gray-200">Disclaimer:</strong> Earnings
              estimates are illustrative only. Actual payout rates vary
              significantly by content niche, audience geography, engagement
              rate, and individual platform tier. This tool does not guarantee
              any specific level of earnings. Always verify current rates
              directly with each platform before making business decisions.
            </p>
            <p>
              <strong className="text-gray-200">Data refresh:</strong> Platform
              payout data is reviewed quarterly. Last updated: Q2 2025.
            </p>
          </div>
          <p className="text-xs text-gray-600 mt-4">
            Built by{" "}
            <a
              href="https://github.com/midnghtsapphire/revvel-standards"
              className="text-violet-500 hover:underline"
            >
              Revvel Standards
            </a>{" "}
            · Issue #13641 · {new Date().getFullYear()}
          </p>
        </div>
      </section>

      <footer className="border-t border-gray-800 text-center py-6 text-xs text-gray-600">
        Creator Payout Tracker · Data estimates for informational purposes only
