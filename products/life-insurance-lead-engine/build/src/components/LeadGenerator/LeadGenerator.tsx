...
export default function LeadGenerator() {
  ...
  const tierCount = (t: "A" | "B" | "C") =>
    leads.filter((l) => l.tier === t).length;

  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h2 className="text-2xl font-semibold mb-4">🎯 NPI Lead Generator</h2>
      <p className="text-sm text-slate-600 mb-4">
        Enter ZIP codes (comma-separated) to pull medical professionals from the
        public NPPES Registry. Leads are auto-scored: <strong>Tier A</strong>{" "}
        (surgeons, specialists) → <strong>Tier C</strong> (general practice).
      </p>
      ...
      {leads.length > 0 && (
        <div>
          <div className="flex gap-4 mb-4 text-sm">
            ...
            <span className="bg-slate-100 text-slate-800 px-3 py-1 rounded">
              Tier C: {tierCount("C")}
            </span>
            <button
              onClick={download}
              className="ml-auto bg-emerald-600 text-white px-3 py-1 rounded hover:bg-emerald-700"
            >
          ...
          <div className="overflow-x-auto">
              ...
              <tbody>
                {leads.slice(0, 10).map((l) => (
                  <tr key={l.npi} className="border-t">
                    ...
                    <td className="p-2">
                      {l.firstName} {l.lastName} {l.credential}
                    </td>
            ...
            {leads.length > 10 && (
              <p className="text-xs text-slate-500 mt-2">
                Showing top 10 of {leads.length}. Download CSV for all leads +
                full pitch scripts.
              </p>
            )}
          </div>
          ...
          {leads[0] && (
            <details className="mt-4 bg-slate-50 p-4 rounded">
              <summary className="cursor-pointer font-semibold">
                Sample Pitch Script (Tier {leads[0].tier})
              </summary>
              <p className="mt-2 text-sm italic">
                {generatePitchScript(leads[0])}
              </p>
            </details>
          )}
        </div>
