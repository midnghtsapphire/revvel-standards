...
const NEWSLETTER_SUBSCRIBERS_KEY =
  "life-insurance-lead-engine:newsletter-subscribers";
const NEWSLETTER_OPT_OUTS_KEY =
  "life-insurance-lead-engine:newsletter-opt-outs";
...
function writeStoredEmails(key: string, emails: string[]): boolean {
  try {
    window.localStorage.setItem(
      key,
      JSON.stringify(Array.from(new Set(emails)).sort()),
    );
    return true;
  } catch {
    return false;
...
export default function Newsletter() {
  ...
  const [optOuts, setOptOuts] = useState<string[]>(() =>
    readStoredEmails(NEWSLETTER_OPT_OUTS_KEY),
  );
  const [message, setMessage] = useState<string | null>(null);

  const normalizedEmail = useMemo(() => normalizeEmail(email), [email]);
  const isSuppressed =
    normalizedEmail.length > 0 && optOuts.includes(normalizedEmail);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === "optOut") {
      ...
      const nextOptOuts = Array.from(
        new Set([...optOuts, normalizedEmail]),
      ).sort();
      ...
      const savedOptOut = writeStoredEmails(
        NEWSLETTER_OPT_OUTS_KEY,
        nextOptOuts,
      );
      if (!removedSubscriber || !savedOptOut) {
        setMessage(
          "Opt-out could not be saved because browser storage is unavailable.",
        );
        return;
      }
      setOptOuts(nextOptOuts);
      setMessage(
        "Opt-out saved. This email is blocked from newsletter capture in this browser.",
      );
      return;
    }
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow border">
      ...
      <p className="text-sm text-slate-600 mb-4">
        Get one battle-tested prospecting script + one closing tactic every
        Monday. Free.
      </p>

      <div
        className="mb-4 flex rounded border border-slate-200 p-1 text-sm"
        aria-label="Newsletter preference mode"
      >
        <button
          type="button"
          onClick={() => {
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          ...
          <input
            ...
            aria-describedby={
              message
                ? "newsletter-privacy newsletter-status"
                : "newsletter-privacy"
            }
      ...
      {isSuppressed && mode === "subscribe" && (
        <p className="mt-3 text-sm text-amber-700">
          This address is on the local opt-out list, so newsletter capture is
          disabled.
        </p>
      )}

      <p id="newsletter-privacy" className="mt-3 text-xs text-slate-500">
        Opt-out requests are stored in this browser and suppress this app&apos;s
        local newsletter capture for the submitted email.
      </p>
    </div>
  );
}
