...
const HIGH_VALUE_SPECIALTIES: Record<
  string,
  { tier: "A" | "B" | "C"; angle: string }
> = {
  Surgery: {
    tier: "A",
    angle: "disability income protection + IUL for tax-advantaged growth",
  },
  "Orthopaedic Surgery": {
    tier: "A",
    angle: "high disability risk — own-occupation DI + whole life",
  },
  "Plastic Surgery": {
    tier: "A",
    angle: "business succession + estate planning life insurance",
  },
  Anesthesiology: {
    tier: "A",
    angle: "disability income + IUL retirement supplement",
  },
  Cardiology: {
    tier: "A",
    angle: "estate planning + permanent life insurance",
  },
  Radiology: { tier: "A", angle: "asset protection + IUL" },
  Dermatology: {
    tier: "A",
    angle: "practice continuation + buy-sell life insurance",
  },
  Dentist: {
    tier: "B",
    angle: "practice owner buy-sell + key-person life insurance",
  },
...
function scoreProvider(taxonomyDesc: string): {
  tier: "A" | "B" | "C";
  angle: string;
} {
...
export async function fetchNPIByZip(
  zip: string,
  limit = 50,
): Promise<NPIProvider[]> {
...
export function generatePitchScript(p: NPIProvider): string {
  return (
    `Hi Dr. ${p.lastName}, this is [AGENT NAME] from [AGENCY]. ` +
