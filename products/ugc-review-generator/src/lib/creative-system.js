...
function buildAmazonVisualPrompt({ productName, hookType }) {
  const safeProductName = cleanText(
    productName,
    "Amazon tech gadget or lifestyle product",
  );
...
function buildAmazonScript({ productName, problem, affiliateLink }) {
  const safeProblem = cleanText(
    problem,
    "messy cables destroying your desk setup",
  );
...
function buildAmazonReviewPacket({
  productName,
  problem,
  hookType = "unboxing",
  affiliateLink,
...
function buildOverlayLines({
  ...
  const normalizedTrial = Number.isNaN(parsedTrial)
    ? 7
    : Math.min(30, Math.max(1, parsedTrial));
...
function buildLeadHooks({ audience, businessType, proofMetric, brandName }) {
  const safeAudience = cleanText(audience, "Real estate agents");
  const safeBusinessType = cleanText(businessType, "real estate");
  const safeProofMetric = cleanText(
    proofMetric,
...
function buildComplianceNotes({ proofMetric, brandName }) {
  ...
  if (!cleanText(proofMetric)) {
    notes.unshift(
      `No proof metric was provided, so ${safeBrandName} defaults to qualitative social-proof language.`,
    );
  }
  return notes;
}
...
function packetToMarkdown(packet) {
  ...
  return [
    ...
    ...packet.contentCalendar.map(
      (entry) =>
        `| ${entry.day} | ${entry.format} | ${entry.angle} | ${entry.hook} | ${entry.cta} |`,
    ),
    "",
    "## Compliance Notes",
