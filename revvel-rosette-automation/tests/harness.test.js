...
test("pyproject.toml is valid", () => {
  ...
  assert(
    content.includes('name = "revvel-rosette-automation"'),
    "wrong project name",
  );
  assert(content.includes('version = "1.0.0"'), "wrong version");
  assert(
    /^dependencies\s*=\s*\[/m.test(content),
    "no dependencies array under [project]",
  );
  assert(
...
test("orchestrator can load config", () => {
  ...
  const result = spawnSync("python3", ["src/orchestrator.py", "--status"], {
    encoding: "utf8",
    cwd: PROJECT_ROOT,
  });
  ...
  try {
    ...
    assert(
      status.timestamp !== "2026-05-02T02:30:00Z",
...
test("scheduler can list jobs", () => {
  ...
  const result = spawnSync("python3", ["src/scheduler.py", "--list"], {
    encoding: "utf8",
    cwd: PROJECT_ROOT,
  });
...
test("gatekeeper health check works", () => {
  ...
  const result = spawnSync("python3", ["src/gatekeeper.py", "--health-check"], {
    encoding: "utf8",
    cwd: PROJECT_ROOT,
  });
...
test("parent gatekeeper-sync.sh is accessible", () => {
  const parentScript = path.resolve(
    PROJECT_ROOT,
    "..",
    "scripts",
    "gatekeeper-sync.sh",
