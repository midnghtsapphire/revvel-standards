...
async function generateCompleteProposal(params) {
  ...
  console.log(
    `   Total words: ${proposal.metadata.totalWordCount.toLocaleString()}`,
  );
  console.log(
    `   Total time: ${proposal.metadata.totalGenerationTime.toFixed(2)}s`,
  );
...
if (require.main === module) {
  const opportunity = {
    ...
    requirements: {
      need_statement:
        "Describe the educational gap your program will address, supported by local data.",
    },
    evaluationCriteria: {
      need_statement:
        "Clarity of need (30%), use of data (30%), alignment with funder priorities (40%)",
    },
    wordLimits: {
      need_statement: 500,
    },
  ...
  const organization = {
    ...
    mission:
      "Providing technology education and resources to underserved communities",
    type: "nonprofit",
    founded: 2015,
    annualBudget: 750000,
    staffCount: 12,
    trackRecord:
      "Served 2,500+ students with 95% completion rate and 80% job placement rate",
  };

  const project =
    "A 12-month after-school STEM program for 100 middle school students in underserved neighborhoods";
