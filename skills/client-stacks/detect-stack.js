  ...
  const walk = (dir, level) => {
    ...
    for (const e of entries) {
      ...
      if (
        ...
        (e.name === "node_modules" ||
          e.name === "vendor" ||
          (e.name.startsWith(".") && e.name !== ".github"))
      )
        continue;
...
function detectCi(root) {
  ...
  if (exists(root, "azure-pipelines.yml", "azure-pipelines.yaml"))
...
function loadLane(stack) {
  try {
    return JSON.parse(
      fs.readFileSync(path.join(STACKS_DIR, `${stack}.json`), "utf8"),
    );
  } catch {
    return null;
  }
...
function detect(root) {
  ...
  if (csproj.length)
    hit("dotnet", 3, `.csproj/.sln present (${path.basename(csproj[0])})`);
  if (
    exists(
      root,
      "global.json",
  ...
  if (findByExt(root, [".java", ".kt"]).length)
    hit("java", 1, "Java/Kotlin sources");

  // --- Python ---
  if (exists(root, "pyproject.toml", "setup.py", "setup.cfg"))
    hit("python", 3, "pyproject.toml / setup.py");
  if (exists(root, "requirements.txt", "Pipfile", "poetry.lock", "uv.lock"))
    hit("python", 2, "requirements/Pipfile/lockfile");
  ...
  if (findByExt(root, [".ts", ".tsx"]).length)
    hit("typescript", 1, "TS sources");
  ...
  if (exists(root, "config.ru", "Rakefile"))
    hit("ruby", 1, "config.ru / Rakefile");
  ...
  return {
    ...
    lane: lane
      ? {
          file: `skills/client-stacks/stacks/${detected}.json`,
          verify: lane.verify,
          prompt_pack: lane.prompt_pack,
        }
      : null,
  };
}
...
function main() {
  ...
  console.log(
    `Detected stack:  ${result.stack} (${result.language}, score ${result.score})`,
  );
