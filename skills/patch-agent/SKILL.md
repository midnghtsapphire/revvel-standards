...
**Recommendation: a repo-native agent**, because patching is a _code_ task that needs
git, the lockfile, and CI — all of which GitHub Actions already provides deterministically
and for free. n8n/gumloop would each need a code runner _anyway_, while adding an external
...
3. With `--fix`, raises only the _declared range floor_ to the minimal patched version
...
`fix: true` to have it open a PR. It is intentionally _not_ auto-triggered — this repo's
