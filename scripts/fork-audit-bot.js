...
function buildAuditBody({ cand, repo, score }) {
  ...
  return [
    ...
    '---',
    '',
    '_Generated by `scripts/fork-audit-bot.js`. This issue carries the ' +
      'required routing labels so `.github/workflows/openrouter-assignee.yml` ' +
      'will assign `@Copilot` and hand it to the OpenRouter orchestrator ' +
...
async function processCandidate(token, cand, defaults) {
  ...
  try {
    const mirror = await createMirrorIssue(token, SELF_REPO, {
      title,
      body,
      labels: Array.from(ROUTING_LABELS).concat(['upstream-contribution']),
    });
