...
const MAX_FILES_TO_FETCH = parseInt(
  process.env.MAX_FILES_TO_FETCH || "100",
  10,
);
...
async function getPRFiles() {
  ...
  while (hasMore && allFiles.length < MAX_FILES_TO_FETCH) {
    ...
    if (!files || files.length === 0) {
      ...
      if (
        allFiles.length >= MAX_FILES_TO_FETCH ||
        files.length < GITHUB_PAGE_SIZE
      ) {
        hasMore = false;
      } else {
        page++;
...
async function callOpenRouter(systemPrompt, userPrompt) {
  ...
  await new Promise((resolve) =>
    setTimeout(resolve, OPENROUTER_RATE_LIMIT_DELAY),
  );
...
function parseReviewResponse(response) {
  try {
    ...
    if (
      ...
      console.warn(
        `Invalid overall_assessment: ${parsed.overall_assessment}. Defaulting to COMMENT.`,
      );
    ...
    const validInlineComments = (parsed.inline_comments || [])
      .filter((c) => {
        if (!c.file || !c.line || !c.comment) {
          console.warn(
            `Skipping invalid inline comment - missing required fields:`,
            c,
          );
          return false;
        }
        if (typeof c.line !== "number" || c.line <= 0) {
          console.warn(
            `Skipping invalid inline comment - invalid line number:`,
            c,
          );
          return false;
        }
    ...
    const fallbackSummary =
      "Automated code review completed, but response format was unexpected. Review the full feedback below.";
...
async function main() {
  ...
  if (PR_HEAD_REPO && PR_HEAD_REPO !== GITHUB_REPOSITORY) {
    console.log(
      `::warning::PR is from fork (${PR_HEAD_REPO}). Skipping automated review for security.`,
    );
    process.exit(0);
  }
  ...
  console.log(
    `Configuration: MAX_DIFF_SIZE=${MAX_DIFF_SIZE}, MAX_FILES=${MAX_FILES_TO_FETCH}`,
  );
  ...
  try {
    ...
    if (files.length >= MAX_FILES_TO_FETCH) {
      console.warn(
        `Warning: PR has at least ${MAX_FILES_TO_FETCH} files. Review may be incomplete.`,
      );
    }
    ...
    const reviewBodyParts = ["## 🔍 Automated Code Review", "", review.summary];
    ...
    if (files.length >= MAX_FILES_TO_FETCH) {
      reviewBodyParts.push("");
      reviewBodyParts.push(
        `⚠️ **Note**: This PR changes ${files.length}+ files. Review may not cover all files.`,
      );
    }
    ...
    reviewBodyParts.push("---");
    reviewBodyParts.push(
      `_Automated review generated by ${MODEL} via OpenRouter_`,
    );
    ...
    console.error(
      `::error::Failed to perform automated review: ${error.message}`,
    );
