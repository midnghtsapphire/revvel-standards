...
function stuckAgeVerdict(
  createdAt,
  { now = Date.now(), minStuckAgeMs = DEFAULT_MIN_STUCK_AGE_MS, maxAgeMs } = {},
) {
...
async function findAssociatedPr({
  ...
  for (const prNumber of [...new Set(mentionedPrNumbers)]) {
    try {
      ...
      if (core.notice) {
        core.notice(
          `Could not inspect PR #${prNumber} mentioned on #${issue.number}: ${error.message}`,
        );
      }
    }
  }
