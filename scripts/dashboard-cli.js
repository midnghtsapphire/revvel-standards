...
function loadData() {
  if (!fs.existsSync(DATA_FILE)) {
    console.log(
      `${colors.yellow}⚠️  Dashboard data not found. Generating...${colors.reset}`,
    );
...
function printProject(project, index) {
  ...
  console.log(
    `   Status: ${statusColor}${project.status || "Unknown"}${colors.reset}`,
  );
  if (project.description)
    console.log(`   Description: ${project.description.substring(0, 80)}...`);
  if (project.link)
    console.log(`   Link: ${colors.cyan}${project.link}${colors.reset}`);
...
function search(data, query) {
  ...
  // Search URLs
  results.urls = data.urls.filter(
    (u) =>
      u.url.toLowerCase().includes(q) || u.project.toLowerCase().includes(q),
  );

  // Search domains
  results.domains = data.domains.filter(
    (d) =>
...
function showSummary(data) {
  ...
  console.log(
    `${colors.bright}Total Projects:${colors.reset} ${colors.green}${data.projects.length}${colors.reset}`,
  );
  console.log(
    `${colors.bright}Active Services:${colors.reset} ${colors.green}${data.inventory.masterInventory?.services.filter((s) => s.status.includes("Active") || s.status.includes("✅")).length || 0}${colors.reset}`,
  );
  console.log(
    `${colors.bright}Test URLs:${colors.reset} ${colors.green}${data.urls.length}${colors.reset}`,
  );
  console.log(
    `${colors.bright}Tracked Domains:${colors.reset} ${colors.green}${data.domains.length}${colors.reset}`,
  );
  console.log(
    `${colors.bright}Last Updated:${colors.reset} ${new Date(data.lastUpdated).toLocaleString()}`,
  );
  ...
  data.domains.slice(0, 5).forEach((d, i) => {
    ...
    console.log(
      `  ${i + 1}. ${d.name} - ${statusColor}${d.status}${colors.reset} (${d.purpose})`,
    );
  });
  ...
  console.log(
    `  ${colors.cyan}dashboard projects${colors.reset}    - List all projects`,
  );
  console.log(
    `  ${colors.cyan}dashboard urls${colors.reset}        - List all test URLs`,
  );
  console.log(
    `  ${colors.cyan}dashboard domains${colors.reset}     - List all domains`,
  );
  console.log(
    `  ${colors.cyan}dashboard search <term>${colors.reset} - Search everything`,
  );
  console.log(
    `  ${colors.cyan}dashboard refresh${colors.reset}     - Regenerate dashboard`,
  );
  console.log(
    `  ${colors.cyan}dashboard open${colors.reset}        - Open HTML dashboard in browser`,
  );
}
...
function showProjects(data, filter = null) {
  ...
  console.log(
    `Found ${colors.bright}${projects.length}${colors.reset} projects\n`,
  );
...
function showDomains(data, filter = null) {
  ...
  console.log(
    `Found ${colors.bright}${domains.length}${colors.reset} domains\n`,
  );
...
function showSearch(data, query) {
  ...
  if (results.projects.length > 0) {
    console.log(
      `${colors.bright}${colors.green}Projects (${results.projects.length}):${colors.reset}\n`,
    );
  ...
  if (results.urls.length > 0) {
    console.log(
      `${colors.bright}${colors.green}URLs (${results.urls.length}):${colors.reset}\n`,
    );
  ...
  if (results.domains.length > 0) {
    console.log(
      `${colors.bright}${colors.green}Domains (${results.domains.length}):${colors.reset}\n`,
    );
...
function openBrowser() {
  ...
  if (!fs.existsSync(dashboardPath)) {
    console.log(
      `${colors.yellow}⚠️  Dashboard not found. Generating...${colors.reset}`,
    );
    execSync("node scripts/aggregate-project-dashboard.js", {
      cwd: REPO_ROOT,
      stdio: "inherit",
...
function main() {
  ...
  if (
    ...
    console.log(
      `${colors.bright}${colors.cyan}Dashboard CLI - Project Visibility Tool${colors.reset}`,
    );
    console.log(`\nUsage: dashboard [command] [options]\n`);
    console.log(`Commands:`);
    console.log(
      `  ${colors.green}summary${colors.reset}             Show dashboard summary (default)`,
    );
    console.log(
      `  ${colors.green}projects [filter]${colors.reset}   List all projects`,
    );
    console.log(
      `  ${colors.green}urls [filter]${colors.reset}       List all test URLs`,
    );
    console.log(
      `  ${colors.green}domains [filter]${colors.reset}    List all domains`,
    );
    console.log(
      `  ${colors.green}search <term>${colors.reset}       Search everything`,
    );
    console.log(
      `  ${colors.green}refresh${colors.reset}             Regenerate dashboard data`,
    );
    console.log(
      `  ${colors.green}open${colors.reset}                Open HTML dashboard in browser`,
    );
    console.log(
