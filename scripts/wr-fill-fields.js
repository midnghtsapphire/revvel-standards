...
function parseIssueBody(body) {
  ...
  const fields = found.map((entry, i) => {
    ...
    const raw = body.slice(
      entry.endHeading,
      next ? next.startBlock : body.length,
    );
  ...
  if (mode === "derived") {
    ...
    if (field.heading === "Summary") {
      ...
      if (objective) {
        ...
        if (firstLine)
          return {
            value: firstLine.trim().slice(0, 160),
            source: "derived from Objective first line",
          };
      }
    }
  }
...
function fillDeterministic({ title, fields }) {
  ...
  for (const f of fields)
    if (!isBlank(f.kind, f.value)) filledMap[f.heading] = f.value;
...
async function fillWithLlm({ title, fields, filledMap, log }) {
  ...
  const changes = [];
  const enableLlm =
    process.env.WR_LLM === "1" && process.env.OPENROUTER_API_KEY;
  ...
  for (const f of fields) {
    ...
    if (enableLlm) {
      try {
        ...
        log(
          `llm refine failed for "${f.heading}": ${err.message} — using fallback`,
        );
      }
    }
...
function refineOneField({ heading, title, filledMap }) {
  ...
  return withCascade([
    () =>
      openRouterCall(
        process.env.WR_LLM_MODEL || "anthropic/claude-sonnet-4",
        systemPrompt,
        userPrompt,
      ),
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    const a = argv[i];
    if (a === "--issue-file" && argv[i + 1]) {
      out.issueFile = argv[++i];
      continue;
    }
    if (a === "--out" && argv[i + 1]) {
      out.out = argv[++i];
...
async function main() {
  ...
  if (args.issueFile) body = fs.readFileSync(args.issueFile, "utf8");
    ...
    process.stderr.write("wr-fill-fields: pass --issue-file or --stdin\n");
    process.exit(2);
  }
  if (!args.title) {
    process.stderr.write("wr-fill-fields: --title is required\n");
    process.exit(2);
  }
  ...
  for (const c of result.changes)
    process.stderr.write(`  - ${c.heading} [${c.source}]\n`);
}
...
if (require.main === module) {
  main().catch((e) => {
    process.stderr.write(`wr-fill-fields: ${e.stack || e.message}\n`);
    process.exit(1);
  });
}
