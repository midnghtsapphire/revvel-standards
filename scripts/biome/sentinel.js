...
const STUCK_LABEL_RE =
  /stuck|checking|self-heal|auto-fix|won't-merge|wont-merge/i;
...
function buildIncidentBody(section, generatedAtIso) {
  ...
  if (f.length) {
    ...
    for (const r of f)
      lines.push(
        `- ${r.name || r.path || r.id} — \`${r.conclusion}\` ([run](${r.url}))`,
      );
    lines.push("");
  }
  if (s.length) {
    ...
    for (const it of s)
      lines.push(
        `- #${it.number} — ${it.title} (last update ${it.updated_at})`,
      );
    lines.push("");
  }
  lines.push("---");
  lines.push(
    "_biome-medic will attempt rule-based remediation; this issue auto-resolves when the fleet recovers._",
  );
  return lines.join("\n");
}
...
if (require.main === module) {
  (async () => {
    ...
    if (!runsResp)
      console.warn(
        "[biome-sentinel] warning: failed to fetch recent workflow runs (API error) — treating as 0 runs for this sweep",
      );
    const issuesResp = await repoApi(
    ...
    const search = await repoApi(
      "/issues?state=open&labels=biome,scorecard&per_page=50",
      { allowError: true },
    );
    const existing = Array.isArray(search)
      ? search.find(
          (i) => !i.pull_request && (i.body || "").includes(INCIDENT_MARKER),
        )
      : null;
    ...
    if (!shouldFileIncident(section)) {
      if (existing) {
        ...
        console.log(
          `[biome-sentinel] fleet healthy — auto-resolved incident #${existing.number}`,
        );
      } else {
    ...
    if (existing) {
      ...
      console.log(
        `[biome-sentinel] refreshed existing incident #${existing.number} in place`,
      );
    } else {
