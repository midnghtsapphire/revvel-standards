...
function assessKeyHealth(env, keyNames = TRACKED_KEYS) {
  ...
  return {
    present,
    missing,
    aiLanesOffline: missing.length > 0,
    checked: keyNames.slice(),
  };
}
...
function buildStatusBody(assessment, generatedAtIso) {
  ...
  if (offline) {
    lines.push(
      `Missing AI keys: ${assessment.missing.map((k) => `\`${k}\``).join(", ")}.`,
    );
    lines.push("");
    lines.push(
    ...
    lines.push(
      `Present AI keys: ${assessment.present.map((k) => `\`${k}\``).join(", ") || "(none tracked)"}.`,
    );
    lines.push("");
    lines.push(
      "All tracked AI lanes are online and rule-based lanes remain active as backup.",
    );
  }
  return lines.join("\n");
}
...
if (require.main === module) {
  (async () => {
    ...
    console.log(
      `[biome-homeostat] ai_lanes=${assessment.aiLanesOffline ? "offline" : "online"} present=${assessment.present.length} missing=${assessment.missing.length}`,
    );

    const search = await repoApi(
      "/issues?state=open&labels=biome,homeostat&per_page=50",
      { allowError: true },
    );
    const existing = Array.isArray(search)
      ? search.find(
          (i) => !i.pull_request && (i.body || "").includes(STATUS_MARKER),
        )
      : null;
    ...
    if (!assessment.aiLanesOffline) {
      ...
      if (existing) {
        ...
        console.log(
          `[biome-homeostat] resolved status note #${existing.number}`,
        );
      } else {
    ...
    if (existing) {
      ...
      console.log(
        `[biome-homeostat] refreshed consolidated status #${existing.number}`,
      );
    } else {
      const created = await repoApi("/issues", {
        ...
        body: {
          title:
            "[BIOME-HOMEOSTAT] AI lanes offline — crew healthy (credit-free)",
          body,
    ...
    if (shouldTriggerRecovery(assessment, process.env)) {
      const recoveryWorkflow =
        process.env.BIOME_RECOVERY_WORKFLOW || "secret-persistence-guard.yml";
      ...
      console.log(
        `[biome-homeostat] dispatched ${recoveryWorkflow} to rehydrate keys from Doppler`,
      );
    }
  })().catch((err) => {
