...
function isExternal(meta) {
  return !!(
    meta &&
    (meta.repo || meta.path === "external" || meta.type === "external")
  );
}
...
function classifyApp({ name, url, status, external }) {
  if (!url) {
    return {
      ...
      http_status: null,
      reachable: false,
      state: external ? "external-no-link" : "no-link",
    };
  }
  ...
  return {
    ...
    reachable,
    state: reachable ? "live" : "unreachable",
  };
}
...
function buildScoreboard(apps, generatedAtIso, baseUrl) {
  ...
  return {
    ...
    overall:
      apps.length === 0
        ? "pending"
        : summary.unreachable + summary.no_link > 0
          ? "incomplete"
          : "all-testable-live",
    summary,
...
function buildWorklistBody(apps, generatedAtIso) {
  ...
  for (const a of incomplete) {
    if (a.state === "no-link") {
      lines.push(
        `- **${a.name}** — 🟠 no live URL could be derived. Set \`deployment.base_url\` in \`docs/app-deployments.yml\` (or give the app an explicit \`live_url\`).`,
      );
    } else {
      lines.push(
        `- **${a.name}** — 🔴 live URL ${a.url} returns \`${a.http_status}\`; deploy/fix the page so it serves 2xx.`,
      );
    }
  }
  lines.push("");
  lines.push("---");
  lines.push(
    "_Auto-resolves when every project is testable-live. Full board: `docs/biome/app-completion.json`._",
  );
  return lines.join("\n");
}
...
if (require.main === module) {
  (async () => {
    ...
    const manifest =
      YAML.parse(
        fs.readFileSync(
          path.join(process.cwd(), "docs", "app-deployments.yml"),
          "utf8",
        ),
      ) || {};
    ...
    async function attempt(url, method) {
      ...
      try {
        const res = await fetch(url, {
          ...
          redirect: "follow",
          signal: ctrl.signal,
          headers: { "User-Agent": "biome-inspector" },
        });
        return res.status;
      } finally {
        clearTimeout(timer);
    ...
    for (const [name, meta] of Object.entries(appsManifest)) {
      ...
      results.push(
        classifyApp({ name, url, status, external: isExternal(meta) }),
      );
    }
    ...
    fs.writeFileSync(
      path.join(outDir, "app-completion.json"),
      `${JSON.stringify(scoreboard, null, 2)}\n`,
    );
    fs.writeFileSync(
      path.join(outDir, "app-completion.html"),
      renderScoreboardHtml(scoreboard),
    );
    console.log(
      `[biome-inspector] ${scoreboard.summary.live}/${scoreboard.summary.total} testable-live · ${scoreboard.summary.unreachable} unreachable · ${scoreboard.summary.no_link} no-link`,
    );

    // Worklist: upsert/resolve a single deduped issue.
    const search = await repoApi(
      "/issues?state=open&labels=biome,dod-gap&per_page=50",
      { allowError: true },
    );
    const existing = Array.isArray(search)
      ? search.find(
          (i) => !i.pull_request && (i.body || "").includes(WORKLIST_MARKER),
        )
      : null;
    ...
    if (incomplete.length === 0) {
      if (existing) {
        await repoApi(`/issues/${existing.number}/comments`, {
          method: "POST",
          body: {
            body: `${WORKLIST_MARKER}\n✅ All projects testable-live as of ${generatedAt}. Resolving.`,
          },
          allowError: true,
        });
        ...
        console.log(
          "[biome-inspector] all projects testable-live — no worklist.",
        );
      }
      return;
    }
    ...
    if (existing) {
      ...
      console.log(
        `[biome-inspector] refreshed worklist #${existing.number} (${incomplete.length} incomplete)`,
      );
    } else {
      const created = await repoApi("/issues", {
        method: "POST",
        body: {
          title: `[BIOME-INSPECTOR] ${incomplete.length} project(s) not testable-live`,
          body,
          labels: ["biome", "dod-gap", "self-heal"],
        },
      });
      console.log(
