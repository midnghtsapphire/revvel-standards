...
function decideRemediations({
  ...
  const relabels = stuckIssues.slice(0, c.maxRelabels).map((it) => ({
    ...
    remove: [],
    comment:
      "biome-medic: re-surfacing stuck item for the self-healing loop (rule-based, credit-free).",
  }));
  ...
  const reruns = c.allowReruns
    ? recentFailures
        ...
        .map((f) => ({
          run_id: f.id,
          name: f.name,
          reason: "recent failure — rule-based re-run",
        }))
    : [];
...
if (require.main === module) {
  (async () => {
    ...
    const allowReruns =
      String(process.env.BIOME_ALLOW_RERUNS || "").toLowerCase() === "true";
    ...
    if (!runsResp)
      console.warn(
        "[biome-medic] warning: failed to fetch recent workflow runs (API error) — treating as 0 runs for this sweep",
      );
    ...
    const keyBlocked = issuesResp.filter(
      (it) =>
        !it.pull_request &&
        it.state === "open" &&
        labelNames(it).includes("openrouter:needs-key"),
    );
    ...
    for (const action of [...plan.relabels, ...plan.keyUnblocks]) {
      ...
      if (action.comment) {
        await repoApi(`/issues/${action.issue}/comments`, {
          method: "POST",
    ...
    for (const r of plan.reruns) {
      await repoApi(`/actions/runs/${r.run_id}/rerun`, {
        method: "POST",
        allowError: true,
      });
