...
function glob(rel) {
  return walk(path.join(REPO_ROOT, rel)).map((p) =>
...
const CHECKS = [
  ...
  // re-adding Doppler, this now verifies Doppler IS enabled.
  {
    ...
    run() {
      ...
      const guardHasKillSwitch =
        /if false; then\s*#\s*was: if \[ -n "\$DOPPLER_TOKEN" \]/i.test(guard);
      const sentinelHasKillSwitch = /DOPPLER DISABLED|Auto-heal disabled/i.test(
        sentinel,
      );
      ...
      const detail = pass
        ...
        : [
            guardHasKillSwitch
              ? "- secret-persistence-guard.yml: Doppler has `if false` kill switch (disabled)"
              : "",
            sentinelHasKillSwitch
              ? "- secrets-sentinel.yml: Heal step has DOPPLER DISABLED notice"
              : "",
          ]
            .filter(Boolean)
            .join("\n");
  ...
  // ─── WR-without-PR groundhog day ──────────────────────────────────────
  {
    ...
    run() {
      ...
      return {
        ...
        detail: pass
          ? ""
          : `${missing.length} WR file(s) without a linked PR and not marked closed:\n` +
            missing
              .slice(0, 25)
              .map((x) => `  - ${x}`)
              .join("\n") +
  ...
  // ─── No throwaway dev files at repo root ──────────────────────────────
  {
    ...
    run() {
      const junkPatterns =
        /^(plan|finish_clean|fix_boilerplate|update_wr|tmp[_-].*|scratch.*|temp[_-].*|throwaway.*|notes)\.(js|mjs|ts|md|sh|py|txt)$/;
      let root;
      try {
  ...
  // ─── Subscription tracker inventory completeness ──────────────────────
  {
    ...
    run() {
      ...
      const entries = yml.split(/^ {2}- name:\s*/m).slice(1);
      const missing = [];
      for (const e of entries) {
        const firstLine = e.split("\n", 1)[0].trim();
        const name = firstLine || "(unnamed)";
        const block = e.split(/^ {2}- name:/m)[0];
        const hasTrial = /^\s{4,}trial_end:\s*\d{4}-\d{2}-\d{2}/m.test(block);
        const hasRenewal = /^\s{4,}renewal_date:\s*\d{4}-\d{2}-\d{2}/m.test(
          block,
        );
        const usagePriced = /^\s{4,}billing_cycle:\s*(usage|free)/im.test(
          block,
        );
      ...
      return {
        ...
        detail:
          missing.length === 0
            ? ""
            : `Entries missing a date (fill from each provider's dashboard):\n` +
...
function runAll() {
  return CHECKS.map((c) => {
    ...
    try {
      ...
      r = { pass: false, detail: `Check errored: ${e.message}` };
    }
...
function renderReport(results) {
  ...
  if (failed.length === 0) {
    ...
    if (informational.length) {
      lines.push(
        `### 🟡 Informational (pre-existing backlog, doesn't block CI) — ${informational.length}`,
      );
      lines.push("");
    }
  ...
  lines.push("---");
  lines.push(
    "_Maintained by `.github/workflows/auditor-controller.yml`. Add new assertions in `scripts/auditor-controller.js` → `CHECKS`._",
  );
  return lines.join("\n");
}
...
if (args.includes("--check") && results.some((r) => !r.pass && r.gating))
  process.exit(1);
