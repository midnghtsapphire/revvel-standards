  ...
  try {
    ...
    if (fs.existsSync(lookupPath)) {
      ...
      if (data.fallback_chain && data.models) {
        ...
        const excludedModels = resolvedModels.filter(
          (model) => !supportedModes.has(model.mode),
        );
...
async function ask(prompt, opts = {}) {
  ...
  if (preferNoKey) {
    try {
      ...
      return {
        text,
        lane: "no-key-perplexity",
        modelUsed: "perplexity/sonar (no-key)",
      };
    } catch (err) {
      if (!silent) {
        console.warn(
          `⚠️ no-key Perplexity lane failed, falling back to OpenRouter: ${err.message}`,
        );
      }
    }
  }
