...
function loadModelLookup() {
  try {
    ...
    console.warn(
      "Could not load model-lookup.json, falling back to defaults",
      err.message,
    );
  }
  return null;
...
if (lookupData && lookupData.profiles && lookupData.models) {
  ...
  ROUTING_PROFILES = {
    ...
    repo_surgery: {
      description:
        "Multi-file edits, bug fixing, refactors, and 'take initiative' tasks",
      models: [
    ...
    cheap_batch_edits: {
      description:
        "Repetitive transforms, test generation, lint-fix loops, and lower-cost bulk changes",
      models: ["deepseek/deepseek-v3.2", "anthropic/claude-sonnet-4"],
    },
    hard_debug: {
      description:
        "Difficult failures, ambiguous root-cause analysis, and second-opinion patches",
      models: [
        "openai/gpt-5.2-codex",
        "anthropic/claude-sonnet-4",
        "deepseek/deepseek-v3.2",
...
async function callOpenRouter({
  ...
  const referer =
    ...
    "https://github.com/midnghtsapphire/revvel-standards";
  const title =
    appTitle ||
    process.env.OPENROUTER_APP_TITLE ||
    "Revvel Standards OpenRouter Routing";
...
async function routedChat({
  ...
  if (!profileConfig) {
    ...
    throw new Error(
      `Unknown profile: ${profile}. Available profiles: ${available}`,
    );
  }
  ...
  if (!silent) {
    ...
    console.log(
      `🎯 Requested models (fallback order): ${profileConfig.models.join(" → ")}`,
    );
  }
