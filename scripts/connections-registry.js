...
function load(registryPath = REGISTRY_PATH) {
  ...
  return doc && Array.isArray(doc.connections)
    ? doc
    : { meta: {}, connections: [] };
}
...
function renderMarkdown(doc) {
  ...
  out.push("");
  out.push(
    `**${conns.length}** connections · **${verified}** verified · **${unverified}** unverified (confirm before relying on them).`,
  );
  out.push("");
...
function renderReadmeBlock(doc) {
  ...
  lines.push("");
  lines.push(
    `${conns.length} wired integrations (${verified} verified). Full table: [docs/CONNECTIONS_REGISTRY.md](docs/CONNECTIONS_REGISTRY.md) · on-demand dashboard: [connections-dashboard.html](connections-dashboard.html). SSOT: \`config/connections.yml\`.`,
  );
  lines.push("");
  ...
  for (const type of [
    ...
    const examples = items
      ...
      .join(", ");
    lines.push(
      `| ${TYPE_TITLE[type] || type} | ${items.length} | ${examples}${items.length > 4 ? ", …" : ""} |`,
    );
  }
  lines.push("");
  lines.push(END);
...
function renderFallbackBlock(doc) {
  ...
  if (retired.length) {
    lines.push("");
    lines.push(
      `*Retired: ${retired.map((r) => `\`${r.id}\` (${r.note})`).join("; ")}*`,
    );
  }
  lines.push(FB_END);
  return lines.join("\n");
...
function main(argv = process.argv.slice(2)) {
  const doc = load();
  const wantHtml =
    argv.includes("--html") || argv.includes("--all") || argv.length === 0;
  const wantMd =
    argv.includes("--markdown") || argv.includes("--all") || argv.length === 0;
  ...
  if (wantDrift) {
    ...
    if (missing.length) {
      console.log(
        `⚠️ ${missing.length} integration(s) in the repo but missing from config/connections.yml:`,
      );
