...
function validateSchema(obj) {
  ...
  if (obj._parseError) {
    ...
    return {
      ...
      pass: false,
      errors: [
        { property: "json", message: `JSON parse error near: ${safeRaw}` },
      ],
      warnings: [],
    };
  }
  ...
  if (!obj["@context"]) {
    ...
    if (!/(?:^|[./])schema\.org(?:[/"#]|$)/.test(ctxStr)) {
      warnings.push({
        property: "@context",
        message: `@context "${ctxStr}" does not reference schema.org`,
      });
    }
  }
  ...
  const rules = Object.hasOwn(SCHEMA_RULES, primaryType)
    ? SCHEMA_RULES[primaryType]
    : undefined;
  if (rules) {
    for (const prop of rules.required) {
      if (obj[prop] === undefined || obj[prop] === null || obj[prop] === "") {
        errors.push({
          property: prop,
          message: `Required property "${prop}" is missing for @type "${primaryType}"`,
        });
      }
    }
    for (const prop of rules.recommended) {
      if (obj[prop] === undefined || obj[prop] === null || obj[prop] === "") {
        warnings.push({
          property: prop,
          message: `Recommended property "${prop}" is missing for @type "${primaryType}" — add to improve rich result eligibility`,
        });
      }
    }
  } else {
    warnings.push({
      property: "@type",
      message: `@type "${primaryType}" is not in the Revvel known-types list — manual review recommended`,
    });
  }
...
function _validatePropertyShapes(type, obj) {
  ...
  if (["Article", "NewsArticle", "BlogPosting"].includes(type) && obj.author) {
    ...
    for (const a of authors) {
      if (
        ...
        typeof a === "object" &&
        a["@type"] &&
        !["Person", "Organization"].includes(a["@type"])
      ) {
        warnings.push({
          property: "author",
          message: `author @type should be "Person" or "Organization", got "${a["@type"]}"`,
        });
      }
    }
  }
  ...
  if (
    ...
    obj.headline.length > 110
  ) {
    errors.push({
      property: "headline",
      message: `headline exceeds 110 characters (${obj.headline.length}) — Google will truncate or reject`,
    });
  }

  // FAQPage: mainEntity items must be Question with acceptedAnswer
  if (type === "FAQPage" && obj.mainEntity) {
    const items = Array.isArray(obj.mainEntity)
      ? obj.mainEntity
      : [obj.mainEntity];
  ...
  if (type === "BreadcrumbList" && obj.itemListElement) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (li?.["@type"] !== "ListItem") {
        ...
        if (!Number.isInteger(li.position) || li.position < 1)
          errors.push({
            property: `itemListElement[${i}].position`,
            message:
              '"position" must be a positive integer (≥ 1) on each ListItem',
          });
        if (!li.name && !li.item)
          errors.push({
            property: `itemListElement[${i}].name`,
            message: '"name" or "item" is required on each ListItem',
          });
      }
    }
  }
    ...
    for (let i = 0; i < offers.length; i++) {
      ...
      if (offer && offer["@type"] === "Offer") {
        if (offer.price === undefined && offer.priceRange === undefined) {
          warnings.push({
            property: `offers[${i}].price`,
            message:
              'Offer should have "price" or "priceRange" for rich results',
          });
        }
        if (!offer.priceCurrency) {
          warnings.push({
            property: `offers[${i}].priceCurrency`,
            message: 'Offer should have "priceCurrency" (e.g., "USD")',
          });
        }
      }
    }
...
function checkRichResults(obj, precomputedValidation) {
  if (!obj || obj._parseError) {
    return {
      eligible: false,
      type: "unknown",
      reason: "Invalid JSON-LD — cannot determine eligibility",
    };
  }
  ...
  if (types.length === 0) {
    return {
      eligible: false,
      type: "unknown",
      reason: "Missing @type — required for rich result eligibility",
    };
  }
...
const TOOL_LINKS = {
  ...
  schemaOrgValidator: {
    ...
    description:
      "Official schema.org validator for JSON-LD, Microdata, and RDFa",
  },
  bingWebmaster: {
  ...
  merkleTester: {
    ...
    description:
      "Popular free tool for validating code snippets (favored by technical SEOs)",
  },
  seoSiteCheckup: {
...
function generateReport(checkResult) {
  ...
  if (summary.total === 0) {
    lines.push(
      '> ⚠️ No `<script type="application/ld+json">` blocks found on this page.',
    );
    lines.push("");
  }
  ...
  for (const r of results) {
    ...
    lines.push(
      `### Google Rich Results — ${g.eligible ? "✅ ELIGIBLE" : "⚠️  NOT ELIGIBLE"}`,
    );
    lines.push("");
    lines.push(`${g.reason}`);
    lines.push("");
