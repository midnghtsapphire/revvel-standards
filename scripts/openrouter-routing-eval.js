...
function generateMarkdownReport(results, prompt, systemPrompt) {
  ...
  for (const result of results) {
    ...
    if (result.success) {
      ...
      lines.push(
        `- **Requested Models (fallback chain):** ${result.requestedModels.join(" → ")}`,
      );
  ...
  if (successfulResults.length > 0) {
    const sorted = [...successfulResults].sort(
      (a, b) => b.responseLength - a.responseLength,
    );
    lines.push(
      `- **Most detailed:** ${sorted[0].profile} (${sorted[0].responseLength} chars)`,
    );
    lines.push(
      `- **Most concise:** ${sorted[sorted.length - 1].profile} (${sorted[sorted.length - 1].responseLength} chars)`,
    );
  } else {
  ...
  if (successfulResults.length > 0) {
    ...
    lines.push(
      `- **Fastest:** ${sorted[0].profile} (${sorted[0].responseTime}ms)`,
    );
    lines.push(
      `- **Slowest:** ${sorted[sorted.length - 1].profile} (${sorted[sorted.length - 1].responseTime}ms)`,
    );
  } else {
    lines.push("- No successful responses to compare");
  }
  ...
  lines.push(
    "- **Independence:** Does the response show initiative and suggest improvements?",
  );
  lines.push(
    "- **Completeness:** Does the response cover all aspects of the task?",
  );
  lines.push(
    "- **Code Quality:** Is the generated code idiomatic and well-structured?",
  );
  ...
  lines.push("");
  lines.push(
    "Different models have different costs per token. Based on typical OpenRouter pricing:",
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "For cost-sensitive batch operations, prefer `cheap_batch_edits` profile.",
  );
  lines.push(
    "For critical bug fixes where quality matters most, prefer `repo_surgery` or `hard_debug`.",
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "1. **Use `repo_surgery` for:** [Add recommendations based on response quality]",
  );
  lines.push(
    "2. **Use `cheap_batch_edits` for:** [Add recommendations based on cost/quality balance]",
  );
  lines.push(
    "3. **Use `hard_debug` for:** [Add recommendations based on debugging effectiveness]",
  );
  lines.push("");
...
async function main() {
  ...
  if (args.length === 0 || args.includes("--help") || args.includes("-h")) {
    ...
    console.log("Example:");
    console.log(
      '  node scripts/openrouter-routing-eval.js "Fix the memory leak in the session handler"',
    );
    console.log();
    console.log(
      "This will run the prompt through all three routing profiles and generate a comparison report.",
    );
    process.exit(0);
  }
  ...
  const systemPrompt =
    "You are an expert software engineer. Provide a detailed, practical response with code examples where appropriate.";
  ...
  for (const profile of PROFILES) {
    ...
    if (result.success) {
      console.log(
        `✅ Success (${result.modelUsed}, ${result.responseTime}ms, ${result.responseLength} chars)`,
      );
    } else {
  ...
  const outputPath = path.join(
    outputDir,
    `openrouter-routing-eval-${timestamp}.md`,
  );
