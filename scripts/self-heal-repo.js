...
async function cleanupStaleIssues() {
  ...
  const issues = await getIssues("open", {
    since: THIRTY_DAYS_AGO.toISOString(),
  });
...
async function labelStaleIssues() {
  ...
  const issues = await getIssues("open", {
    since: FOURTEEN_DAYS_AGO.toISOString(),
  });
...
async function cleanupDuplicates() {
  ...
  for (const [title, duplicates] of Object.entries(byTitle)) {
    if (duplicates.length > 1) {
      console.log(
        `  Found ${duplicates.length} duplicates: "${title.substring(0, 50)}..."`,
      );
      ...
      console.log(
        `    Keeping #${oldest.number}, closing ${toClose.length} duplicates`,
      );
...
async function checkWorkflowHealth() {
  ...
  console.log(
    `  ❌ ${bad.length} invalid workflow file(s) — surfacing for repair`,
  );
  ...
  try {
    ...
    const existing = open.find((i) =>
      (i.title || "").startsWith("[SELF-HEAL] Invalid workflow YAML"),
    );
    if (existing) {
      await addComment(
        existing.number,
        `Still detecting invalid workflow YAML:\n\n${list}`,
      );
    } else {
      await post("/issues", {
        title,
        body,
        labels: ["auto-error", "needs-human", "ci"],
      });
  ...
  for (const pr of prs) {
    ...
    try {
      ...
      const del = async (l) => {
        if (labels.includes(l)) {
          try {
            await api(`/issues/${n}/labels/${encodeURIComponent(l)}`, {
              method: "DELETE",
            });
      ...
      if (pr.draft) {
        ...
        for (const l of STATUS_LABELS.filter((s) => s !== "status:draft"))
          await del(l);
        continue;
      }
