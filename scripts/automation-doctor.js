...
class AutomationDoctor {
  ...
  async run() {
    ...
    if (this.options.stuck || this.options.all) {
      ...
      this.results.stuck.push({
        note: "Requires GitHub API - use workflow for live check",
      });
    }
  ...
  async validateWorkflows() {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (missingTimeout.length > 0) {
          this.results.workflows.missingTimeout.push({
            file,
            jobs: missingTimeout,
          });
        }
    ...
    console.log(
      `  ✓ Valid: ${validCount}, Invalid: ${invalidCount}, Missing timeout: ${this.results.workflows.missingTimeout.length}`,
    );
  }
  ...
  checkLabels() {
    ...
    if (fs.existsSync(labelFile)) {
      ...
      console.log(
        "  ℹ️ No labels.yml found - labels would be created via workflow",
      );
      this.results.labels.missing = CONFIG.requiredLabels;
    }

    console.log(
      `  Present: ${this.results.labels.present.length}, Missing: ${this.results.labels.missing.length}`,
    );
  }
  ...
  generateReport() {
    ...
    report.push(
      `- Invalid workflows: ${this.results.workflows.invalid.length}`,
    );
    report.push(
      `- Jobs missing timeout: ${this.results.workflows.missingTimeout.length}`,
    );
    report.push("");
    ...
    const reportPath =
      process.env.AUTOMATION_DOCTOR_REPORT || "AUTOMATION-DOCTOR-REPORT.md";
...
async function main() {
  ...
  if (
    results.workflows.invalid.length > 0 ||
    results.workflows.missingTimeout.length > 0
  ) {
    process.exit(1);
  }
}
