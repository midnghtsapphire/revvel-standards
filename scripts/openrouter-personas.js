...
// (skills/agentic-workflow-fleet/FLEET.yml)
//
// Members are derived from the committed catalog (agent-creator-data.json,
// generated by scripts/build-agent-creator-data.js) so FLEET.yml stays the
// single source of truth and this file needs no YAML dependency. If the
// catalog is missing, the fleet is simply unavailable for that run — the
// nine core personas above are unaffected.
try {
  ...
  const fleetDef = fleetCatalog.fleet || {};
