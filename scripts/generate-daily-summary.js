...
async function fetchPRsSince(since) {
  try {
    ...
    return results
      ...
      .map((pr) => ({ ...pr, merged_at: pr.pull_request.merged_at }));
  } catch (error) {
...
async function generateMarkdownReport(date, issues, prs) {
  ...
  if (issues.length === 0) {
    ...
    for (const issue of issues) {
      ...
      lines.push(
        `| [#${issue.number}](${issue.html_url}) | ${issue.title} | @${issue.user.login} | ${formatDate(issue.created_at)} | ${labels} |`,
      );
    }
  }
  lines.push("");
    ...
    for (const pr of prs) {
      const status = pr.merged_at
        ...
        : pr.state === "closed"
          ...
          : "🔄 Open";
      lines.push(
        `| [#${pr.number}](${pr.html_url}) | ${pr.title} | @${pr.user.login} | ${formatDate(pr.created_at)} | ${status} |`,
      );
    }
  }
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "_This report is automatically generated by `.github/workflows/daily-wr-summary.yml`._",
  );
...
async function main() {
  ...
  console.log(
    `🔍 Looking for activity since midnight today (${today} 00:00 UTC)\n`,
  );
