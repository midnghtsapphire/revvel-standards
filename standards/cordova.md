...
platform :android do
  ...
  lane :cordova_internal do
    ...
    # )
    UI.message("⚠️  Cordova Android lane not yet active. See standards/cordova.md")
  end
end
...
platform :ios do
  ...
  lane :cordova_testflight do
    ...
    UI.message("⚠️  Cordova iOS lane not yet active. See standards/cordova.md")
  end
end
```
...
| `standards/security.md` | Security requirements for mobile apps |
| `standards/vault-agent.md` | Keystore and API key secret management |
| `standards/deployment.md` | How releases flow through CI to the stores |
