...
- **Human escalation as a last resort** — agents attempt automated remediation first; they escalate to a human only after exhausting automated options (see Ralph Loop in `standards/agent-factory.md`).
...
| Security headers | Helmet.js config vs. `standards/security.md` requirements |
...
Dependabot handles routine dependency bumps (see `standards/dependabot.md`). The Remediation Bot supplements it for **P0/P1 CVEs only**, applying fixes without waiting for the Dependabot schedule:
...
The **Ralph Loop** (`templates/cicd/ralph-loop.yml`) is the CI implementation of the self-healing loop defined in `standards/agent-factory.md`. The Automated Audit Agent integrates with the Ralph Loop by:
...
- `standards/agent-factory.md` — Ralph Loop and agent orchestration
- `standards/security.md` — base security requirements
- `standards/osint.md` — threat intelligence that feeds into audit agents
- `standards/api-gatekeeper.md` — gatekeeper enforcement of audit findings
