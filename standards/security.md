...
For applications serving organization members or enterprise users, SAML SSO is **required** in addition to Clerk. Follow `standards/sso-saml.md` for the full implementation guide.
...
- See `standards/repository-privacy-migration.md` for the full process and bulk-privatization script.
...
3. If the commit introduced unauthorized code: treat as a P0 security event — open a GitHub Issue, rotate any credentials accessible from that code path, and initiate a git history rewrite per `standards/repository-privacy-migration.md §3.4`.
