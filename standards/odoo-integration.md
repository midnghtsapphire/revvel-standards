...
- A **CRM** (leads, opportunities, contacts, pipelines) — already standardised in [`standards/leads.md`](standards/leads.md).
...
| **Database** | Managed PostgreSQL (shared Revvel instance) — see [`standards/database-architecture.md`](standards/database-architecture.md) |
...
| **CRM module** | Odoo `crm` (CE) — pipelines align 1:1 with [`standards/leads.md`](standards/leads.md) stages |
...
| `FREEDOM_ANGEL_CORPS` | Enterprise org (see [`standards/repository-privacy-migration.md`](standards/repository-privacy-migration.md)) | Future consolidated entity |
...
| `crm` | Sales pipelines, leads, opportunities | ✅ Always on | Manual spreadsheets; aligns with [`standards/leads.md`](standards/leads.md) |
...
| `marketing_automation` | Drip campaigns | ❌ **Do not use** — Enterprise-only; use Loops / Mailchimp per [`standards/marketing-automation.md`](standards/marketing-automation.md) |
...
| `crm.stage` | Pipeline stages | Matches [`standards/leads.md`](standards/leads.md) stage codes exactly |
...
1. Authenticates callers against Revvel Vault (see [`standards/vault-agent.md`](standards/vault-agent.md)) and Revvel API Gatekeeper rules (see [`standards/api-gatekeeper.md`](standards/api-gatekeeper.md)).
...
5. Writes an audit row to the Revvel audit log (see [`standards/security.md`](standards/security.md)).
...
| Revvel Lead form (`standards/leads.md`) | Upsert `crm.lead` in the lead's company scope | `crm.lead`, `res.partner` |
...
- Aligns with the $600 threshold rules already codified in [`standards/affiliate-marketing.md`](standards/affiliate-marketing.md).
...
- [ ] Map `crm.stage` records to the stage codes in [`standards/leads.md`](standards/leads.md).
...
- [ ] Add Odoo to uptime monitoring and Sentry error forwarding per [`standards/security.md`](standards/security.md).

---

## 8. Security, Privacy & Compliance

1. **Secrets** — DB password, admin password, bridge HMAC key live in Vault. No secret may appear in `odoo.conf`, git, or `.env.example`. Enforced by the pre-commit secret-scanning hooks (`detect-secrets` and `detect-private-key`) defined by [`standards/syntax-error-prevention.md`](standards/syntax-error-prevention.md).
2. **Access** — Portal users (customers, tenants) get read-only access to their own records through Odoo's built-in portal. Internal users authenticate via Google OAuth (and SAML later per [`standards/sso-saml.md`](standards/sso-saml.md)).
3. **Repo hosting** — `revvel_odoo_bridge` source lives in a **private** Freedom Angel Corps repo per [`standards/repository-privacy-migration.md`](standards/repository-privacy-migration.md). PII test fixtures are never committed.
4. **Backups** — Encrypted at rest (DO Spaces SSE). Restore drill quarterly, logged in [`DARE_LOG.md`](DARE_LOG.md).
5. **Data retention** — Follows the retention matrix in [`standards/security.md`](standards/security.md). Soft-delete is disabled in Odoo; we rely on archival (`active = False`) + fiscal-year lockdown in accounting.
6. **Audit log** — `mail.thread`/`mail.activity` + the bridge audit table give us full write traceability, consumable by the Audit Agent ([`standards/automated-audit-agent.md`](standards/automated-audit-agent.md)).
...
| ID | Check | Priority |
|---|---|---|
| I1 | `standards/odoo-integration.md` is referenced from any project that posts invoices, sales orders, leads, or inventory to an external system | P1 |
...
OCA publishes `openupgrade` scripts for each LTS→LTS jump. We upgrade once per LTS, in a staging environment first per [`standards/test-environments.md`](standards/test-environments.md).
...
|---|---|
| [`standards/leads.md`](standards/leads.md) | CRM stage codes in Odoo must equal the codes here |
| [`standards/database-architecture.md`](standards/database-architecture.md) | Odoo DB lives on the shared managed Postgres |
| [`standards/api-gatekeeper.md`](standards/api-gatekeeper.md) | Bridge traffic must pass API Gatekeeper rules |
| [`standards/vault-agent.md`](standards/vault-agent.md) | All Odoo and bridge secrets via Vault |
| [`standards/security.md`](standards/security.md) | Backups, retention, audit |
| [`standards/marketing-automation.md`](standards/marketing-automation.md) | Marketing automation stays out of Odoo |
| [`standards/affiliate-marketing.md`](standards/affiliate-marketing.md) | IRS $600 threshold tagging on `res.partner` |
| [`ENTITY_HIERARCHY.md`](ENTITY_HIERARCHY.md) | Source of truth for the company list |
| [`standards/repository-privacy-migration.md`](standards/repository-privacy-migration.md) | Bridge addon repo privacy |
| [`standards/test-environments.md`](standards/test-environments.md) | Staging → live-test → prod pipeline for Odoo upgrades |
