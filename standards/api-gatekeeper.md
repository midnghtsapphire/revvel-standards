...
| CORS | Validate `Origin` against allowlist (see `standards/security.md` §4) | Block + 403 |
...
The Gatekeeper consumes blocklists produced by the OSINT pipeline (`standards/osint.md`) and enforces them at the request layer:
...
- `standards/security.md` — base security requirements (CORS, headers, rate limiting §5)
- `standards/osint.md` — OSINT blocklists consumed by the gatekeeper
- `standards/automated-audit-agent.md` — audit agents that monitor gatekeeper logs
- `standards/vault-agent.md` — secret storage for tokens and keys
- `standards/agent-factory.md` — agent trigger and routing matrix
