...
def studio_create(
    ...
    Args:
        ...
        - video: video_format (explainer|brief|cinematic|short),
          visual_style (auto_select|custom|classic|whiteboard|kawaii|anime|
          watercolor|retro_print|heritage|paper_craft; not for cinematic/short),
          video_style_prompt
        - infographic: orientation (landscape|portrait|square),
          detail_level (concise|standard|detailed),
          infographic_style (auto_select|sketch_note|professional|bento_grid|
...
def studio_revise(
    ...
    if not confirm:
        return {
            ...
            "note": (
                "This creates a NEW slide deck with revisions applied."
                " The original is not modified. Set confirm=True after user approves."
            ),
        }
