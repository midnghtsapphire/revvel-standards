...
**Next Action:** Land #17091, then rebase/merge #17097 (script-based lockfile gate) on top of it and fix its own review findings (byte-compare `cmp` of regenerated lockfiles false-fails across npm versions; temp-dir regeneration cannot resolve `file:` dependencies). Clean up Neon project branches (console or scheduled cleanup) so `Create Neon Branch` can pass anywhere. Then a human/agent merge sweep over the 7 `awaiting-approval` + 3 `ready-to-merge` PRs once auto-approve fires again.
