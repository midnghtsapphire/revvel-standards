...
test("decay: higher softness is more forgiving at the same count", () => {
  ...
  assert.ok(
    lenient > strict,
    "higher softness should yield a higher score for the same count",
  );
});
...
test("scoreDimensions: badCode stacks all four failure types", () => {
  ...
  assert.ok(
    none.badCode > many.badCode,
    "multiple finding types should lower badCode",
  );
});
...
test("scoreDimensions: revertedAfterMerge tanks rash below 0.5", () => {
  ...
  assert.ok(
    dims.rash < 0.5,
    `rash should be < 0.5 when reverted, got ${dims.rash}`,
  );
});
...
test("scoreDimensions: all hallucination signals stack", () => {
  const clean = scoreDimensions({});
  const hall = scoreDimensions({
    unresolvedRefs: 2,
    unbackedClaims: 1,
    crossModelHallucinationFlags: 1,
  });
  assert.ok(
    clean.hallucination > hall.hallucination,
    "multiple hallucination signals should lower score",
  );
});
...
test("updateTrust: NaN previousTrust falls back to STARTING_TRUST", () => {
  ...
  assert.ok(
    t > STARTING_TRUST,
    "NaN previous should converge up from STARTING_TRUST",
  );
});
