...
const { generatePromptPacket, packetToMarkdown, clampScore } = require(
  path.join(
    __dirname,
    "..",
    "products",
...
run("generates packet with all sections", () => {
  const p = generatePromptPacket({
    idea: "OSINT tool for tracking competitor pricing",
    audience: "founders",
  });
...
run("blue-ocean score boosts on niche keywords", () => {
  const niche = generatePromptPacket({
    idea: "AI OSINT compliance audit for vertical B2B",
  });
...
run("markdown export contains all section headers", () => {
  const p = generatePromptPacket({
    idea: "Polar.sh funding analytics for OSS maintainers",
  });
...
run("adds Spotify visual-link prompts for music embedding requests", () => {
  ...
  assert.equal(
    p.implementationPrompts.length,
    baseline.implementationPrompts.length + 2,
  );
});
...
run("does not add music-link prompts for non-music social requests", () => {
  ...
  assert.equal(
    nonMusicPacket.implementationPrompts.length,
    baseline.implementationPrompts.length,
  );
  assert.ok(!joined.includes("cannot contain clickable hyperlinks"));
});

run(
  "red-ocean scoring baseline, incremental keyword match, and case-insensitivity",
  () => {
    // Base score without any red-ocean keywords
    const basePacket = generatePromptPacket({
      idea: "A simple utility for organizing files",
    });
    assert.equal(basePacket.scores.redOcean, 30);

    // Single keyword match (+10)
    const singleMatch = generatePromptPacket({
      idea: "A generic utility for organizing files",
    });
    assert.equal(singleMatch.scores.redOcean, 40);

    // Two keywords match (+20)
    const doubleMatch = generatePromptPacket({
      idea: "A generic CRM utility for organizing files",
    });
    assert.equal(doubleMatch.scores.redOcean, 50);

    // Case-insensitive match (+10)
    const caseMatch = generatePromptPacket({
      idea: "A gEnErIc utility for organizing files",
    });
...
run("red-ocean scoring exact-100 and over-100 clamping", () => {
  ...
  const allKeywords = generatePromptPacket({
    idea: "social chat todo note crm generic crypto utility",
  });
