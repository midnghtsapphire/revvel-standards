...
function runTests() {
  ...
  runTest("sync-flow-charts.js has proper shebang", () => {
    ...
    assert(
      ...
      "Script should have node shebang",
    );
  });
  ...
  runTest("sync-flow-charts.js runs in DRY_RUN mode", () => {
    try {
      const result = execSync(
        `REPO_ROOT="${repoRoot}" DRY_RUN=true node "${scriptPath}"`,
        {
          encoding: "utf8",
          timeout: 30000,
          cwd: repoRoot,
      ...
      throw new Error(
        `Script execution failed: ${err.message}\n${err.stdout || ""}\n${err.stderr || ""}`,
      );
    }
  });
  ...
  runTest("sync-flow-charts.js detects flow charts directory", () => {
    try {
      ...
      assert(
        result.includes("Flow charts dir:"),
        "Should report flow charts directory",
      );
    } catch (err) {
  ...
  runTest("sync-flow-charts.js reports metadata updates", () => {
    try {
      ...
      assert(
        result.includes("Meta updates:"),
        "Should report metadata updates count",
      );
    } catch (err) {
      throw new Error(`Script execution failed: ${err.message}`);
    }
  ...
  runTest("sync-flow-charts.js checks for broken references", () => {
    try {
      ...
      assert(
        result.includes("Checking for broken file references"),
        "Should check for broken references",
      );
      assert(
        result.includes("Ref patches:"),
        "Should report reference patches count",
      );
    } catch (err) {
      throw new Error(`Script execution failed: ${err.message}`);
    }
  ...
  runTest("sync-flow-charts.js does not modify files in DRY_RUN mode", () => {
    try {
      ...
      assert(
        result.includes("DRY_RUN — no files were modified"),
        "Should confirm no files modified",
      );
    } catch (err) {
      throw new Error(`Script execution failed: ${err.message}`);
    }
  ...
  runTest("sync-flow-charts.js fails gracefully with missing directory", () => {
    try {
      ...
      throw new Error(
        "Script should have exited with error for missing directory",
      );
    } catch (err) {
      ...
      assert(
        output.includes("Flow charts directory not found") || err.status === 1,
  ...
  runTest("sync-flow-charts.js processes expected flow chart files", () => {
    try {
      ...
      assert(
        foundFiles >= 2,
        `Should mention at least 2 of the expected files, found ${foundFiles}`,
      );
    } catch (err) {
      throw new Error(`Script execution failed: ${err.message}`);
    }
