...
test("extractCitations pulls + dedupes URLs and strips trailing punctuation", () => {
  const text =
    "See https://a.com/x. Also (https://b.com/y) and https://a.com/x again.";
  assert.deepEqual(extractCitations(text), [
    "https://a.com/x",
    "https://b.com/y",
  ]);
});
...
test("parseAdjudication recovers JSON from fenced/prose output", () => {
  ...
  assert.deepEqual(parseAdjudication(fenced), {
    answer: "ok",
    agreement_score: 0.8,
  });
...
test("buildTwinResult uses adjudicator fields and emits the twin schema", () => {
  const runA = {
    answer: "A says https://a.com",
    citations: ["https://a.com"],
    latency_ms: 1000,
    cost_usd: 0.001,
  };
  const runB = {
    answer: "B says https://b.com",
...
test("buildTwinResult surfaces error state (so eval can measure error-rate)", () => {
  ...
  const degraded = buildTwinResult({
    queryId: "q",
    models,
    runA: ok,
    runB: empty,
...
test("buildTwinResult falls back when adjudication is null (overlap + union)", () => {
  ...
  const r = buildTwinResult({
    queryId: "q",
    models: { modelA: "a", modelB: "b", adjudicator: "c" },
    runA,
    runB,
    adjudication: null,
  });
...
test("buildTwinReport wraps results with the adjudicated strategy labels", () => {
  const rep = buildTwinReport([{ query_id: "q" }], {
    modelA: "a",
    modelB: "b",
    adjudicator: "c",
  });
