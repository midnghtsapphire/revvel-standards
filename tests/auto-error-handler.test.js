...
function detectErrorCategory(errorMessage) {
  ...
  if (
    errorLower.includes("infrastructure") ||
...
(async () => {
  ...
  await test("detectErrorCategory identifies infrastructure errors", () => {
    assert.equal(
      detectErrorCategory("Infrastructure error: DNS resolution failed"),
      "infrastructure",
    );
    assert.equal(
      detectErrorCategory("Network timeout connecting to server"),
      "infrastructure",
    );
  });
  ...
  await test("detectErrorCategory identifies data loss errors", () => {
    assert.equal(
      detectErrorCategory("Data loss detected in database"),
      "data_loss",
    );
    assert.equal(
      detectErrorCategory("Corrupt data preventing operation"),
      "data_loss",
    );
  });
  ...
  await test("detectErrorCategory identifies security errors", () => {
    assert.equal(
      detectErrorCategory("Unauthorized access attempt"),
      "security",
    );
    assert.equal(
      detectErrorCategory("Forbidden: insufficient permissions"),
      "security",
    );
  });
  ...
  await test("detectErrorCategory identifies rate limit errors", () => {
    assert.equal(
      detectErrorCategory("Rate limit exceeded (429)"),
      "rate_limit",
    );
    assert.equal(
      detectErrorCategory("Too many requests, please retry"),
      "rate_limit",
    );
  });
  ...
  await test("formatErrorTitle adds correct prefix for each category", () => {
    assert.ok(
      formatErrorTitle("infrastructure", "test-workflow").startsWith("[INFRA]"),
    );
    assert.ok(
      formatErrorTitle("data_loss", "test-workflow").startsWith("[DATA LOSS]"),
    );
    assert.ok(
      formatErrorTitle("security", "test-workflow").startsWith("[SECURITY]"),
    );
    assert.ok(
      formatErrorTitle("rate_limit", "test-workflow").startsWith(
        "[RATE LIMIT]",
      ),
    );
    assert.ok(
      formatErrorTitle("generic", "test-workflow").startsWith("[FAILURE]"),
    );
  });
