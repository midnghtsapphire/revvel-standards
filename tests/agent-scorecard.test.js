...
const {
  findUnresolvedRefs,
  addedLinesByFile,
} = require("../scripts/agent-scorecard/reference-resolver");
...
test("hallucination is weighted hardest", () => {
  const hallu = engine.prQualityScore({
    ciGreen: true,
    unresolvedRefs: 3,
  }).score;
  ...
  assert.ok(
    hallu < rash,
    `hallucination (${hallu}) should hurt more than rashness (${rash})`,
  );
});
...
test('claim checker catches an unbacked "added tests" claim', () => {
  const { count, findings } = checkClaims(
    "This PR adds tests for the parser.",
    ["src/parser.js"],
  );
  assert.equal(count, 1);
...
test("claim checker passes when the claim is backed", () => {
  const { count } = checkClaims("Added tests for the parser.", [
...
test("cross-model verdict parser tolerates code fences and prose", () => {
  const v = crossModel.parseVerdict(
...
test("cross-model review degrades to a no-op when the model lane fails", async () => {
  ...
  const r = await crossModel.review(
    { title: "t", body: "b", diff: "d" },
    { ask: fakeAsk },
  );
...
test("cross-model review counts flags from the model", async () => {
  const fakeAsk = async () => ({
    text: '{"flags":[{"claim":"a"},{"claim":"b"}],"summary":"two"}',
    modelUsed: "m",
  });
  const r = await crossModel.review(
    { title: "t", body: "b", diff: "d" },
...
test("recordOutcome attributes the weak link correctly", () => {
  assert.deepEqual(
    remediate.recordOutcome({ tier: "prompt-correction", postPassed: true })
      .weakLink,
    "prompt",
  );
  assert.deepEqual(
    remediate.recordOutcome({ tier: "agent-handoff", postPassed: true })
      .weakLink,
    "agent",
  );
  assert.deepEqual(
    remediate.recordOutcome({ tier: "escalate-claude", postPassed: true })
      .weakLink,
    "task-difficulty",
  );
  assert.deepEqual(
    remediate.recordOutcome({ tier: "agent-handoff", postPassed: false })
      .weakLink,
    "unresolved",
  );
});
...
test("agentTrends flags improving vs regressing dimensions", () => {
  const mk = (h, r) => ({
    ...
    agent: "x",
    quality: 70,
    dimensions: { hallucination: h, rash: r, badCode: 1, directions: 1, ci: 1 },
  });
