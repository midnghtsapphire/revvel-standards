...
if (
  test("extractVercelUrls finds Vercel URLs", () => {
    const text =
      "Check out https://myapp-123.vercel.app and https://demo-abc.vercel.app";
...
if (
  test("extractVercelUrls handles duplicates", () => {
...
if (
  test("extractVercelUrls returns empty for no URLs", () => {
...
function extractRepoReferences(text) {
  if (!text) return [];
  const repoPattern =
    /(?:^|[\s@]|repo:)([a-zA-Z0-9-]+\/[a-zA-Z0-9._-]+)(?=[\s,.)]|$)/g;
  const matches = [];
  let match;
...
if (
  test("extractRepoReferences finds valid repo references", () => {
    const text =
      "See midnghtsapphire/revvel-standards and @octocat/hello-world";
...
if (
  test("extractRepoReferences excludes URLs", () => {
...
if (
  test("extractRepoReferences handles repo: prefix", () => {
...
if (
  test("formatDate includes UTC timezone", () => {
...
if (
  test("DATE_OVERRIDE is supported in getToday", () => {
...
if (
  test("getYesterday respects DATE_OVERRIDE", () => {
...
if (
  test("HTML escaping prevents XSS in malicious input", () => {
    ...
    if (id === "@octokit/rest") {
      return {
        Octokit: function () {
          ...
          this.rest = {
            search: { issuesAndPullRequests: "search.issuesAndPullRequests" },
          };
        },
      };
    }
...
async function runProductionQueryTests() {
  ...
  if (
    await testAsync(
      "fetchPRsSince constructs correct search query with repo, type, and date filter",
      async () => {
        ...
        assert.strictEqual(
          call.params.q,
          "repo:test-org/test-repo is:pr created:>=2026-05-01",
        );
  ...
  if (
    await testAsync(
      "fetchIssuesSince constructs correct search query with repo, type, and date filter",
      async () => {
        capturedCalls.length = 0;
        ...
        assert.strictEqual(
          call.params.q,
          "repo:test-org/test-repo is:issue created:>=2026-03-15",
        );
        assert.strictEqual(call.params.sort, "created");
        assert.strictEqual(call.params.order, "desc");
        assert.strictEqual(call.params.per_page, 100);
  ...
  if (
    await testAsync(
      "fetchPRsSince filters results to only items with pull_request property",
      async () => {
  ...
  if (
    await testAsync(
      "fetchIssuesSince filters out items with pull_request property",
      async () => {
        const mod = requireProductionWithMock(async () => [
          { number: 1, pull_request: { url: "http://..." } },
          { number: 2 },
