...
test("undefined oldBody is handled without throwing", () => {
  const result = findNewlyCheckedFollowUps(undefined, "- [x] Follow-up: x");
  assert.equal(result.length, 0);
});
...
test("reordered checkboxes are still detected correctly", () => {
  const oldBody = ["- [ ] Follow-up: alpha", "- [ ] Follow-up: beta"].join(
    "\n",
  );
  const newBody = ["- [ ] Follow-up: beta", "- [x] Follow-up: alpha"].join(
    "\n",
  );
  const result = findNewlyCheckedFollowUps(oldBody, newBody);
  assert.equal(result.length, 1);
