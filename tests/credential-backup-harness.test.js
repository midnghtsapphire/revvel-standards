...
function testGithubActionsOutputUsesEnvHeredoc() {
  ...
  assert.ok(
    !output.includes("::add-mask::"),
    "must not write workflow commands to GITHUB_ENV",
  );
...
function testLegacySyncSummary() {
  ...
  withEnv(
    {
      CREDENTIAL_BACKUP_JSON: JSON.stringify({
        LEGACY_SYNC_KEY: "value-from-json",
      }),
    ...
    () => {
      ...
      assert.deepStrictEqual(summary.requested, [
        "LEGACY_SYNC_KEY",
        "ALREADY_SET_KEY",
      ]);
