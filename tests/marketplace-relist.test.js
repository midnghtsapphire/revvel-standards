...
test('itemsFromLinks extracts ASIN from dp links', () => {
  const items = relist.itemsFromLinks('https://www.amazon.com/dp/B07FDJMC9Q not-a-link');
  assert.equal(items.length, 1);
  assert.equal(items[0].asin, 'B07FDJMC9Q');
...
test('listing copy prompt forbids price research and auto-posting', () => {
  const prompt = relist.buildListingCopyPrompt({ title: 'Widget', pricePaid: 20 }, 'Used', 12);
...
test('enrichment prompt forbids account scraping', () => {
  const prompt = relist.buildEnrichmentPrompt({ title: 'Widget', link: 'https://example.com/p' });
...
test('dry-run pack for a USED item forbids AI images (real photos only)', async () => {
  ...
  const listing = await relist.buildListingPack(item, {
    outDir, condition: 'used', spendTracker: new relist.SpendTracker(), live: false,
  });
  assert.match(listing.images.policy, /real-photographs-only/);
  assert.equal(listing.images.prompts.length, 0, 'no AI prompts for used items');
...
test('dry-run pack for a NEW item records image prompts (3 or 5)', async () => {
  const outDir = tmpDir();
  const item = { id: '001-widget', title: 'Widget', asin: 'B000000000', pricePaid: 20 };
  const listing = await relist.buildListingPack(item, {
    outDir, condition: 'new', photos: 5, video: true,
    spendTracker: new relist.SpendTracker(), live: false,
  });
...
test('live pack stops generating images at the spend cap', async () => {
  ...
  const fakeRequest = async (body) => {
    if (body.modalities) {
      imageCalls++;
      return { choices: [{ message: { images: [{ image_url: { url: 'data:image/png;base64,' + Buffer.from('x').toString('base64') } }] } }] };
    }
    return { choices: [{ message: { content: '{"title":"Widget","officialImageUrls":["https://e.com/a.jpg"],"listingTitle":"Widget","description":"d","condition":"New"}' } }] };
  };
  const item = { id: '001-widget', title: 'Widget', asin: null, pricePaid: 20 };
  const listing = await relist.buildListingPack(item, {
    outDir, condition: 'new', photos: 5, live: true, request: fakeRequest,
    spendTracker: new relist.SpendTracker({ capUsd: 0.08, costPerImageUsd: 0.04 }),
  });
...
test('renderDashboard embeds packs and the human-review banner', async () => {
  ...
  assert.ok(!html.includes('Widget <b>'), 'titles are HTML-escaped in embedded JSON');
...
test('applyDecisions records approve/reject/retry onto listing.json', async () => {
  ...
  for (const id of ['001-a', '002-b', '003-c']) {
    await relist.buildListingPack({ id, title: id, pricePaid: 10 }, {
      outDir, condition: 'used', spendTracker: new relist.SpendTracker(), live: false,
    });
  }
  ...
  const read = (id) => JSON.parse(fs.readFileSync(path.join(outDir, id, 'listing.json'), 'utf8'));
...
test('image_gen lane matches the agent-models.yml profile', () => {
  const yaml = fs.readFileSync(path.join(__dirname, '..', '.github', 'agent-models.yml'), 'utf8');
  assert.ok(yaml.includes('image_gen:'), 'agent-models.yml must define image_gen');
  assert.ok(yaml.includes(relist.LANES.image_gen.primary));
  assert.ok(yaml.includes(relist.LANES.image_gen.fallback));
  assert.ok(/profile:\s*image_gen/.test(yaml), 'routing_tree needs an image_gen leaf');
});
