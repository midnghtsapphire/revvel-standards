 ...
 *
 * Main later grew the missing inputs, but recovery was still a no-op:
 *  - assignee skipped any issue already labelled `openrouter` (every stuck WR)
 *  - reset never dispatched openrouter-triage (the job that posts the comment)
 *  - triage workflow_dispatch still ran a full-repo sweep instead of one issue
 *
 * These tests pin the inputs callers/docs pass, the fields the reset actually
 * forwards, force-reroute, targeted triage, and the requirement that success
 * text is conditional on dispatch.
 */
