...
const DEVICE_TREE = yaml.parse(
  fs.readFileSync(path.join(REPO_ROOT, "config", "device-tree.yml"), "utf8"),
);
...
test("device-tree.yml roles align with the schema role enum", () => {
  const schema = JSON.parse(
    fs.readFileSync(
      path.join(REPO_ROOT, "schemas/agent-contract.schema.json"),
      "utf8",
    ),
  );
  ...
  for (const k of DEVICE_TREE.kinds || []) {
    assert.ok(
      roleEnum.has(k.role),
      `device-tree kind ${k.name} has role ${k.role} not in schema enum`,
    );
  }
});
...
test("decompose: single-Block fallback when body has no ## Blocks section", () => {
  ...
  assert.ok(
    VALID_KINDS.has(contract.blocks[0].threads[0].kind),
    "thread kind must exist in device-tree.yml",
  );
});
...
test("decompose: parses ## Blocks bullets into ordered Blocks", () => {
  ...
  // Names preserved
  assert.strictEqual(
    contract.blocks[0].name,
    "Host + Device Tree + Agent Contract Schema",
  );
  assert.strictEqual(
    contract.blocks[3].name,
    "Docs for CUDA model and agent lineup",
  );
});
...
test("decompose: classifier routes names to the right role/kind", () => {
  ...
  for (const k of kinds)
    assert.ok(VALID_KINDS.has(k), `kind ${k} not in device-tree.yml`);
});
...
test("decompose: high-stakes and owner-review-per-block labels also enable gating", () => {
  const c1 = host.decompose(
    baseWr({ body: MULTI_BLOCK_BODY, labels: ["high-stakes"] }),
  );
  const c2 = host.decompose(
    baseWr({ body: MULTI_BLOCK_BODY, labels: ["owner-review-per-block"] }),
  );
...
test("decompose: phase:* label maps to grid.goal_phase", () => {
  const contract = host.decompose(
    baseWr({ labels: ["work-request", "phase:build"] }),
  );
...
test('extractBlockHints: tolerates bold and "of N" suffixes', () => {
  const hints = host.extractBlockHints(
    "## Blocks\n- **Block 1 of 3**: Alpha\n- Block 2/3: Beta\n",
  );
