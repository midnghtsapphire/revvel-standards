...
function tmpFile(ext = ".jsonl") {
  return path.join(
    os.tmpdir(),
    `ledger-test-${Date.now()}-${Math.random().toString(36).slice(2)}${ext}`,
  );
}
...
test("readEvents: silently skips malformed JSON lines", () => {
  ...
  try {
    ...
    assert.strictEqual(
      events.length,
      2,
      "should skip 2 bad lines and keep 2 good ones",
    );
...
test("appendEvent: preserves event fields and does not overwrite caller ts", () => {
  ...
  try {
    appendEvent(
      { type: "remediation", agent: "openrouter", pr: 5, recovered: true },
      f,
    );
...
test("renderLeaderboard: single agent appears in the table", () => {
  const events = [
    { type: "score", agent: "mybot", pr: 1, quality: 75, hallucination: 0 },
  ];
...
test("renderLeaderboard: multiple agents are sorted by trust descending", () => {
  ...
  assert.ok(
    aaPos < zzPos,
    "higher trust agent (agent-aa) should appear before lower trust agent (agent-zz)",
  );
});
...
test("writeLeaderboard: writes a file and returns its path", () => {
  ...
  try {
    const agents = aggregate([
      { type: "score", agent: "bot", quality: 80, hallucination: 0 },
    ]);
...
test("writeLeaderboard: written content is valid markdown with required sections", () => {
  ...
  try {
    const agents = aggregate([
      { type: "score", agent: "x", quality: 95, hallucination: 0 },
    ]);
