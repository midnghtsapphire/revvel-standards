...
test('callPerplexityNoKey uses the python no-key bridge with sonar + auto fallback', async () => {
  const calls = [];
...
test('workflow installs helallao/perplexity-ai and does not require PERPLEXITY_API_KEY', () => {
...
test('no-key integration doc exists and references the Rex / oAudrey lane', () => {
