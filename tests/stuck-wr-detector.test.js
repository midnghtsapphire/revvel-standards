...
const issue = {
  number: 13460,
  title:
    "[WR] create engine for website ui creation using openrouter or open hands Orchestrator use swarms for research",
};
...
(async () => {
  ...
  await test("stuckAgeVerdict windows a WR by age", () => {
    ...
    const at = (mins) =>
      new Date("2026-06-13T15:00:00Z").getTime() + mins * 60000;
    ...
    assert.equal(
      stuckAgeVerdict(created, { now: at(200), maxAgeMs: 60 * 60 * 1000 }),
      "too_old",
    );
    // Old behaviour, asserted for contrast.
    assert.equal(
      stuckAgeVerdict(created, { now: at(12), minStuckAgeMs: 15 * 60 * 1000 }),
      "too_young",
    );
  });
