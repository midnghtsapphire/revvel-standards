...
function resolveInputs(wdInputs = {}, rdPayload = {}) {
  const description = wdInputs.fix_description || rdPayload.fix_description || '';
...
(async () => {
  ...
  await test('buildBranchName: includes self-heal/ prefix', () => {
    ...
    assert.ok(name.startsWith('self-heal/'), `expected self-heal/ prefix, got: ${name}`);
  });

  await test('buildBranchName: uses suffix when provided', () => {
    const name = buildBranchName('2026-07-13', 'script', 'fix-wr-lint', 'run999');
  ...
  await test('full workflow metadata — dispatch + branch + commit + labels', () => {
    ...
    const branch  = buildBranchName('2026-07-13', inputs.fixType, inputs.suffix, 'run1');
    const msg     = buildCommitMessage(inputs.fixType, inputs.description, inputs.issue);
    ...
    assert.equal(title, 'fix(workflow): fix workflow_run missing workflows list');
    assert.ok(labels.includes('devin-review'));
  });
