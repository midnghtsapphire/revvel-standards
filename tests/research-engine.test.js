...
async function run() {
  ...
  queue("uses three OpenRouter models for triangulated research", () => {
    assert.deepStrictEqual(
      engine.selectModels("triangulated"),
      engine.MODEL_TRIAD,
    );
  ...
  queue(
    "builds lane prompts with checklist and automatic-fix requirements",
    () => {
      const lane = engine.LANE_DEFINITIONS.find(
        (item) => item.id === "review-autofix",
      );
  ...
  queue(
    "keeps the competitor-pricing rule in sync with wr/WR_TEMPLATE_FULL.md",
    () => {
      ...
      const prompt = engine.buildSynthesisPrompt({ query: "Sync check" }, [
        {
      ...
      assert.ok(
        ...
        "synthesis prompt must carry the pricing sentinel",
      );
      assert.ok(
        template.includes(sentinel),
        "WR template must carry the pricing sentinel",
      );
      // The rule rejects vague price labels in favor of actual figures.
      assert.ok(
        prompt.includes("Paid tiers"),
        "synthesis prompt must reference the vague-label example",
      );
      assert.ok(
        template.includes("Paid tiers"),
        "WR template must reference the vague-label example",
      );
    },
  );
  ...
  queue("review comment carries the coder trigger phrase only for PRs", () => {
    ...
    const issueComment = engine.buildReviewRequestComment({
      outputFile: "/tmp/x.md",
      laneReports: lanes,
    });
    const prComment = engine.buildReviewRequestComment({
      outputFile: "/tmp/x.md",
      laneReports: lanes,
    ...
    assert.ok(
      !issueComment.includes("Research Findings:"),
      "issue comment must omit the coder trigger phrase",
    );
    assert.ok(
      prComment.includes("Research Findings:"),
      "PR comment must include the coder trigger phrase",
    );
  });

  queue(
    "findings comment carries synthesis so WR generation can proceed without a committed packet",
    () => {
  ...
  queue(
    "formats missing-key packets as visible infrastructure blockers",
    () => {
      ...
      assert.strictEqual(
        packet.laneReports.length,
        engine.LANE_DEFINITIONS.length,
      );
  ...
  queue(
    "runs the engine offline with mocked model calls and writes output",
    async () => {
  ...
  queue(
    "findAndRequestLinkedPrReviews processes all linked PRs concurrently",
    async () => {
      const labelCalls = [];
      const commentCalls = [];
      ...
      assert.strictEqual(
        labelCalls.length,
        3,
        "should call addLabels for each PR",
      );
      assert.strictEqual(
        commentCalls.length,
        3,
        "should call postComment for each PR",
      );
      assert.deepStrictEqual(
  ...
  queue(
    "findAndRequestLinkedPrReviews is skipped when context.prNumber is set",
    async () => {
      ...
      assert.strictEqual(
        listCalled,
        false,
        "should not list PRs when prNumber is set",
      );
    },
  );

  queue(
    "findAndRequestLinkedPrReviews continues and logs when one PR write fails",
    async () => {
      const commentCalls = [];
      const warnings = [];
      const origLog = console.log;
      console.log = (msg) => {
        if (String(msg).startsWith("::warning::")) warnings.push(msg);
      };
      try {
        await engine.findAndRequestLinkedPrReviews(
          {
      ...
      assert.strictEqual(
        commentCalls.length,
        1,
        "successful PR should still get a comment",
      );
      assert.strictEqual(
        commentCalls[0].number,
        2,
        "the successful comment should be for PR #2",
      );
      assert.strictEqual(warnings.length, 1, "failed PR should emit a warning");
      assert.ok(
        warnings[0].includes("#1"),
        "warning should identify the failed PR number",
      );
    },
  );
