...
for (const file of BIOME_WORKFLOWS) {
  ...
  test(`${file} sets GH_REPO and the token-with-fallback pattern`, () => {
    ...
    assert.ok(
      raw.includes("GH_REPO: ${{ github.repository }}"),
      "missing GH_REPO env",
    );
    assert.ok(
      raw.includes(TOKEN_FALLBACK),
      "missing token-with-fallback pattern",
    );
  });
...
test("biome-sheaf is the only worker granted contents: write (single committer)", () => {
  ...
  for (const file of BIOME_WORKFLOWS) {
    const { doc } = readWf(file);
    if (doc.permissions && doc.permissions.contents === "write")
      writers.push(file);
  }
