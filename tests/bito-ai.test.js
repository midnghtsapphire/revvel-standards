...
function shouldPostSkipComment(eventName, existingComments) {
  ...
  return !existingComments.some(
    (c) => c.user?.type === "Bot" && c.body?.includes("BITO AI review skipped"),
  );
}
...
(async () => {
  ...
  await test("shouldRunBitoReview blocks [skip-bito] title", () => {
    assert.ok(
      !shouldRunBitoReview(
        "pull_request",
        { draft: false, title: "[skip-bito] Test PR" },
        null,
      ),
    );
  });
  ...
  await test("shouldRunBitoReview checks [skip-bito] anywhere in title", () => {
    ...
    assert.ok(
      !shouldRunBitoReview(
        "pull_request",
        { draft: false, title: "PR with [skip-bito] anywhere" },
        null,
      ),
    );
  });
  ...
  await test("shouldRunBitoReview allows /review command", () => {
    assert.ok(
      shouldRunBitoReview("issue_comment", null, {
        body: "/review",
        issue: { pull_request: {} },
      }),
    );
  ...
  await test("extractPrNumber gets from workflow_dispatch", () => {
    const num = extractPrNumber("workflow_dispatch", null, null, {
      pr_number: "789",
    });
