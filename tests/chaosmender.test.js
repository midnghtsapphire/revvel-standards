...
(async () => {
  ...
  await test("loadLedger loads the real error-ledger.json", () => {
    const ledger = loadLedger(
      path.join(__dirname, "..", "config", "error-ledger.json"),
    );
  ...
  await test("LABEL-RACE-001 is in the ledger", () => {
    ...
    assert.ok(
      entry.fix_code_snippet,
      "LABEL-RACE-001 must have a fix_code_snippet",
    );
  });
  ...
  await test("scanBareRemoveLabel: does NOT flag removeLabelSafe wrapper pattern", () => {
    ...
    assert.equal(
      findings.length,
      0,
      "removeLabelSafe wrapper should not be flagged",
    );
  });
  ...
  await test("runChecks: returns findings for known patterns in a test repo", () => {
    ...
    assert.ok(
      ...
      "every finding should have a title",
    );
    assert.ok(
      findings.every((f) => f.fix),
      "every finding should have a fix",
    );
  });
  ...
  await test("buildIssueBody: groups multiple findings by errorId", () => {
    ...
    assert.equal(
      headingCount,
      1,
      "should produce one heading per error id group",
    );
  ...
  await test("chaosmender scan on real repo completes without throwing", () => {
    ...
    let findings;
    assert.doesNotThrow(() => {
      findings = runChecks(repoRoot, ledger);
    });
