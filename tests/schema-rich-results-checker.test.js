  ...
  mainEntity: [
    {
      ...
      acceptedAnswer: {
        "@type": "Answer",
        text: "A shared vocabulary for structured data.",
      },
    },
  ],
};
...
test("validateSchema handles prototype property name as @type without crashing", () => {
  ...
  assert.ok(
    result.warnings.some((w) =>
      w.message.includes("not in the Revvel known-types list"),
    ),
  );
});
...
test("validateSchema errors on BreadcrumbList ListItem with position 0", () => {
  const obj = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [{ "@type": "ListItem", position: 0, name: "Home" }],
  };
  const result = validateSchema(obj);
  assert.strictEqual(result.pass, false);
  assert.ok(result.errors.some((e) => e.property.includes("position")));
...
test("validateSchema errors on BreadcrumbList ListItem with non-numeric position", () => {
  const obj = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [{ "@type": "ListItem", position: "first", name: "Home" }],
  };
  const result = validateSchema(obj);
  assert.strictEqual(result.pass, false);
  assert.ok(result.errors.some((e) => e.property.includes("position")));
...
test("runChecks includes toolLinks in output", () => {
  ...
  assert.strictEqual(
    result.toolLinks.googleRichResults.url,
    "https://search.google.com/test/rich-results",
  );
  assert.strictEqual(
    result.toolLinks.schemaOrgValidator.url,
...
test("generateReport warns when no schemas found", () => {
  ...
  const report = generateReport(checkResult);
  assert.ok(
    report.includes('No `<script type="application/ld+json">` blocks found'),
  );
});
...
test("GOOGLE_RICH_RESULT_TYPES includes core types", () => {
  ...
  for (const t of coreTypes) {
    assert.ok(
      GOOGLE_RICH_RESULT_TYPES.has(t),
      `GOOGLE_RICH_RESULT_TYPES missing "${t}"`,
    );
  }
});
