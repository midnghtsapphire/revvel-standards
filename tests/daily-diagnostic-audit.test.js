...
test("buildUserPrompt truncates when the overall prompt budget is exceeded", () => {
  ...
  const prompt = buildUserPrompt(scopeFiles, {
    sinceHours: 48,
    maxPromptChars: 2000,
  });
  assert.ok(
    prompt.length < 6000,
    "prompt should be bounded, not include all 50 files verbatim",
  );
...
test('parseDiagnosticResponse normalizes an invalid confidence value to "low"', () => {
  const result = parseDiagnosticResponse(
    JSON.stringify({ issueFound: true, confidence: "extremely sure" }),
  );
  assert.strictEqual(result.valid, true);
...
test("renderWrBody substitutes every {TOKEN} in WR_TEMPLATE_BASIC.md", () => {
  const template = fs.readFileSync(
    path.join(REPO_ROOT, "wr", "WR_TEMPLATE_BASIC.md"),
    "utf8",
  );
  const body = renderWrBody({
    ...
    today: "2026-07-13",
    runUrl:
      "https://github.com/midnghtsapphire/revvel-standards/actions/runs/123",
  });
...
test("loadPlaybookContext returns non-empty content whether or not the live playbook file exists", () => {
  ...
  assert.match(
    context,
    /Self-Healing Correction Pattern Catalog|Pattern Catalog/,
  );
});
...
test("auto-diagnosed label is registered in .github/labels.yml", () => {
  const labelsDoc = yaml.parse(
    fs.readFileSync(path.join(REPO_ROOT, ".github", "labels.yml"), "utf8"),
  );
  const found = labelsDoc.labels.find((l) => l.name === "auto-diagnosed");
  assert.ok(
    found,
    "auto-diagnosed label must be defined in .github/labels.yml",
  );
...
test("workflow has schedule + workflow_dispatch triggers and narrow permissions", () => {
  ...
  assert.strictEqual(
    ...
    undefined,
    "must never need PR write — never opens a PR",
  );
});
...
test("workflow schedule does not collide exactly with self-healing.yml or repo-self-healer.yml", () => {
  ...
  const selfHealing = yaml.parse(
    fs.readFileSync(
      path.join(REPO_ROOT, ".github", "workflows", "self-healing.yml"),
      "utf8",
    ),
  );
  const repoSelfHealer = yaml.parse(
    fs.readFileSync(
      path.join(REPO_ROOT, ".github", "workflows", "repo-self-healer.yml"),
      "utf8",
    ),
  );
