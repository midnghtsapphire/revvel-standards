...
/**
 * Regression tests for WR #16827 fleet-maintenance package targeting
 * midnghtsapphire/music-projects. Ensures the portable package stays complete.
 */
...
const PKG = path.join(ROOT, "wr", "fleet-maintenance", "music-projects");
const WR_DOC = path.join(
  ROOT,
  "wr",
  "issues",
  "issue-16827-fleet-maintenance-midnghtsapphire-music-projects.md",
);
...
test("WR research document exists and marks tasks complete", () => {
  ...
  assert.match(body, /fleet-maintenance:midnghtsapphire\/music-projects/);
  assert.match(body, /Marketing \/ SEO keywords/i);
  assert.match(body, /Monetization path/i);
  assert.match(body, /deezer\/spleeter/);
