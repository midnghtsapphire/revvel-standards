...
const { detect, detectCi, PRIORITY } = require('../skills/client-stacks/detect-stack');
const { buildEngagementPlan, emitActionLog, slugify } = require('../skills/client-stacks/run-engagement');

const STACKS_DIR = path.join(__dirname, '..', 'skills', 'client-stacks', 'stacks');
...
function sampleDotnetRepo() {
  return tmpRepo({
    'Contoso.Api.sln': 'Microsoft Visual Studio Solution File, Format Version 12.00\n',
...
test('every stack in PRIORITY has a lane pack with prompt pack + verify commands', () => {
  for (const stack of PRIORITY) {
    ...
    assert.ok(Array.isArray(lane.verify) && lane.verify.length > 0, `${stack}: verify commands`);
    assert.ok(Array.isArray(lane.prompt_pack.idioms) && lane.prompt_pack.idioms.length > 0, `${stack}: idioms`);
    assert.ok(typeof lane.prompt_pack.pr_style === 'string' && lane.prompt_pack.pr_style.length > 0, `${stack}: pr_style`);
...
test('detects Python, TypeScript (Next.js), PHP (Laravel) and Java stacks', () => {
  const py = detect(tmpRepo({ 'pyproject.toml': '[project]\nname = "x"\ndependencies = ["fastapi"]\n', 'app/main.py': 'x = 1\n' }));
  assert.equal(py.stack, 'python');
  assert.equal(py.framework, 'fastapi');

  const ts = detect(tmpRepo({
  ...
  const php = detect(tmpRepo({ 'composer.json': '{}', artisan: '<?php\n', 'app/Model.php': '<?php\n' }));
  assert.equal(php.stack, 'php');
  assert.equal(php.framework, 'laravel');

  const java = detect(tmpRepo({ 'pom.xml': '<project/>', 'src/Main.java': 'class Main {}\n' }));
...
test('detectCi recognizes azure pipelines', () => {
  assert.equal(detectCi(tmpRepo({ 'azure-pipelines.yml': 'trigger: []\n' })), 'azure-pipelines');
...
test('reference engagement: sample .NET repo → plan with client-style PR + client verify + action log', () => {
  ...
  const plan = buildEngagementPlan({ repoPath: root, task, client: 'Contoso Ltd' });
  ...
  assert.equal(plan.pr.branch, 'contract/contoso-ltd/update-nuget-dependencies-with-security-patches');
  ...
  const logPath = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'client-log-')), 'contoso.jsonl');
