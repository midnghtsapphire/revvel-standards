...
test("assessKeyHealth treats empty/whitespace/undefined as missing", () => {
  ...
  assert.deepEqual(
    a.missing.sort(),
    ["ANTHROPIC_API_KEY", "PERPLEXITY_API_KEY"].sort(),
  );
...
test("shouldTriggerRecovery: only when AI lanes offline AND opt-in flag enabled", () => {
  ...
  assert.equal(
    shouldTriggerRecovery(offline, { BIOME_RECOVER_KEYS: "true" }),
    true,
  );
  assert.equal(
    shouldTriggerRecovery(offline, { BIOME_RECOVER_KEYS: "false" }),
    false,
  );
  assert.equal(shouldTriggerRecovery(offline, {}), false); // flag absent => off by default
  assert.equal(
    shouldTriggerRecovery(online, { BIOME_RECOVER_KEYS: "true" }),
    false,
  );
});
