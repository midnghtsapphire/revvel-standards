...
function parseIssueNumber(payload, inputs, runUrl) {
  ...
  const issueNumber = payload?.issue?.number || inputs?.issue_number;
...
function isWrIssue(title, labels, issueType) {
  ...
  const hasWrRouteTag =
    /#(?:app|apps|tool|tools|pdf|pdfs|doc|docs|api|apis|cli|mcp|skill|skills|website)\b/i.test(
      title,
    );
  const hasRouteTag =
    /(^|\s)#(app|web-app|tool|tools|pdf|docs?|documentation|api|cli|mcp|video|skill|skills|website)\b/i.test(
      title,
    );
  const hasTitleRouteTag =
    /(?:^|\s)#(?:app|apps|tool|tools|cli|api|mcp|pdf|doc|docs|skill|skills|website)(?=\s|$)/i.test(
      title,
    );
...
function isCompletionTrigger(eventName, action, labelName) {
  ...
  if (eventName === "issues" && ["opened", "reopened"].includes(action))
    return true;
  return false;
}
...
(async () => {
  ...
  await test("parseIssueNumber extracts from payload.issue", () => {
    const result = parseIssueNumber({ issue: { number: 14569 } }, {}, "");
    assert.equal(result, "14569");
  });

  await test("parseIssueNumber extracts from inputs.issue_number", () => {
    const result = parseIssueNumber({}, { issue_number: "14569" }, "");
    assert.equal(result, "14569");
  });
  ...
  await test("isWrIssue detects work-request label", () => {
    assert.equal(
      isWrIssue("Some issue", [{ name: "work-request" }], null),
      true,
    );
  });
  ...
  await test("isWrIssue detects weekly-research label", () => {
    assert.equal(
      isWrIssue("Some issue", [{ name: "weekly-research" }], null),
      true,
    );
  });
  ...
  await test("isWrIssue detects title route tags for title-only intake", () => {
    assert.equal(
      isWrIssue("places to buy pbmt tools #tool #app", [], null),
      true,
    );
    assert.equal(isWrIssue("Need a new CLI helper #cli", [], null), true);
    assert.equal(
      isWrIssue("Regulation-of-Skin-Collagen.pdf#tools #app", [], null),
      true,
    );
    assert.equal(isWrIssue("Red light guide #pdf", [], null), true);
    assert.equal(
      isWrIssue("s12967-025-07466-3.pdf#tools #apps", [], null),
      true,
    );
    assert.equal(isWrIssue("landing page #app", [], null), true);
    assert.equal(
      isWrIssue("Red light therapy stretch marks #tools #app", [], null),
      true,
    );
    assert.equal(
      isWrIssue(
        "Red Light Therapy for Stretch Marks: Science & Protocols (20#tool #app",
        [],
        null,
      ),
      true,
    ...
    assert.equal(
      isWrIssue("Red light therapy guide#tools #app", [], null),
      true,
    );
  ...
  await test("shouldCreatePr returns false for closed issues", () => {
    assert.equal(
      shouldCreatePr(closedWrIssue, "issues", "labeled", null),
      false,
    );
  });
  ...
  await test("shouldCreatePr returns false for issue:done label", () => {
    const issue = {
      ...wrIssue,
      labels: [...wrIssue.labels, { name: "issue:done" }],
    };
  ...
  await test("generateBranchName creates valid branch name", () => {
    const result = generateBranchName(
      14569,
      "[WR] I need 5 sellable .pdf on gumloop",
    );
  ...
  await test("isOperationalBotComment rejects non-bot comments", () => {
    assert.equal(isOperationalBotComment("Some comment", "user"), false);
  });
  ...
  await test("isCompletionTrigger allows wr:complete labeled event", () => {
    assert.equal(isCompletionTrigger("issues", "labeled", "wr:complete"), true);
  });

  await test("isCompletionTrigger allows wr:reset labeled event (self-heal)", () => {
    assert.equal(isCompletionTrigger("issues", "labeled", "wr:reset"), true);
  });
