...
test("missing --secrets exits non-zero", () => {
  ...
  assert(
    /--secrets is required/.test(r.stderr),
    "expected error about --secrets",
  );
});

test("malformed --repo exits non-zero", () => {
  const r = spawnSync(SCRIPT, ["--secrets", "X", "--repo", "noslash"], {
    encoding: "utf8",
  });
...
test("DRY_RUN --json dedupes, trims, and emits expected JSON shape", () => {
  const r = spawnSync(
    ...
    [
      "--secrets",
      "OPENROUTER_API_KEY, DIGITALOCEAN_API_TOKEN ,, OPENROUTER_API_KEY",
      "--repo",
  ...
  assertEq(
    summary.synced,
    ["OPENROUTER_API_KEY", "DIGITALOCEAN_API_TOKEN"],
    "synced (deduped, trimmed)",
  );
