...
function assertLabelDefinition(labelsYaml, label) {
  ...
  assert(
    pattern.test(labelsYaml),
    `.github/labels.yml missing canonical label definition for "${label}"`,
  );
}
...
test("active Work Request templates stay in sync with portable template copies", () => {
  ...
  for (const [activeName, portableName] of pairs) {
    ...
    assert(
      active === portable,
      `${activeName} must match templates/issue-template/${portableName}`,
    );
  }
});
...
test("Work Request templates apply canonical WR routing labels", () => {
  ...
  const quick = fs.readFileSync(
    path.join(
      ...
      ".github",
      "ISSUE_TEMPLATE",
      "10-openhands-system-wr.yml",
    ),
    "utf8",
  );
  const labelsYaml = fs.readFileSync(
  ...
  for (const label of canonicalWrLabels) {
    assert(
      labelsFromTemplate(primary).includes(label),
      `primary WR template missing ${label}`,
    );
    assert(
      labelsFromTemplate(quick).includes(label),
      `quick WR template missing ${label}`,
    );
...
test("WR workflows accept BASIC WR issue type, work-request label, and title route tags", () => {
  ...
  for (const workflow of [wrPrCreation, weeklyResearch]) {
    assert(
      workflow.includes("labelSet.has('work-request')"),
      "WR workflow must accept work-request label",
    );
    assert(
      workflow.includes("labelSet.has('wr:new') && hasTitleRouteTag"),
      "WR workflow must accept title-only wr:new route-tag intake",
    );
    assert(
      workflow.includes("'basic wr'"),
      "WR workflow must accept BASIC WR issue type",
    );
    assert(
      workflow.includes("hasWrRouteTag"),
      "WR workflow must detect title-only route tags",
    );
    assert(
      workflow.includes("tool|tools"),
      "WR workflow must accept #tool title route tags",
    );
    assert(
      workflow.includes("app|apps"),
      "WR workflow must accept #app title route tags",
    );
    assert(
    ...
    assert(
      workflow.includes(
        "#(app|web-app|tool|tools|pdf|docs?|documentation|api|cli|mcp|video|skill|skills|website)",
      ),
    ...
    assert(
      workflow.includes("const hasRouteTag ="),
      "WR workflow must accept title-only route tags",
    );
    assert(
      workflow.includes("'weekly-research'"),
      "WR workflow must apply weekly-research label",
    );
    assert(
      workflow.includes("'deep-research'"),
      "WR workflow must apply deep-research label",
    );
    assert(
      workflow.includes("'openrouter'"),
      "WR workflow must apply openrouter label",
    );
    assert(
      workflow.includes("'role:orchestrator'"),
      "WR workflow must apply role:orchestrator label",
    );
  }
});
...
test("weekly-research accepts route-tagged title-only source intake", () => {
  ...
  assert(
    wf.includes("looksLikeTaggedSourceIntake") && wf.includes("hasSourceUrl"),
...
test("WR workflows infer routing from title tags for title-only intake", () => {
  ...
  assert(
    openrouterAutoRoute.includes("inferOutputTypeFromTitle") &&
      openrouterAutoRoute.includes(
        "No Output Type section or route tags found",
      ),
...
test("WR routing workflows infer Output Type from title route tags", () => {
  ...
  for (const workflow of [classify, route, wrPr]) {
    assert(
      workflow.includes("#tool") && workflow.includes("#tools"),
      "must recognize #tool/#tools route tags",
    );
    assert(
      workflow.includes("#app") && workflow.includes("#apps"),
      "must recognize #app/#apps route tags",
    );
    assert(
...
test("PDF pipeline batch dropdown exists with expected options", () => {
  ...
  const tmpl = fs.readFileSync(tmplPath, "utf8");
  assert(
    tmpl.includes("id: pdf_pipeline_batch"),
    "pdf_pipeline_batch id missing",
  );
  const opts = dropdownOptionsFromTemplate(tmpl, "pdf_pipeline_batch");
  sortedEq(
    opts,
    ["Autocreate 20", "Autocreate 3", "Not applicable"],
    "PDF pipeline batch options",
  );
...
test("Work Request heavy template keeps only Output Type required", () => {
  ...
  assert(
    requiredFlagFromTemplate(tmpl, "output_type"),
    "output_type should remain required",
  );
...
test("PDF_WR_PLAYBOOK links AUTOMATED_PRODUCT_PIPELINE and PDF shape", () => {
  const pb = fs.readFileSync(
    path.join(REPO_ROOT, "workflows", "PDF_WR_PLAYBOOK.md"),
    "utf8",
  );
  assert(
    pb.includes("AUTOMATED_PRODUCT_PIPELINE.md"),
    "playbook should reference pipeline standard",
  );
  assert(
    pb.includes("standards/shapes/PDF.md"),
    "playbook should reference PDF shape",
  );
});
