...
function assertValidState(relativePath) {
  ...
  assert(
    ok,
    `${relativePath} failed state schema: ${ajv.errorsText(validate.errors)}`,
  );
  return state;
}
...
test("schema requires product_slug instead of legacy slug", () => {
  assert(
    schema.properties.product_slug,
    "schema is missing product_slug property",
  );
  assert(!schema.properties.slug, "schema still exposes legacy slug property");
  assert(
    schema.required.includes("product_slug"),
    "schema does not require product_slug",
  );
  assert(
    !schema.required.includes("slug"),
    "schema still requires legacy slug",
  );
});
...
test("checked-in agent-generated example validates against state schema", () => {
  assertValidState(
    "projects/agent-generated/_examples/demo-product/state.json",
  );
});
...
test("state shape emitted by init-product.sh validates against state schema", () => {
  ...
  assert(
    ok,
    `generated init-product state failed schema: ${ajv.errorsText(validate.errors)}`,
  );
});
...
test("legacy slug-only state is rejected", () => {
  ...
  assert(
    /product_slug/.test(errors),
    `expected missing product_slug error, got: ${errors}`,
  );
});

test("init-product.sh stamps product_slug", () => {
  const script = fs.readFileSync(
    path.join(REPO_ROOT, "scripts/init-product.sh"),
    "utf8",
  );
  assert(
    /state\["product_slug"\]\s*=\s*slug/.test(script),
    "init-product.sh does not stamp product_slug",
  );
  assert(
    !/state\["slug"\]\s*=\s*slug/.test(script),
    "init-product.sh still stamps legacy slug",
  );
});
