...
(async () => {
  ...
  await test("effectiveDueDate prefers trial_end while on trial, else renewal", () => {
    ...
    assert.deepEqual(effectiveDueDate({ status: "trial", trial_end: null }), {
      kind: "trial",
      date: null,
    });
  });
  ...
  await test("RecurseML trial: due_soon 5 days before it ends (the seeded case)", () => {
    ...
    const verdict = subscriptionVerdict(recurseml, {
      now: AT("2026-06-22T09:00:00Z"),
    });
  ...
  await test("warnWithinDays is honoured at the boundary", () => {
    ...
    assert.equal(
      subscriptionVerdict(sub, { now, warnWithinDays: 7 }).status,
      "ok",
    );
    assert.equal(
      subscriptionVerdict(sub, { now, warnWithinDays: 8 }).status,
      "due_soon",
    );
  });
  ...
  await test("reposForSubscription normalises string and list forms", () => {
    ...
    assert.deepEqual(reposForSubscription({ repositories: ["a/b", "c/d"] }), [
      "a/b",
      "c/d",
    ]);
