...
test("isProtected shields the immune system + the controller itself", () => {
  assert.equal(
    isProtected({ path: ".github/workflows/self-healing.yml" }),
    true,
  );
  assert.equal(
    isProtected({ path: ".github/workflows/fleet-controller.yml" }),
    true,
  );
  assert.equal(
    isProtected({
      name: "BIOME Sentinel",
      path: ".github/workflows/biome-sentinel.yml",
    }),
    true,
  );
  assert.equal(
    isProtected({
      path: ".github/workflows/some-research-arm.yml",
      name: "Research Arm",
    }),
    false,
  );
  ...
  const c = classifyRun(
    {
      id: 4,
      status: "in_progress",
      run_started_at: iso(20 * MIN),
      updated_at: iso(16 * MIN),
    },
    now,
  );
  ...
  const c = classifyRun(
    {
      id: 5,
      status: "in_progress",
      run_started_at: iso(75 * MIN),
      updated_at: iso(75 * MIN),
    },
    now,
  );
...
test("buildControllerFeed emits the v1 schema with dry-run cut labels", () => {
  ...
  const feed = buildControllerFeed({
    classified,
    preemptions,
    preemptEnabled: false,
    generatedAtIso: iso(0),
  });
  ...
  const exhausted = planPreemptions(cut, {
    priorReassigns: {
      ".github/workflows/foo.yml": {
        count: 2,
        tried: DEFAULT_MODEL_CHAIN.slice(0, 2),
      },
    },
  })[0];
...
test("DEFAULT_MODEL_CHAIN honours the SSOT model policy (twins + Fable 5, no denylisted models)", () => {
  ...
  assert.deepEqual(DEFAULT_MODEL_CHAIN.slice(0, 2), [
...
test("evaluate is a pure node: feed + stop + self-healing ingestion", () => {
  const runs = [
    ...
    },
    {
      ...
      run_started_at: iso(90 * MIN),
      updated_at: iso(90 * MIN),
    },
  ];
...
test("buildStopSignal + hasStop round-trip", () => {
  const stop = buildStopSignal(
    [{ id: 42, name: "Y", reason: "stalled" }],
    iso(0),
  );
