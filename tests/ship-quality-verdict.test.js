...
test('SEEDED: a red test suite can never PASS, whatever the LLM says', () => {
  assert.equal(combineVerdict({ testsResult: 'fail', aiResult: 'pass' }), 'fail');
  assert.equal(combineVerdict({ testsResult: 'fail', aiResult: 'warn' }), 'fail');
  assert.equal(combineVerdict({ testsResult: 'fail', aiResult: 'fail' }), 'fail');
...
test('ungrounded verdict (suite never ran) may WARN, never PASS', () => {
  ...
  assert.equal(combineVerdict({ testsResult: undefined, aiResult: 'pass' }), 'warn');
...
test('a green suite lets the LLM keep pass or downgrade only', () => {
  assert.equal(combineVerdict({ testsResult: 'pass', aiResult: 'pass' }), 'pass');
  assert.equal(combineVerdict({ testsResult: 'pass', aiResult: '' }), 'pass');
  assert.equal(combineVerdict({ testsResult: 'pass', aiResult: 'warn' }), 'warn');
  assert.equal(combineVerdict({ testsResult: 'pass', aiResult: 'fail' }), 'fail');
});
