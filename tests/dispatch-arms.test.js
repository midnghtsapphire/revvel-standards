...
test("extractCitations dedupes and strips trailing punctuation", () => {
  assert.deepEqual(
    extractCitations(
      "see https://x.com, and https://x.com. also https://y.com)",
    ),
...
test("parseRunnerOutput: twin/triplet JSON report -> answer/citations/cost", () => {
  ...
  const r = parseRunnerOutput(
    "twin",
    `[twin] noise on the same buffer ${JSON.stringify(report)}`,
  );
...
test("makeDispatch single: streams progress, parses deep-search stdout", async () => {
  ...
  const p = dispatch(
    { id: "a", kind: "single", model: "deep_search" },
    { onProgress: () => progress.push(1) },
  );
  child.stdout.emit(
    "data",
