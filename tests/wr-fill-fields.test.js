...
test("defaults YAML lists every field from the issue form (source-of-truth alignment)", () => {
  ...
  const formHeadings = form.body
    .filter((b) =>
      ["dropdown", "input", "textarea", "checkboxes"].includes(b.type),
    )
  ...
  assert.deepStrictEqual(
    missing,
    [],
    `defaults YAML is missing fields present on the form: ${missing.join(", ")}`,
  );
  assert.deepStrictEqual(
    extra,
    [],
    `defaults YAML has fields not on the form: ${extra.join(", ")}`,
  );
});
...
test("parseIssueBody rebuilds an ordered field list for the blank WR fixture", () => {
  ...
  assert.ok(
    headings.length >= 20,
    `expected at least 20 fields, got ${headings.length}`,
  );
  ...
  for (const h of headings)
    assert.ok(rendered.includes(`### ${h}`), `render is missing "${h}"`);
});

test("fillDeterministic fills every non-LLM blank field from the fixture", () => {
  const fields = filler.parseIssueBody(BLANK_WR_BODY);
  const result = filler.fillDeterministic({
  ...
  assert.strictEqual(
    tickedCount,
    5,
    "all 5 acknowledgement checkboxes should be ticked by default",
  );
});
...
test("fillDeterministic leaves LLM-refinement fields for the LLM pass", () => {
  ...
  const llmPending = result.changes
    .filter((c) => c.source === "llm-pending")
    .map((c) => c.heading);
  assert.ok(
    llmPending.includes("Objective") === false,
    "Objective was already filled in fixture",
  );
...
test("fillWithLlm with WR_LLM disabled fills LLM-pending fields from fallback strings", async () => {
  const fields = filler.parseIssueBody(BLANK_WR_BODY);
  const det = filler.fillDeterministic({
    title: "[WR] wire OpenHands PR review action",
    fields,
  });
  ...
  try {
    ...
    assert.match(
      bh("Required Bundle").value,
      /Implementation code, tests, docs/i,
    );
    ...
    if (prev === undefined) delete process.env.WR_LLM;
    else process.env.WR_LLM = prev;
  }
});
...
test("end-to-end fill() (WR_LLM=0) produces a body with no blank markers left", async () => {
  ...
  try {
    ...
    for (const f of defs.fields) {
      assert.ok(
        result.body.includes(`### ${f.heading}`),
        `missing heading "${f.heading}"`,
      );
    }
...
test("output_type=sellable-pdf routes PDF batch, delivery shape, and artifact bundle correctly", () => {
  ...
  assert.match(
    bh("Purchase Validation (functions-as-purchased)").value,
    /storefront/i,
  );
});
