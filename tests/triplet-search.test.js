...
test("buildNpletResult assembles the nplet block (n, k, per-model sources, consensus)", () => {
  const runs = [
    ...
    {
      answer: "B https://a.com",
      citations: ["https://a.com"],
      latency_ms: 1500,
      cost_usd: 0.004,
    },
    {
      answer: "C https://b.com",
  ...
  const r = buildNpletResult({
    ...
    adjudication: {
      ...
      citations: ["https://a.com"],
      agreement_score: 0.9,
      adjudication_quality: 0.95,
      disagreements: 0,
      unsupported_claims: 0,
    },
    adjudicatorLatencyMs: 500,
    adjudicatorCostUsd: 0.001,
    error: false,
  });
...
test("buildNpletResult unions ALL N models citations when adjudicator returns none (no dropped 3rd arm)", () => {
  ...
  assert.deepEqual(r.citations.sort(), [
    "https://a.com",
    "https://b.com",
    "https://c.com",
  ]); // c.com NOT dropped
