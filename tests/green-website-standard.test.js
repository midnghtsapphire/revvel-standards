...
function assertGreenWorkflow(relativePath, { requiresFixedDefaultUrl }) {
  ...
  const artifactStep = steps.find(
    (step) => step.name === "Upload carbon artifact",
  );
  const commitStep = steps.find(
    (step) => step.name === "Commit green website report",
  );
  const resolveStep = steps.find((step) => step.name === "Resolve target URL");
  const markerStep = steps.find(
    (step) => step.name === "Verify README carbon marker",
  );
  ...
  if (!on.push || !on.schedule || !on.workflow_dispatch) {
    throw new Error(
      `${relativePath} must support push, weekly schedule, and workflow_dispatch`,
    );
  }
  ...
  if (actionStep.env?.URL !== "${{ steps.target.outputs.url }}") {
    throw new Error(
      `${relativePath} must pass the resolved target URL to the green-website step`,
    );
  }
  if (
    !String(actionStep.with?.script || "").includes(
      "api.websitecarbon.com/site?url=",
    )
  ) {
  ...
  if (
    artifactStep.with?.name !== "carbon" ||
    artifactStep.with?.path !== "carbon"
  ) {
    throw new Error(
      `${relativePath} must upload the generated carbon file as the carbon artifact`,
    );
  }
  if (
    !resolveStep ||
    !JSON.stringify(resolveStep).includes("GREEN_WEBSITE_URL")
  ) {
    throw new Error(`${relativePath} must resolve vars.GREEN_WEBSITE_URL`);
  }
  if (
    !markerStep ||
    !String(markerStep.run || "").includes("<!-- CARBON-STATS -->")
  ) {
    throw new Error(
      `${relativePath} must guard against missing README carbon marker`,
    );
  }
  if (
    !commitStep ||
    !String(commitStep.run || "").includes("git add README.md carbon")
  ) {
  ...
  if (!content.includes("FORCE_JAVASCRIPT_ACTIONS_TO_NODE24")) {
    throw new Error(
      `${relativePath} must include the repo-standard JavaScript action runtime env`,
    );
  }
  if (
    requiresFixedDefaultUrl &&
    !content.includes("https://midnghtsapphire.github.io/revvel-standards/")
  ) {
    throw new Error(
      `${relativePath} must default to the revvel-standards GitHub Pages URL`,
    );
  }
}

test("active green website workflow follows Revvel contract", () => {
  assertGreenWorkflow(".github/workflows/green-website.yml", {
    requiresFixedDefaultUrl: true,
  });
});

test("portable green website template follows Revvel contract", () => {
  assertGreenWorkflow("templates/cicd/green-website.yml", {
    requiresFixedDefaultUrl: false,
  });
  ...
  if (
    ...
    throw new Error(
      "template must default to the consuming repo GitHub Pages URL",
    );
  }
});
...
test("README exposes carbon card insertion point or generated card", () => {
  ...
  if (
    ...
    throw new Error(
      "README.md must contain the green-action marker or generated card",
    );
  }
