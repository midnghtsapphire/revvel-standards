...
test("deriveLiveUrl: explicit live_url wins", () => {
  assert.equal(
    deriveLiveUrl("x", { live_url: "https://x.dev" }, BASE),
    "https://x.dev",
  );
});
...
test("deriveLiveUrl: internal app derives /docs/<app>/", () => {
  assert.equal(
    deriveLiveUrl(
      "osint-hub",
      { path: "osint-hub", type: "web", live_url: "" },
      BASE,
    ),
    `${BASE}/docs/osint-hub/`,
  );
...
test("deriveLiveUrl: external app with no explicit url derives nothing", () => {
  assert.equal(
    deriveLiveUrl(
      "sessiono",
      { path: "external", repo: "midnghtsapphire/sessiono", live_url: "" },
      BASE,
    ),
    "",
  );
...
test("deriveLiveUrl: non-http(s) values are ignored (not counted as live)", () => {
  ...
  assert.equal(
    deriveLiveUrl("x", { path: "x", live_url: "" }, "javascript:alert(1)"),
    "",
  );
});
...
test("classifyApp: 2xx is live, others unreachable, missing url is no-link", () => {
  ...
  assert.equal(
    classifyApp({ name: "a", url: "u", status: 404 }).state,
    "unreachable",
  );
  assert.equal(
    classifyApp({ name: "a", url: "u", status: 0 }).state,
    "unreachable",
  );
  assert.equal(
    classifyApp({ name: "a", url: "", external: false }).state,
    "no-link",
  );
  assert.equal(
    classifyApp({ name: "a", url: "", external: true }).state,
    "external-no-link",
  );
});
...
test("scoreboard overall is all-testable-live when nothing incomplete", () => {
  const apps = [
    classifyApp({ name: "a", url: "u", status: 200 }),
    classifyApp({ name: "b", url: "", external: true }),
  ];
...
test("renderScoreboardHtml escapes and is self-contained", () => {
  const sb = buildScoreboard(
    [classifyApp({ name: "x<b>", url: "u", status: 200 })],
    "t",
    BASE,
  );
