...
const WORKFLOW = path.resolve(__dirname, '..', '.github', 'workflows', 'devinci-debug.yml');
...
test('only the credential-free URL is published to the summary', () => {
  ...
  assert.ok(!raw.includes('steps.devinci.outputs.url }}'), 'credentialed url output must not be echoed');
});
