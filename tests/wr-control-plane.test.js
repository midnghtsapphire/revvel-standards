...
test("server source files exist", () => {
  ...
  assert(
    fs
      .statSync(path.join(SERVER_DIR, "wr_control_plane", "__init__.py"))
      .isFile(),
    "__init__.py missing",
  );
});
...
test("TAVILY_API_KEY becomes required when WR signals contain URLs", () => {
  ...
  assertEq(
    ...
    "tavily required when WR has URLs",
  );
});
...
test("research_mode = jules-plus-firecrawl-and-tavily when URLs + both keys", () => {
  ...
  assertEq(
    JSON.parse(r.stdout.trim()),
    "jules-plus-firecrawl-and-tavily",
    "both-tools mode",
  );
});
...
test("control_plane_status returns server, default_repo, providers, readiness", () => {
  ...
  assert(
    Array.isArray(data.readiness.required_missing),
...
test("render_control_plane_mcp_entry returns documented entry for repo profile", () => {
  ...
  assert(
    entry.args.some((a) =>
      a.includes("mcp-servers/wr-control-plane/wr_control_plane/server.py"),
    ),
...
test("render_control_plane_mcp_entry rejects unknown profiles", () => {
  ...
  assert(
    /profile must be 'repo' or 'template'/.test(r.stderr),
...
test("architecture_summary documents both current and next layers", () => {
  ...
  const data = pyJson(snippet);
  assert(
    data.current_research_layer.includes("Jules"),
    "Jules in current_research_layer",
  );
  assert(
...
test("architecture_summary exposes machine-readable v0_1_0_trade_offs", () => {
  ...
  const data = pyJson(snippet);
  assert(
    Array.isArray(data.v0_1_0_trade_offs),
    "v0_1_0_trade_offs must be a list",
  );
  assert(
    data.v0_1_0_trade_offs.length >= 3,
    "expect at least 3 documented trade-offs",
  );
  const blob = data.v0_1_0_trade_offs.join(" ").toLowerCase();
  assert(
    /substring|tokenizer|integration/.test(blob),
    "trade-offs should cover substring match",
  );
  assert(/paginat|comment/.test(blob), "trade-offs should cover pagination");
  assert(
    /env|memois|cache/.test(blob),
    "trade-offs should cover env reads / caching",
  );
});
...
test(".env.example documents every required control-plane variable", () => {
  ...
  assert(
    ...
    ".env.example OBOT_ALLOWED_HOSTS must include api.tavily.com",
  );
});
...
test("templates/mcp/mcp.revvel-custom.json exposes the wr-pr-control-plane entry", () => {
  ...
  assert(
    entry.env && "TAVILY_API_KEY" in entry.env,
...
test("scripts/setup-mcp.sh references the control-plane install path", () => {
  const script = fs.readFileSync(
    path.join(REPO_ROOT, "scripts", "setup-mcp.sh"),
    "utf8",
  );
  assert(
    /mcp-servers\/wr-control-plane/.test(script),
    "setup-mcp.sh missing wr-control-plane reference",
  );
  assert(
    /uv pip install -e \./.test(script),
    "setup-mcp.sh missing uv pip install hint",
  );
});
...
test("docs/MCP_REVVEL_CATALOG.md documents the wr-pr-control-plane entry", () => {
  ...
  assert(
    /wr-pr-control-plane/.test(cat),
    "catalog missing wr-pr-control-plane",
  );
  assert(
    /`mcp-servers\/wr-control-plane`/i.test(cat) ||
  ...
  assert(
    /v0\.1\.0 trade-offs/i.test(cat),
    "catalog must include v0.1.0 trade-offs section",
  );
  assert(
    /TAVILY_API_KEY/.test(cat),
    "catalog must show TAVILY_API_KEY in MCP entry",
  );
});
...
test("mcp-servers/wr-control-plane/README.md documents trade-offs and Tavily", () => {
  ...
  assert(
    /Known v0\.1\.0 trade-offs/i.test(readme),
    "server README must include a Known v0.1.0 trade-offs section",
  );
  assert(
    /research_mode|jules-only|tavily-search|firecrawl-agent/i.test(readme),
    "server README must document research_mode values",
  );
  assert(
    /TAVILY_API_KEY/.test(readme),
    "server README must list TAVILY_API_KEY in env block",
  );
});
...
test("SYSTEM_STATE.md records the wr-pr-control-plane MCP server", () => {
  ...
  assert(
    /wr-pr-control-plane/.test(sys),
    "SYSTEM_STATE.md missing wr-pr-control-plane",
  );
});
