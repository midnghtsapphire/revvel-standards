...
test("rubricScore matches keywords case-insensitively", () => {
  const r = rubricScore("Set GH_REPO to github.repository", [
    "gh_repo",
    "github.repository",
    "missing",
  ]);
...
test("clean improvement yields keep_candidate", () => {
  ...
  const report = compareRuns(
    scoreRun(baseline, fixtures),
    scoreRun(candidate, fixtures),
  );
...
test("error-rate spike forces rollback_candidate", () => {
  ...
  const candidate = {
    results: good.map((r, i) =>
      i === 0 ? { query_id: r.query_id, error: true } : r,
    ),
  };
  const report = compareRuns(
    scoreRun(baseline, fixtures),
...
test("bundled example fixtures evaluate to tune_candidate", () => {
  const baselineRun = loadJson(
    path.join(EVAL_DIR, "fixtures", "baseline.example.json"),
  );
  const candidateRun = loadJson(
    path.join(EVAL_DIR, "fixtures", "candidate.example.json"),
  );
...
test("twinMetricsFor returns null when no twin data is present", () => {
  const run = {
    results: [{ query_id: "x", answer: "a", citations: ["https://a.com/1"] }],
  };
    ...
    results: [
      {
        query_id: "q1",
        answer: "a",
        twin: { model_a: "m1", model_b: "m2", unsupported_claims: ["x", "y"] },
      },
      {
        query_id: "q2",
        answer: "b",
        twin: { model_a: "m1", model_b: "m2", unsupported_claims: 3 },
      },
    ],
  };
...
test("twin beating Fable within budget yields keep_twin", () => {
  const fable = evalMod.scoreRun(
    loadJson(path.join(EVAL_DIR, "fixtures", "fable.example.json")),
    fixtures,
  );
  const twin = evalMod.scoreRun(
    loadJson(path.join(EVAL_DIR, "fixtures", "twin-llm.example.json")),
    fixtures,
  );
  ...
  const twinRun = {
    results: fixtures.queries.map((q) => ({
      ...
      twin: {
        ...
        adjudication_quality: 0.9,
        sources_a: ["https://a.com/1"],
        sources_b: ["https://b.com/1"],
        unsupported_claims: 0,
      },
    })),
  };
  const cmp = compareTwinToFable(
    scoreRun(twinRun, fixtures),
    scoreRun(fableRun, fixtures),
  );
...
test("better twin over cost budget yields tune_twin", () => {
  const base = {
    ...
    twin_metrics: {
      adjudication_quality: 0.9,
      source_overlap: 0.2,
      hallucination_or_unsupported_claim_flags: 0,
    },
  };
...
test("too few queries yields needs_human_review for twin", () => {
  ...
  assert.equal(
    decideTwin(base, ref, { rubric_mean: 0.2 }).decision,
    "needs_human_review",
  );
});
...
test("evaluateStrategies prefers adjudicated twin and reports keep_twin vs Fable", () => {
  const runs = {
    ...
    twin_llm: loadJson(
      path.join(EVAL_DIR, "fixtures", "twin-llm.example.json"),
    ),
    twin_llm_adjudicated: loadJson(
      path.join(EVAL_DIR, "fixtures", "twin-llm-adjudicated.example.json"),
    ),
  };
  ...
  assert.equal(
    report.comparisons.twin_vs_fable.twin.strategy,
    "twin_llm_adjudicated",
  );
  assert.ok(
    report.strategies.fable &&
      report.strategies.twin_llm &&
      report.strategies.twin_llm_adjudicated,
  );
});
...
test("existing baseline/candidate comparison still works unchanged", () => {
  ...
  assert.equal(
    evaluate(fixtures, baselineRun, candidateRun).decision,
    "tune_candidate",
  );
});
