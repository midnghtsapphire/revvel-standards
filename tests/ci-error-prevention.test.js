...
test("Workflows should have rate limit handling patterns", () => {
  ...
  for (const file of files) {
    ...
    if (content.includes("listLabelsOnIssue")) {
      ...
      const hasRateLimitHandling =
        ...
        content.includes("e.status");
      const hasWarning =
        content.includes("core.warning") || content.includes("core.error");
  ...
  if (needsAttention.length > 0) {
    console.log(
      `   ⚠ Workflows that may need rate limit attention: ${needsAttention.join(", ")}`,
    );
    console.log(
      "   (These use listLabelsOnIssue but may need explicit rate limit handling)",
    );
  } else {
    console.log(
      `   ✓ All ${files.length} workflows have rate limit handling patterns`,
    );
  }
});
...
test("Workflows must use consistent token fallback pattern", () => {
  ...
  console.log(
    `   ✓ All ${files.length - 1} workflows use correct token fallback`,
  );
});
...
test("WR lint script must catch common issues", () => {
  ...
  const requiredChecks = [
    {
      pattern: "RAW_TOKENS|scaffold|STARS|OPEN_ISSUES",
      msg: "unsubstituted generator tokens",
    },
    {
      pattern: "BRACKET_PLACEHOLDER|placeholder|\\[.*\\]",
      msg: "raw placeholders",
    },
  ...
  const missing = requiredChecks.filter(
    (check) => !new RegExp(check.pattern, "i").test(content),
  );

  if (missing.length > 0) {
    throw new Error(
      `WR lint missing checks for: ${missing.map((m) => m.msg).join(", ")}`,
    );
  }
...
test("Workflows should have error handling patterns", () => {
  ...
  for (const file of files) {
    ...
    if (
      content.includes("github.rest") ||
      content.includes("github.graphql") ||
      ...
      if (!content.includes("catch") && !content.includes("try")) {
        ...
        if (
          content.includes("addLabels") ||
          content.includes("createComment") ||
          content.includes("listLabels")
        ) {
          if (
            !content.includes("core.warning") &&
            !content.includes("core.error")
          ) {
            needsAttention.push(file);
          }
        }
  ...
  if (needsAttention.length > 0) {
    console.log(
      `   ⚠ Workflows that may need error handling: ${needsAttention.join(", ")}`,
    );
    console.log(
      "   (These have high-risk API calls without explicit error handling)",
    );
  } else {
    console.log(
      `   ✓ All ${files.length} workflows have error handling patterns`,
    );
  }
});
...
test("Branch protection should have unique status checks", () => {
  ...
  const hasBranchProtection = files.some((f) => {
    ...
    return (
      content.includes("required_status_checks") ||
      content.includes("branch protection")
    );
  });
...
test("Auto-approve workflows must not approve drafts", () => {
  ...
  const files = fs
    .readdirSync(workflowDir)
    .filter((f) => f.includes("auto-approve") || f.includes("auto-merge"));
...
test("Standard labels must be defined in labels.yml", () => {
  ...
  const missing = requiredLabels.filter(
    (label) => !labelsContent.includes(label),
  );
...
test("WR templates must have all required sections", () => {
  ...
  for (const file of files) {
    ...
    if (file.includes("basic") || file.includes("work-request")) {
      const requiredSections = ["title", "description", "Status"];
...
test("issue-state-machine.yml must detect and close URL-only issue titles", () => {
  ...
  assert.ok(
    content.includes("urlOnlyRe") ||
      content.includes("url_only") ||
      content.includes("URL-only"),
...
test("issue-state-machine.yml must only swallow removeLabel 404 races", () => {
  ...
  const scripts = Object.values(workflow.jobs || {}).flatMap((job) =>
    (job.steps || []).map((step) => step.with?.script).filter(Boolean),
  );
  const scriptsWithRemoveLabel = scripts.filter((script) =>
    script.includes("removeLabel("),
  );
...
test("issue-state-machine.yml keeps numeric dispatch input and github-script v9", () => {
  ...
  assert.ok(
    String(transitionSteps[0]?.with?.script || "").includes(
      "github.event.inputs.target_state",
    ),
  ...
  console.log(
    "   ✓ issue-state-machine.yml keeps numeric input handling and v9 github-script",
  );
});
