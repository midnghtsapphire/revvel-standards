...
test("extractJsonFromContent parses a bare JSON string", () => {
  const result = extractJsonFromContent(
    '{"provider":"heygen","rationale":"best"}',
  );
...
test("orchestrator safeParse delegates to balanced JSON extraction", () => {
  const source = fs.readFileSync(
    path.join(
      __dirname,
      "../products/music-video-creator/src/lib/orchestrator.ts",
    ),
    "utf8",
  );
...
test("buildErrorJobStatus accepts a custom render_status", () => {
  const status = buildErrorJobStatus(
    "No keys configured",
...
test("Music Video Creator polling treats provider completion as terminal success", () => {
  ...
  const terminalStatusesBlock = source.match(
    /const TERMINAL_STATUSES = new Set\(\[\n([\s\S]*?)\n\]\);/,
  );
  assert.ok(terminalStatusesBlock, "page.tsx must define TERMINAL_STATUSES");
  assert.ok(
    /["']artifact_created["']/.test(terminalStatusesBlock[1]),
    "artifact_created must stop polling because provider completed maps to artifact_created",
  );

  const successStatusesBlock = source.match(
    /const SUCCESS_STATUSES = new Set\(\[\n([\s\S]*?)\n\]\);/,
  );
  assert.ok(successStatusesBlock, "page.tsx must define SUCCESS_STATUSES");
  assert.ok(
    /["']artifact_created["']/.test(successStatusesBlock[1]),
    "artifact_created must render as a settled success state",
  );

  const processingStatusesBlock = source.match(
    /const PROCESSING_STATUSES = new Set\(\[\n([\s\S]*?)\n\]\);/,
  );
  assert.ok(
    processingStatusesBlock,
    "page.tsx must define PROCESSING_STATUSES",
  );
  assert.ok(
    !/["']artifact_created["']/.test(processingStatusesBlock[1]),
