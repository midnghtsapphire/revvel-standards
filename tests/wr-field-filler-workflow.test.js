...
test("wr-field-filler self-heals on downstream failure via workflow_run", () => {
  ...
  assert.ok(
    Array.isArray(wr.workflows),
    "workflow_run.workflows must be an array",
  );
  assert.ok(
    wr.workflows.includes("WR PR Creation"),
    "must listen to WR PR Creation failures",
  );
  assert.ok(
    wr.workflows.includes("WR Lint"),
    "must listen to WR Lint failures",
  );
});
...
test("wr-field-filler exposes workflow_dispatch for manual runs", () => {
  assert.ok(
    ...
    "workflow_dispatch must exist for manual runs",
  );
});
...
test("wr-field-filler permissions grant issues:write and actions:write", () => {
  ...
  assert.strictEqual(
    perms.issues,
    "write",
    "issues: write is required for the fill step",
  );
  assert.strictEqual(
    perms.actions,
    "write",
    "actions: write is required to re-dispatch downstream workflows",
  );
});
...
test("wr-field-filler toggles wr:reset after a successful fill (self-heal loop)", () => {
  ...
  assert.match(
    runScript,
    /--remove-label\s+["']?wr:reset/,
    "must remove wr:reset first (label cycle)",
  );
  assert.match(
    runScript,
    /--add-label\s+["']?wr:reset/,
    "must re-add wr:reset to trigger downstream",
  );
});
...
test("wr-field-filler skips when the triggering workflow_run was successful", () => {
  ...
  assert.ok(
    skipStep,
    "discover must have a step guarded by conclusion != failure",
  );
});
