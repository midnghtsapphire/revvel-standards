...
test("buildSystemPrompt includes triage sections and provided labels", () => {
  ...
  assert.ok(
    prompt.includes(
      "do NOT classify them as incomplete just because sections are empty",
    ),
  );
  assert.ok(
    prompt.includes(
      "Infer the likely implementation ask from the title, labels, comments, and repository conventions",
    ),
  );
});
    ...
    comments: [
      {
        ...
        body: "Please review https://lnkd.in/gCM4qf6v",
      },
    ],
  });
  assert.ok(
    prompt.includes(
      "Comment 1 (midnghtsapphire): Please review https://lnkd.in/gCM4qf6v",
    ),
  );
