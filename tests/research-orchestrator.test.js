...
test("isStalled: running arm with no progress past stallMs", () => {
  assert.equal(
    isStalled({ status: "running", lastProgressAt: 0 }, 5000, 2000),
    true,
  );
  assert.equal(
    isStalled({ status: "running", lastProgressAt: 4000 }, 5000, 2000),
    false,
  );
  assert.equal(
    isStalled({ status: "done", lastProgressAt: 0 }, 5000, 2000),
    false,
  ); // not running
});
  ...
  const arms = [
    {
      ...
      model: "m1",
      status: "done",
      result: { answer: "A", citations: ["https://x.com", "https://y.com"] },
    },
    { id: "a2", model: "m2", status: "running" },
    {
      ...
      model: "m3",
      status: "done",
      result: { answer: "C", citations: ["https://y.com", "https://z.com"] },
    },
  ];
...
test("orchestrate: a stale dispatch resolving after stall→reassign does not clobber the arm", async () => {
  ...
  const dispatch = (arm, ctx) => {
    ...
    return Promise.resolve({
      answer: `fresh ${arm.model}`,
      citations: ["https://ok.com"],
    });
  };
  const res = await orchestrate({
    query: "q",
