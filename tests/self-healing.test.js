...
function checkWorkflowsPresent(workflowsList, requiredWorkflows) {
  const missing = [];
  const normalizedPaths = workflowsList.map((wf) =>
    (wf.path || "").toLowerCase(),
  );
  ...
  for (const required of requiredWorkflows) {
    ...
    const found =
      normalizedPaths.includes(ymlPath) || normalizedPaths.includes(yamlPath);
    if (!found) {
...
function identifyBrokenAreas(
  actionsFailedCount,
  stuckCount,
  agentWorkflowsStatus,
  groundingGateFailures = 0,
) {
  const broken = [];
  ...
  if (!agentWorkflowsStatus.healthy) {
    broken.push(
      `Missing workflows: ${agentWorkflowsStatus.missing.join(", ")}`,
    );
  }

  if (isGroundingGateBroken(groundingGateFailures)) {
    broken.push(
      `Grounding gate: ${groundingGateFailures} ship-quality failures`,
    );
  }
...
function buildHealingMemoryRecord({
  ts,
  broken,
  stuckRelabeled,
  reranRunIds,
...
(async () => {
  ...
  await test("checkWorkflowsPresent returns healthy when all present", () => {
    const workflows = [
      ...
      },
      {
        ...
        path: ".github/workflows/stuck-check-watchdog.yml",
      },
      {
        name: "API Rate Limit Handler",
        path: ".github/workflows/api-rate-limit-handler.yml",
      },
    ];
  ...
  await test("checkWorkflowsPresent matches file path, not display name (issue #15684)", () => {
    ...
    const result = checkWorkflowsPresent(workflows, [
      "agent-dispatcher",
      "issue-state-machine",
    ]);
    assert.ok(result.healthy);
  ...
  await test("checkWorkflowsPresent flags workflows absent from a truncated page (issue #15684)", () => {
    ...
    const result = checkWorkflowsPresent(
      truncatedFirstPage,
      REQUIRED_WORKFLOWS,
    );
    assert.ok(!result.healthy);
  ...
  await test("checkWorkflowsPresent matches on path, not display name (issue #15683)", () => {
    ...
    assert.ok(
      result.healthy,
      `expected healthy, missing: ${result.missing.join(", ")}`,
    );
  });
  ...
  await test("checkWorkflowsPresent does not false-match a slug substring in another path", () => {
    const workflows = [
      {
        name: "Other",
        path: ".github/workflows/not-agent-dispatcher-really.yml",
      },
    ];
  ...
  await test("identifyBrokenAreas detects missing workflows", () => {
    const broken = identifyBrokenAreas(0, 0, {
      healthy: false,
      missing: ["agent-dispatcher"],
    });
  ...
  await test("getSystemHealth returns needs-healing when broken", () => {
    assert.equal(
      getSystemHealth(["GitHub Actions: 5 failures"]),
      "needs-healing",
    );
  });
  ...
  await test("shouldHealGroundingGate returns true for grounding gate broken area", () => {
    assert.ok(
      shouldHealGroundingGate(["Grounding gate: 3 ship-quality failures"]),
    );
  });
  ...
  await test("full health check cycle - healthy system", () => {
    ...
    const broken = identifyBrokenAreas(
      actionsStatus.failedCount,
      stuckCount,
      workflowsStatus,
    );
  ...
  await test("verifyHealing uses same thresholds as detection (at-threshold is healed)", () => {
    const result = verifyHealing(
      FAILED_ACTIONS_THRESHOLD,
      STUCK_ISSUES_THRESHOLD,
    );
    assert.ok(result.healed);
  });
  ...
  await test("buildHealingMemoryRecord captures full provenance", () => {
    const record = buildHealingMemoryRecord({
      ...
      closedIssues: ["15684"],
      runUrl:
        "https://github.com/midnghtsapphire/revvel-standards/actions/runs/1",
    });
