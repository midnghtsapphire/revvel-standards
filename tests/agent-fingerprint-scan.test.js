...
const {
  ...
  skipPath,
  BANNED,
} = require("../scripts/agent-fingerprint-scan");
...
test("scan detects Lovable attribution string", () => {
  withTempFile(
    ...
    (tmp) => {
      ...
      assert.ok(
        hits.some((h) => h.id === "lovable-attr"),
        `expected lovable-attr, got: ${JSON.stringify(hits.map((h) => h.id))}`,
      );
    },
  );
});
...
test("scan detects Next.js default starter page text", () => {
  withTempFile(
    ".tsx",
    "export default function Home() { return <p>Get started by editing app/page</p>; }\n",
    (tmp) => {
      const hits = scan(tmp);
...
test("scan does not flag Co-Authored-By: Claude in docs/ files", () => {
  withTempFile(".md", "# Agent Profile\n\nCo-Authored-By: Claude\n", (tmp) => {
    ...
    try {
      ...
      assert.ok(
        hits.some((h) => h.id === "claude-attr-user-facing"),
        "should flag Co-Authored-By: Claude for non-docs files",
      );
    } finally {
...
test("scan snippet is truncated to 80 chars", () => {
  ...
  withTempFile(".md", longMatch + "\n", (tmp) => {
    ...
    assert.ok(
      hit.snippet.length <= 80,
      `snippet should be ≤80 chars, got ${hit.snippet.length}`,
    );
  });
});
