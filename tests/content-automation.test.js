...
const {
  ...
  callOpenRouter,
  runQualityGates,
} = require("../scripts/content-automation.js");
...
if (
  test("CONFIG format defaults to blog", () => {
    // When CONTENT_FORMAT is not set, should default to 'blog'
    assert(
      CONFIG.format === "blog" ||
        CONFIG.format === "video" ||
        CONFIG.format === "social" ||
        CONFIG.format === "email" ||
        CONFIG.format === "all",
      "CONFIG.format should be a valid format",
    );
  })
)
  passed++;
...
if (
  test("CONFIG audience has default value", () => {
    assert(
      typeof CONFIG.audience === "string",
      "CONFIG.audience should be a string",
    );
...
if (
  test("CONFIG brandVoice has default value", () => {
    assert(
      typeof CONFIG.brandVoice === "string",
      "CONFIG.brandVoice should be a string",
    );
    assert(
      CONFIG.brandVoice.length > 0,
      "CONFIG.brandVoice should not be empty",
    );
  })
)
  passed++;
...
if (
  test("CONFIG keywords is an array", () => {
    assert(
      Array.isArray(CONFIG.keywords),
      "CONFIG.keywords should be an array",
    );
  })
)
  passed++;
...
if (
  test("CONFIG autoPublish is a boolean", () => {
    assert(
      typeof CONFIG.autoPublish === "boolean",
      "CONFIG.autoPublish should be a boolean",
    );
    assert(
      CONFIG.autoPublish === false,
      "CONFIG.autoPublish should default to false when CONTENT_AUTO_PUBLISH not set to true",
    );
  })
)
  passed++;
...
if (
  test("callOpenRouter is a function", () => {
    assert(
      typeof callOpenRouter === "function",
      "callOpenRouter should be a function",
    );
  })
)
  passed++;
...
if (
  test("Output directory path generation logic", () => {
...
if (
  test("Slug generation handles special characters", () => {
    ...
    const slug = topic
      ...
      .slice(0, 50);
    assert(
      slug === "best-budget-headphones-2026-top-picks",
      "Slug should remove special chars and trim hyphens",
    );
...
if (
  test("Slug generation handles empty/special-only topics", () => {
    const emptySlug = (
      "!!!"
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
    ...
    assert(
      emptySlug === "untitled",
      'Empty slug should fallback to "untitled"',
    );
    ...
    assert(
      spaceOnlySlug === "untitled",
      'Space-only slug should fallback to "untitled"',
    );
  })
)
  passed++;
...
if (
  test("Metadata structure is valid", () => {
    ...
    assert(
      ...
      "metadata.title should be string",
    );
    assert(
      typeof metadata.format === "string",
      "metadata.format should be string",
    );
    assert(
      typeof metadata.word_count === "number",
      "metadata.word_count should be number",
    );
    assert(
      Array.isArray(metadata.keywords),
      "metadata.keywords should be array",
    );
...
if (
  test("Valid formats are recognized", () => {
...
if (
  test("Markdown headings convert to HTML", () => {
...
if (
  test("OpenRouter API key structure validation", () => {
    // Test that if a key is provided, it should be a non-empty string
    if (CONFIG.openrouterKey) {
      assert(
        typeof CONFIG.openrouterKey === "string",
        "API key should be string if provided",
      );
      assert(
        CONFIG.openrouterKey.length > 0,
        "API key should not be empty if provided",
      );
    } else {
      // If no key is provided, it should be undefined or empty
      assert(
        CONFIG.openrouterKey === undefined || CONFIG.openrouterKey === "",
        "API key should be undefined or empty when not set",
      );
    }
  })
)
...
if (
  test("runQualityGates returns failure for content with too few headings", () => {
    ...
    assert(
      ...
      "Structure gate should fail with < 3 H2 headings",
    );
  })
)
  passed++;
...
if (
  test("runQualityGates returns all-passed for well-structured content", () => {
...
Wrapping up the content with a strong call to action and final thoughts about the material covered.`;
    ...
    assert(
      allPassed,
      `Expected all gates to pass but got failures: ${JSON.stringify(results)}`,
    );
  })
)
  passed++;
...
if (
  test("runQualityGates is exported as a function", () => {
    assert(
      typeof runQualityGates === "function",
      "runQualityGates should be exported",
    );
  })
)
  passed++;
