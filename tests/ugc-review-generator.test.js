...
run(
  "falls back to compliant qualitative language when proof is missing",
  () => {
    const packet = buildLocalLeadPacket({
      brandName: "Lead Hook System",
      businessType: "real estate",
    ...
    assert.match(
      packet.complianceNotes[0],
      /defaults to qualitative social-proof language/,
    );
  },
);
