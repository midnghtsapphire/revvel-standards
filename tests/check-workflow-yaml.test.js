...
test("findInvalidWorkflows returns an array with the expected entry shape", () => {
  ...
  for (const entry of bad) {
    assert.ok(
      ...
      "each entry must have a string file field",
    );
    assert.ok(
      typeof entry.error === "string",
      "each entry must have a string error field",
    );
  }
});
...
test("heuristicError does not flag a document separator", () => {
  ...
  assert.strictEqual(
    heuristicError(yaml),
    null,
    "--- is a legal YAML document separator",
  );
});
...
test("workflowRunMissingWorkflows flags inline form that is missing workflows", () => {
  ...
  assert.ok(
    err !== null,
    "should flag inline workflow_run with no workflows key",
  );
});
