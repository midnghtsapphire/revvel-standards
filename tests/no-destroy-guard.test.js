...
const opts = {
  protectedPrefixes: ["products/", "apps/"],
  maxDeleted: 400,
  allowDestroy: false,
};
...
test("flags deletion of an existing app file", () => {
  const r = evaluateDiff(
    [
      {
        path: "products/foo/src/app/page.tsx",
        added: "0",
        deleted: "120",
        status: "D",
      },
...
test("allows a normal small edit", () => {
  const r = evaluateDiff(
    [{ path: "products/foo/page.tsx", added: "10", deleted: "4", status: "M" }],
    opts,
  );
...
test("ignores deletions outside protected paths", () => {
  const r = evaluateDiff(
    [{ path: "docs/old.md", added: "0", deleted: "5000", status: "D" }],
    opts,
  );
  assert.strictEqual(r.fail, false);
});
...
test("ignores lockfile churn in protected paths", () => {
  const r = evaluateDiff(
    [
      {
        path: "products/foo/package-lock.json",
        added: "1",
        deleted: "900",
        status: "M",
      },
