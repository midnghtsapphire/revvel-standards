...
const mockData = {
  projects: [
    {
      ...
      link: "https://oaudrey.com",
    },
    {
      name: "music-video-creator",
      status: "🔵 In Progress",
      description: "AI video tool",
    },
    {
      name: "affiliate-hub",
      status: "✅ Deployed",
  ...
  urls: [
    {
      url: "https://work-1-pmfpbcfktzqkmepq.prod-runtime.all-hands.dev",
      project: "oaudrey",
      type: "app",
    },
...
test("search finds projects by status", () => {
  ...
  assert.strictEqual(
    results.projects.length >= 1,
    true,
    "Should find projects with Live status",
  );
});
...
test("search finds URLs by URL text", () => {
  ...
  assert.strictEqual(
    results.urls.length >= 1,
    true,
    "Should find URL containing vercel",
  );
});
...
test("search finds domains by purpose", () => {
  ...
  assert.strictEqual(
    results.domains.length >= 1,
    true,
    "Should find domains by purpose",
  );
});
...
test("search is case insensitive", () => {
  ...
  assert.strictEqual(
    upper.projects.length,
    lower.projects.length,
...
test("showSummary function is exported", () => {
  ...
  assert.strictEqual(
    typeof dashboard.showSummary,
    "function",
    "showSummary should be exported",
  );
});
...
test("showProjects function is exported", () => {
  const dashboard = require("../scripts/dashboard-cli.js");
  assert.strictEqual(
    typeof dashboard.showProjects,
    "function",
    "showProjects should be exported",
  );
});
...
test("showURLs function is exported", () => {
  const dashboard = require("../scripts/dashboard-cli.js");
  assert.strictEqual(
    typeof dashboard.showURLs,
    "function",
    "showURLs should be exported",
  );
});
...
test("showDomains function is exported", () => {
  const dashboard = require("../scripts/dashboard-cli.js");
  assert.strictEqual(
    typeof dashboard.showDomains,
    "function",
    "showDomains should be exported",
  );
});
...
test("showSearch function is exported", () => {
  const dashboard = require("../scripts/dashboard-cli.js");
  assert.strictEqual(
    typeof dashboard.showSearch,
    "function",
    "showSearch should be exported",
  );
});
...
test("loadData function is exported", () => {
  const dashboard = require("../scripts/dashboard-cli.js");
  assert.strictEqual(
    typeof dashboard.loadData,
    "function",
    "loadData should be exported",
  );
});
...
test("search returns correct structure with projects, urls, domains", () => {
  ...
  assert.ok(
    ...
    "results.projects should be an array",
  );
  assert.ok(Array.isArray(results.urls), "results.urls should be an array");
  assert.ok(
    Array.isArray(results.domains),
    "results.domains should be an array",
  );
});
