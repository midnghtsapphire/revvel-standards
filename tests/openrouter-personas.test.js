...
async function runTests() {
  ...
  const missing = [...CORE_PERSONAS, ...FLEET_PERSONAS].filter(
  ...
  assertEqual(
    getPersona("@oAudrey").name,
    "oAudrey",
    "Resolves a handle with @ and mixed case",
  );
  assertEqual(
    getPersona("🕵️").name,
    "DRAGNET",
    "Resolves an emoji alias to its persona",
  );
  assertEqual(
    getPersona("circleci").name,
    "ORBIT",
    "Resolves the circleci alias to ORBIT",
  );
  assertEqual(
    getPersona("⭕").name,
    "ORBIT",
    "Resolves the ⭕ emoji alias to ORBIT",
  );
  assertEqual(
    getPersona("octopus").name,
    "OCTO",
    "Resolves the octopus alias to OCTO",
  );
  assertEqual(
    getPersona("🐙").name,
    "OCTO",
    "Resolves the 🐙 emoji alias to OCTO",
  );
  assertEqual(
    getPersona("mabl").name,
    "MENDER",
    "Resolves the mabl alias to MENDER",
  );
  assertEqual(
    getPersona("🧪").name,
    "MENDER",
    "Resolves the 🧪 emoji alias to MENDER",
  );
  ...
  assertEqual(
    normalizeMode("right away"),
    INSTANTIATION_MODES.EAGER,
    '"right away" -> eager',
  );
  assertEqual(
    normalizeMode("eager"),
  ...
  assertEqual(
    ...
    false,
    "Deferred handle is not yet instantiated",
  );
  assertEqual(
    deferred.mode,
    INSTANTIATION_MODES.ON_ASSIGNMENT,
    "Deferred handle reports on_assignment mode",
  );
  assertTrue(
    typeof deferred.assign === "function",
    "Deferred handle exposes assign()",
  );
  ...
  assertTrue(
    fleet.every(
      (f) => f.instantiated === false && typeof f.assign === "function",
    ),
