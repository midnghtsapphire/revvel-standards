...
test("pr-state-orchestrator uses the default GITHUB_TOKEN for github-script jobs", () => {
  ...
  assert.ok(
    githubScriptSteps.length > 0,
    "expected github-script steps in pr-state-orchestrator",
  );
...
test("re-sync job is present and no longer prefers ADMIN_GITHUB_TOKEN", () => {
  ...
  assert.ok(
    resyncStep,
    "expected the re-sync job to contain a github-script step",
  );
  assert.equal(
    resyncStep.with?.["github-token"],
    "${{ secrets.GITHUB_TOKEN }}",
  );
});
  ...
  for (const { jobName, step } of githubScriptSteps) {
    ...
    for (const method of [
      ...
      const rawAwait = new RegExp(
        String.raw`await\s+github\.rest\.` +
          method.replace(/\./g, String.raw`\.`) +
          String.raw`\s*\(`,
      );
      assert.ok(
