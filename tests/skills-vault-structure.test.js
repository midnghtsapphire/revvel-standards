...
test('mabl-expert.skill.yml parses and carries core metadata', () => {
  const doc = yaml.parse(
    fs.readFileSync(path.join(SKILLS_DIR, 'mabl-expert', 'mabl-expert.skill.yml'), 'utf8')
  );
  assert.equal(doc.name, 'mabl-expert');
  assert.ok(doc.version, 'version missing');
  assert.ok(doc.description, 'description missing');
  assert.ok(Array.isArray(doc.triggers) && doc.triggers.length > 0, 'triggers missing');
});

test('mabl-expert is registered in SKILLS_INDEX.yml and REGISTRY.md', () => {
  const index = fs.readFileSync(path.join(SKILLS_DIR, 'SKILLS_INDEX.yml'), 'utf8');
  const registry = fs.readFileSync(path.join(SKILLS_DIR, 'REGISTRY.md'), 'utf8');
  assert.ok(index.includes('skills/mabl-expert/'), 'SKILLS_INDEX.yml entry missing');
