...
| D016 | **REVERSE D014 — cut the RecurseML *workflow* triggers, and cut the Octopus route + fallback triggers.** `recurse-ml.yml` (`pull_request`, `push`), `octopus-route.yml` (`issues`) and `octopus-review-fallback.yml` (`issue_comment`) are commented out; every file keeps `workflow_dispatch`. | Owner cut both lanes during the cost freeze. Two things must be said plainly so this decision is not misread the way D007 was. **First: this does not remove either red check.** `recurseml/analysis` is posted by the RecurseML **GitHub App**, and the 🐙 out-of-credits comment by the **octopus-review[bot] App** — both report independently of any workflow in this repository, which is the entire finding D014 rested on. Removing them requires uninstalling or reconfiguring the Apps in repository settings (#17872, owner-only). **Second: D014 is not being called wrong.** D014's reasoning stands — the App was the mechanism actually running and D007 measured the wrong lane. What changed is cost posture, not evidence: the workflow lane no-ops without `RECURSE_ML_API_KEY` and Octopus is out of credits, so both are spending runner time to produce nothing. Reversing the trigger restoration is therefore about waste, not about value. | 2026-08-21 | @midnghtsapphire |
