...
const {
  ...
  formatUSD,
  DISCOUNT_RATE,
} = require("../lib/price-calculator");

test("discount rate is 0.20", () => {
  assert.strictEqual(DISCOUNT_RATE, 0.2);
});

test("paid order: listing = paidPrice * 0.80", () => {
  const result = calculateListingPrice({ paidPrice: 50, isVine: false });
  assert.strictEqual(result.listingPrice, 40.0);
  assert.strictEqual(result.costBasis, 50.0);
});
...
test("vine order: listing uses vineTaxValue when available", () => {
  ...
  assert.strictEqual(result.listingPrice, 24.0);
  assert.strictEqual(result.costBasis, 30.0);
});
...
test("vine order with no FMV yet: falls back to paidPrice (0)", () => {
  ...
  // Should hit minimum
  assert.strictEqual(result.listingPrice, 1.0);
});

test("minimum price enforced: cannot go below $1.00", () => {
  const result = calculateListingPrice({ paidPrice: 0.5, isVine: false });
  assert.strictEqual(result.listingPrice, 1.0);
});
...
test("rounding to 2 decimal places", () => {
  ...
  assert.ok(result.listingPrice >= 1.0);
});
...
test("detectEmailType: vine email", () => {
  assert.strictEqual(
    detectEmailType("Amazon Vine — Your product has arrived"),
    "vine",
  );
...
test("detectEmailType: delivered email", () => {
  assert.strictEqual(
    detectEmailType("Your package has been delivered"),
    "delivered",
  );
  assert.strictEqual(
    detectEmailType("Delivered: your Amazon order"),
    "delivered",
  );
});
...
test("parseAmazonEmail: parses vine email with tax value", () => {
  ...
  assert.strictEqual(result.vineTaxValue, 18.5);
});
...
test("extractImageUrlsFromHtml finds og:image", () => {
  const html =
    '<html><meta property="og:image" content="https://m.media-amazon.com/images/I/abc.jpg"/></html>';
...
test("createJob exposes visible process steps", () => {
  ...
  const job = createJob(
    { orderId: "T1", asin: "B08C4KWM9T", productTitle: "Widget" },
    { count: 3 },
  );
...
test("markListed updates listing fields", () => {
  inventoryModule.markListed("TEST-001", "FB-POST-123", 16.0);
  ...
  assert.strictEqual(product.listingPrice, 16.0);
  assert.ok(product.listedAt);
});

test("markSold updates sold fields", () => {
  inventoryModule.markSold("TEST-001", 15.0);
  const product = inventoryModule.getByOrderId("TEST-001");
  assert.strictEqual(product.sold, true);
  assert.strictEqual(product.soldPrice, 15.0);
...
test("add: creates entry with status=watching", () => {
  const entry = rfyTracker.add({
    productTitle: "Smart Garden Sensor",
    asin: "B0TESTAAAA",
    estPrice: 29.99,
    category: "Garden",
  });
...
test("getById: returns the entry that was added", () => {
  const created = rfyTracker.add({
    productTitle: "Scope USB-C to Phone",
    estPrice: 15.0,
  });
...
test("updateStatus: changes status to ordered and stores orderId", () => {
  ...
  const updated = rfyTracker.updateStatus(
    entry.rfyId,
    "ordered",
    "114-TESTORDER-001",
  );
...
test("updateStatus: throws for invalid status", () => {
  ...
  assert.throws(
    () => rfyTracker.updateStatus(entry.rfyId, "invalid"),
    /invalid status/i,
  );
});
...
test("updateStatus: throws for unknown rfyId", () => {
  assert.throws(
    () => rfyTracker.updateStatus("rfy-unknown", "missed"),
    /not found/i,
  );
});
...
test("updateNotes: updates notes field", () => {
  ...
  const updated = rfyTracker.updateNotes(
    entry.rfyId,
    "Love this but probe too expensive",
  );
...
test("updateNotes: throws for unknown rfyId", () => {
  assert.throws(
    () => rfyTracker.updateNotes("rfy-unknown", "note"),
    /not found/i,
  );
});
...
test("getSummary: returns correct counts and conversion rate", () => {
  ...
  const expectedRate =
    decided > 0 ? parseFloat((ordered / decided).toFixed(4)) : 0;
